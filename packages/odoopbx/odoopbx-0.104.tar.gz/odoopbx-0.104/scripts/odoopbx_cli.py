...
def _config_load():
    ...
    config = yaml.load(open(config_path), yaml.SafeLoader) or {}
    return config
...
def install(service):
    if service == 'all':
        os.execvp('script', ['script', '-ec', 'salt-call -l info state.highstate',
                  TYPESCRIPT_PATH])
    else:
        os.execvp('script', ['script', '-ec',
                  'salt-call -l info state.apply {}'.format(service), TYPESCRIPT_PATH])
...
def show_version():
    click.echo('Odoo PBX ID: {} CLI version: {}.'.format(
        _config_load().get('id', 'asterisk'), odoopbx.scripts.__version__))
