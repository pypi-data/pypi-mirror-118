...
def execute(method, args, kwargs):
    ...
    if __salt__['config.get']('odoo_trace_rpc'):
        log.info('Execute method: %s, args: %s, kwargs: %s', method, args, kwargs)
    else:
...
class OdooConnector:
    ...
    def send_bus_ping(self):
        self.event.fire_event({
            'model': 'remote_agent.agent', 
            'method': 'bus_sendone',
            'args': ['remote_agent/{}'.format(__grains__['id']),
             {'command': 'bus_ping'}]}, 'odoo_execute')
            ...
            if message.get('command') == 'jsonrpc':
                ...
                if message.get('reply_channel'):
                    self.event.fire_event({
                        ...
                        'method': 'bus_sendone',
                        'args': [message.get('reply_channel'), str(response)]},
                        'odoo_execute')
                if self.trace_rpc:
                    log.info('Reply sending complete.')
                return response
            # Restart
