...
from cloudtoken.core.exceptions import AuthenticationError, ConfigurationError, PluginError
...
class Plugin(CloudtokenPlugin):
    ...
    def _advance_authentication(self, response):
        ...
        response = self._execute_challenges(challenges, session_id)
        self._write_cookies()
        return response
    ...
    def _execute_challenges(self, challenges, session_id):
        ...
        for index, challenge in enumerate(challenges):
            ...
            response = self._execute_challenge(challenge, session_id)
            ...
            if self._is_new_auth_package(response):
                ...
                return self._execute_challenges(response["Result"]["Challenges"], session_id)

        raise PluginError("Exhausted all authentication challenges without success.")

    def _execute_challenge(self, challenge, session_id):
        ...
        # self._logger.debug(f"Answer payload for challenge mechanism {friendly_name} of type {type_name} is {payload}")

        return self._post(path=path, payload=payload)

    def _execute_mechanism(self, name):
        self._logger.debug(f"Executing challenge mechanism {name}")
        name = name.lower()
        handler = self._import_mechanism_handler(name)
        return handler(self._config)
