...
class ConfigStatus(wx.ScrolledWindow):
  ...
  def MakeBitmap(self):
    ...
    if application.midi_note < 0:
      self.MakeRow2("Midi note received", '-')
    else:
      self.MakeRow2("Midi note received", application.midi_note)
    self.mem_y += self.dy
...
class App(wx.App):
  ...
  def OnReadSound(self):	# called at frequent intervals
    ...
    if self.midi_note >= 0:
      if self.timer - self.midi_time0 > 0.3:
        self.midi_note = -1
      ...
      if len(self.midi_message) == 3:
        ...
        if status == 0x90:	# Note on
          if not velocity:
            ...
            try:
              name = configure.Settings[4]["MidiNoteDict"][str(note)]
              btn = self.idName2Button[name]
            except:
              pass #traceback.print_exc()
              #print ("Failed to find Midi button for", note)
            else:
