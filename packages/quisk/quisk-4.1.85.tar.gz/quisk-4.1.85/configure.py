...
class QuiskControl(wx.Control):
  ...
  def MakeMenu(self):
    ...
    for idName in application.idName2Button:
      if idName in conf.BandList or idName == "Time":
...
class RadioSection(BaseWindow):		# The pages for each section in the second-level notebook for each radio
  ...
  def MakeControls(self):
    ...
    if self.section == "Keys":
      ...
      index = 0
      for txt_note in self.MidiNoteDict:		# txt_note is a string
        int_note = int(txt_note, base=10)
        self.list_ctrl.InsertItem(index, txt_note)
        self.list_ctrl.SetItemData(index, int_note)
        self.list_ctrl.SetItem(index, 1, self.MidiNoteDict[txt_note])
        index += 1
  ...
  def OnMidiDelete(self, event):
    ...
    del self.MidiNoteDict[txt_note]
    local_conf.settings_changed = True
    if index > 0:
      index -= 1
  ...
  def OnMidiMenu(self, ctrl):
    ...
    if txt and index >= 0:
      ...
      self.MidiNoteDict[txt_note] = txt
      local_conf.settings_changed = True
