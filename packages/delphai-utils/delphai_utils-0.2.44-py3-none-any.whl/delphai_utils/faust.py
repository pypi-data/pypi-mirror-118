...
class FaustAgent():
  ...
  def setup_metrics(self):
    self.metrics.inbound_counter = Counter(
      f'delphai_{self.agent_id.replace(".", "_")}_inbound_counter',
      'incoming requests',
      const_labels={
        'step': self.step,
    ...
    self.metrics.processed_counter = Counter(
      f'delphai_{self.agent_id.replace(".", "_")}_processed_counter',
      'processed requests',
      const_labels={
        'step': self.step,
        'pipeline': self.agent.app.conf.id,
    ...
    self.metrics.lag_timer = Counter(
      f'delphai_{self.agent_id.replace(".", "_")}_lag_timer',
