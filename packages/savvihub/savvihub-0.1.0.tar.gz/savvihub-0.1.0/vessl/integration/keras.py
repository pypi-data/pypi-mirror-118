...
from vessl.util.constant import VESSL_PLOTS_FILETYPE_IMAGE, VESSL_PLOTS_FILETYPE_IMAGES
...
class ModelCallback(keras.callbacks.Callback):
    ...
    def _should_upload_model(self, logs=None) -> bool:
        ...
        if target_metric_name not in logs:
            warnings.warn(f"metric {target_metric_name} not in train logs: {logs}")
            return False
