...
def load_model(model_path: str,
    ...
    model_json_str = json.dumps(model_info['nn_model'])

    model = model_class()
    model.nn_model = tensorflow.keras.models.model_from_json(model_json_str, custom_objects)
    if load_weights:
    ...
    if type(model.nn_model.layers[-1]) == CRF:
...
def convert_to_saved_model(model,
    ...
    if inputs is None:
        inputs = {i.my_name: i for i in model.nn_model.inputs}
    if outputs is None:
        outputs = {o.my_name: o for o in model.nn_model.outputs}
