...
import io
import json
from kdmt.file import unicode_csv_reader, validate_file_hash, extract_archive
...
CORPUS_PATH = os.path.join(DATA_PATH, 'datasets')
...
def _create_dataset_directory(dataset_name):
    def decorator(func):
        ...
        def wrapper(root=CORPUS_PATH, *args, **kwargs):
...
def _download_extract_validate(root, url, url_md5, downloaded_file, extracted_file, extracted_file_md5,
    ...
    if os.path.exists(extracted_file):
        with open(os.path.join(root, extracted_file), 'rb') as f:
            if validate_file_hash(f, extracted_file_md5, hash_type):
