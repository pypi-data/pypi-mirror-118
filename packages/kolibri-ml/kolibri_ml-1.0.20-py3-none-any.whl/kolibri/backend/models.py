...
torch_models_path= resources.get('models/kolibri/torch/models.json').path
tensorflow_models=read_json_file(tensoflow_models_path)
torch_models=read_json_file(torch_models_path)
...
def get_model(model_name, weights=None, bakend='tensorflow'):
    if isinstance(model_name, list):
      ...
      "parameters": {
        ...
        if model is None and bakend=='tensorflow':
            model=tensorflow_models.get(model_name.lower(), None)
        elif model is None and bakend == 'torch':
            model = torch_models.get(model_name.lower(), None)
