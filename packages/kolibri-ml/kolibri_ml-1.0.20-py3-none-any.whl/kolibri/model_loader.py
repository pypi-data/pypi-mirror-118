...
class ModelLoader(object):
    ...
    def create(model_metadata,  component_builder=None,skip_validation=False):
        ...
        i=0
        component_names=model_metadata.component_classes
        for component_name in component_names:
            if isinstance(component_name, list):
                i+=1
                component=[component_builder.load_component(
                c, model_metadata.model_dir, **context) for c in component_name]
                component=[(c.name, c) for c in component]
            else:
                component = component_builder.load_component(
                    component_name, model_metadata.model_dir, **context)
            try:
                if isinstance(component, list):
                    sub_steps.append(('sub_pipeline_'+str(i), Pipeline(component)))
                else:
        ...
        if len(sub_steps)>=2:
            steps.insert(0, ('feature_union',FeatureUnion(sub_steps)))
        elif len(sub_steps)==1:
