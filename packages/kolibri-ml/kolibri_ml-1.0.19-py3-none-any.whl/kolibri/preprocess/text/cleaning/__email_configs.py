...
email_caution_or_front_content=open(resources.get('corpora/gazetteers/default/email_caution_or_front_content.txt').path, encoding='utf-8').readlines()
...
pattern_caution_or_front_content =r'(?P<caution>^\s*(^\s*'+ r'|'.join([c.strip() for c in email_caution_or_front_content + sent_from_my_device if c.strip()])+'))'
