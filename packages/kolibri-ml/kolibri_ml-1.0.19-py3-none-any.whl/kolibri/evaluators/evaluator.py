...
class ClassifierEvaluator(object):
    ...
    def cross_validate_predict(self, classifier, X, y, folds=4, ratio_to_use=1, n_jobs=4):
        ...
        try:
            pred = cross_val_predict(classifier, X, y, cv=cv, n_jobs=n_jobs, method='predict_proba', verbose=1)
            return pred
        except:
    ...
    def classification_report(y_true, y_pred, target_names):
        if y_true is not None and y_pred is not None:
            cr= classification.classification_report(y_true, y_pred, target_names)
            return {str(key):value for key, value in cr.items()}
        return None

    def cv_compute_performance_report(self, X=None, y=None, labels=None, folds=4, ratio_to_use=1, n_jobs=4):
    ...
    def compute_performance_report(self, X, y, X_val, y_val, labels=None):
        ...
        self._y_pred = self.estimator.fit(X, y).predict(X_val)

        return self.get_performance_report(y_val, self.y_pred, self.target_names)
