...
from kolibri.core.combine_features import FeatureUnion
...
#        self.config
    ...
    def _build_pipeline(params):
        ...
        sub_pipelines=[]
        # Transform the passed names of the pipeline components into classes
        for i, component_name in enumerate(params['pipeline']):
            if isinstance_of_class(component_name):
                component=component_name
                component_name=component.name
            elif isinstance(component_name, list):
                sub_pipelines.append(('sub_pipeline_'+str(i), ModelTrainer._build_sub_pipeline(component_name, params)))
                continue
            else:
        ...
        if len(sub_pipelines)>=2:
            feature_union= ('feature_union',FeatureUnion(sub_pipelines))
            steps.insert(0,feature_union)
        elif len(sub_pipelines)==1:
            sub_pipelines.extend(steps)
            steps=sub_pipelines
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if self.config['evaluate-performance']==True and self.config['save-evaluation-output']==True and self.pipeline.estimator[1].validatation_data is not None:
            self.performance_data=pd.DataFrame(X)
            self.performance_data['class']=y
            self.performance_data['prediction']=self.pipeline.estimator[1].validatation_data[:,0]
            self.performance_data['probability']=self.pipeline.estimator[1].validatation_data[:,1]
    ...
    def persist(self, path, fixed_model_name=None):
        ...
        metadata.update(self.pipeline.persist(dir_name))
