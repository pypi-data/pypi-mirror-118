...
class Tile:
	...
	def __init__(self, screen):
		...
		self.cycleIndex = 0
		self.decorated = Config.misc('decorations')
	...
	# turns window decorations on/off (openbox only)
	def _switch_decor(self):
		for window in self.storage.get_all():
			if self.decorated:
				window.remove_decorations()
			else:
	...
	# changes window margins by these values
	def _change_margins(self, marChange, intMarChange):
		intMar = self.state.get('internal_margin')
		mar = self.state.get('margin')
		if intMar != None and mar != None:
			self.state.set('internal_margin', max(0, intMar + intMarChange))
			self.state.set('margin', max(0, mar + marChange))
	...
	def switch_decor(self):
		self._switch_decor()

	def increase_margins(self):
		intMarStep = Config.misc('internal_margin_step')
		marStep = Config.misc('margin_step')
		if intMarStep and marStep:
			self._change_margins(marStep, intMarStep)
