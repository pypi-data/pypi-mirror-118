...
UNIT_TEST_COVERAGE_THRESHOLD: int = 80
INTEGRATION_TEST_COVERAGE_THRESHOLD: int = 70
...
def unit_test() -> float:
    ...
    test_results_dir: Path = Path(root_path).resolve().joinpath('test-results')
...
def units(
        ...
        ignore_integration_tests: bool = typer.Option(False),
    ...
    if not ignore_unit_tests or not ignore_integration_tests:
    ...
    INTEGRATION TESTING
    """
    if not ignore_integration_tests:
