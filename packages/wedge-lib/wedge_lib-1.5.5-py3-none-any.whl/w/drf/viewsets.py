...
class ViewSet(viewsets.ViewSet):
    ...
    def get_validated_data(self, serializer_key=None, serializer_args=None):
        ...
        Args:
            ...
            serializer_args(None|dict): Additional args to pass
                for serializer instanciation
        Returns:
            dict
        """
        ...
        serializer_class = self.get_serializer_class(serializer_key)
        serializer = serializer_class(data=self.request.data, **serializer_args)
        if self.request.query_params and not self.request.data:
            serializer = serializer_class(
                data=self.request.query_params, **serializer_args
            )
    ...
    def check_is_list_valid(self, serializer_key=None):
        try:
            validated_data = self.get_validated_data(serializer_key, {"many": True})
        except ValidationError as e:
            errors = self._simplify_errors(e.detail)
            raise exceptions.ValidationError(errors)
