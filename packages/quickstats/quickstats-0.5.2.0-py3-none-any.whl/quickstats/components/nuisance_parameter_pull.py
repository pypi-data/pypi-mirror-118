...
class NuisanceParameterPull(object):
    ...
    def _run_pulls(input_file:str, data:str, snapshot:str, nuis_name:str, poi_name:str, 
                   ...
                   outdir:str='output', cache=True, logfile_path=None, **kwargs):
    ...
    def run_pulls(self, input_file:str='workspace.root', data:str='combData', poi:str='', 
                  snapshot:str='nominalNuis', outdir:str='output', profile:str='',
                  fix:str='', workspace:Optional[str]=None, model_config:Optional[str]=None,
                  ...
                  strategy:int=0, fix_cache:bool=True, fix_multi:bool=True, offset:bool=True, optimize:int=2,
                  parameter:str="", max_calls:int=-1, max_iters:int=-1, batch_mode:bool=False, 
                  int_bin_precision:float=-1.,parallel:int=0, cache:bool=True, 
                  exclude:str='', **kwargs):
        ...
        if parallel:
            ...
            arguments = (repeat(input_file), repeat(data), repeat(snapshot), nuis_names, 
                         ...
                         repeat(num_cpu), repeat(offset), repeat(optimize), repeat(eigen), 
                         repeat(fix_cache), repeat(fix_multi), repeat(max_calls), repeat(max_iters),
                         repeat(batch_mode), repeat(int_bin_precision),
            ...
            for i, nuis_name in enumerate(nuis_names):
                self._run_pulls(input_file, data, snapshot, nuis_name, poi, fix, profile, 
                                binned, workspace, model_config, minimizer_type, 
                                minimizer_algo, precision, eps, strategy, num_cpu, offset, 
                                optimize, eigen, fix_cache, fix_multi, max_calls, max_iters,
                                batch_mode, int_bin_precision,
                                outdir, cache, logfile_paths[i])
