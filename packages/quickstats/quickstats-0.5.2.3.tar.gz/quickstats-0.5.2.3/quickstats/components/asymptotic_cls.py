...
class AsymptoticCLs(AnalysisObject):
    ...
    def evaluate_limits(self):
        ...
        # profile NP to mu = 0 if unblinded else no profiling
        poi_profile = None if self.do_blind else 0
        globs_np_matching = not self.do_blind
        asimov_data_0 = self.model.generate_asimov(self.poi.GetName(), poi_val=0,
                                                   poi_profile=poi_profile, dataset=self.data,
                                                   globs_np_matching=globs_np_matching)
