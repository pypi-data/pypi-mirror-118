...
import quickstats
from quickstats.components.numerics import is_integer, pretty_float
...
class ExtendedModel(object):
    ...
    def generate_asimov(self, poi_name:str, poi_val:float, poi_profile:Optional[float]=None,
                        conditional_mle:bool=True, do_import:bool=True,
        ...
        """
            ...
            Arguments:
                ...
                poi_val: float
                    Generate asimov data with POI set at the specified value.
                poi_profile: (Optional) float
                    Perform nuisance parameter profiling. If None, nuisance parameters are kept at their
                    initial values when generating the asimov data. Otherwise, a fit is performed by setting 
                    the POI at the specified value (POI is fixed if conditional_mle=True, free otherwise) to
                    obtain the best-fit nuisance parameter values before generating the asimov data.
                    category label. Other keywords are: `{index}` which is the category index.
                do_import: bool, default=True
                    Import the generated asimov data to the current workspace.                    
                conditional_mle: bool, default=True
                    Perform conditional maximum likelihood estimation. If True, best-fit values are obtained
                    for mu = poi_profile. If False, the overal best-fit values are obtained.
        ...
        asimov_data_name = names['asimov'].format(mu=pretty_float(poi_val), 
                                                  mu_cond=pretty_float(poi_profile))
