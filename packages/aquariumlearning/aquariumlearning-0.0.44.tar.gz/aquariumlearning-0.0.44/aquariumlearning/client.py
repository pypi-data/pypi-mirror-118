...
class Client:
    ...
    def create_or_update_dataset(
        ...
        Args:
            ...
            seed_dataset_name_for_unlabeled_search (Optional[str], optional): (Only for unlabeled indexed dataset search). The name of the existing dataset or inf set whose issue elts will be used as the seed for unlabeled indexed search.
        ...
        if is_unlabeled_indexed_dataset:
            # TODO: is this actually a necessary gate?
            if pipeline_mode != "BATCH":
                raise Exception(
                    f"Unlabeled indexed datasets must be uploaded with BATCH mode. {pipeline_mode} is not supported."
                )
            if not seed_dataset_name_for_unlabeled_search:
                raise Exception(
                    "seed_dataset_name_for_unlabeled_search must be specified if uploading an unlabeled indexed dataset."
                )
        ...
        if is_unlabeled_indexed_dataset:
            ...
            payload[
                "seed_dataset_name_for_unlabeled_search"
            ] = seed_dataset_name_for_unlabeled_search
