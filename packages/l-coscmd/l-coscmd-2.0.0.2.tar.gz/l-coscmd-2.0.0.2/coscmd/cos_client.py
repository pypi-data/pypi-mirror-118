...
class CoscmdConfig(object):
    ...
    def __init__(self, appid, region, endpoint, bucket, secret_id, secret_key, token=None,
        ...
        logger.debug("config parameter-> appid: {appid}, region: {region}, endpoint: {endpoint}, l5: {l5}, bucket: {bucket}, part_size: {part_size}, max_thread: {max_thread}".format(
            appid=appid,
            region=region,
            endpoint=endpoint,
            l5=kwargs['l5'],
    ...
    def l5_uri(self, path=None):
        uri = self.uri(path)
        host = urlparse(uri).netloc
        l5_uri = uri
        ip, port = self.ip_port()
        if ip != "" and port != 0:
            l5_uri.replace(host, ip + ":" + str(port))
            self._oldurl = uri
        logger.debug("l5_uri:{l5_uri}".format(l5_uri=l5_uri))
        return host, l5_uri
    ...
    def ip_port(self):
        ...
        if self._l5 != '':
            ...
            if len(splits) == 2:
                ...
                if ret == 0:
                    ip = qos['hostIp']
                    port = int(qos['hostPort'])
        logger.debug("l5-ip:{ip},port:{port}".format(ip=ip, port=port))
        return ip, port
...
class Interface(object):
    ...
    def get_qcloud_cos_client(self):
        try:
            if self._conf._endpoint != "":
                sdk_config = qcloud_cos.CosConfig(Endpoint=self._conf._endpoint,
                ...
                sdk_config = qcloud_cos.CosConfig(Region=self._conf._region,
                                                  SecretId=self._conf._secret_id,
                                                  SecretKey=self._conf._secret_key,
                                                  Token=self._conf._token,
                                                  Scheme=self._conf._schema,
                                                  ...
                                                  UA=self._ua,
                                                  Timeout=self._timeout)
            ip, port = self._conf.ip_port()
            if ip != "" and port != 0:
                sdk_config.set_ip_port(ip, port)
            return qcloud_cos.CosS3Client(sdk_config, self._retry)
    ...
    def list_part(self, cos_path):
        ...
        try:
            while IsTruncated == "true":
                host, url = self._conf.l5_uri(path=quote(to_printable_str(cos_path)) + '?uploadId={UploadId}&upload&max-parts=1000&part-number-marker={nextmarker}'.format(
                    UploadId=self._upload_id,
                    nextmarker=NextMarker))
                http_headers = {}
                http_headers['Host'] = host
                logger.debug("call:{url}".format(url=url))
                rt = self._session.get(url=url, auth=CosS3Auth(self._conf), headers=http_headers)
    ...
    def local2remote_sync_check(self, local_path, cos_path, **kwargs):
        ...
        if kwargs['sync'] is True:
            try:
                rt = self.get_qcloud_cos_client().head_object(
                    Bucket=self._conf._bucket,
                    Key=cos_path
                )
            except Exception:
    ...
    def single_upload(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        try:
            ...
            logger.warn(to_unicode(e))
            return 0
        host, url = self._conf.l5_uri(path=quote(to_printable_str(cos_path)))
    ...
    def multipart_upload(self, local_path, cos_path, _http_headers='{}', **kwargs):
        ...
        def init_multiupload():
            ...
            try:
                ...
                return -1
            try:
                rt = self.get_qcloud_cos_client().create_multipart_upload(Bucket=self._conf._bucket,
                                                          Key=cos_path,
                                                          **http_headers)
            ...
            def multiupload_parts_data(local_path, offset, length, parts_size, idx):
                for j in range(self._retry):
                    try:
                        ...
                        logger.warn(to_unicode(e))
                        return -1
                    host, url = self._conf.l5_uri(path=quote(to_printable_str(
        ...
        def complete_multiupload():
            ...
            try:
                self.get_qcloud_cos_client().complete_multipart_upload(self._conf._bucket,
                                                       cos_path,
                                                       self._upload_id,
                                                       {'Part': lst})
                try:
    ...
    def copy_folder(self, source_path, cos_path, _http_headers='{}', **kwargs):
        ...
        try:
            if self._conf._endpoint is not None:
                ...
                ip, port = self._conf.ip_port()
                if ip != "" and port != 0:
                    sdk_config_source.set_ip_port(ip, port)
                self._client_source = qcloud_cos.CosS3Client(sdk_config_source, self._retry)
        except Exception as e:
            logger.warn(to_unicode(e))
    ...
    def copy_file(self, source_path, cos_path, _http_headers='{}', **kwargs):
        ...
        try:
            if self._conf._endpoint is not None:
                ...
                if ip != "" and port != 0:
                    sdk_config_source.set_ip_port(ip, port)
                _source_client = qcloud_cos.CosS3Client(sdk_config_source, self._retry)
        except Exception as e:
            logger.warn(to_unicode(e))
            logger.warn(u"CopySource is invalid: {copysource}".format(
                copysource=source_path))
        ...
        try:
            if _move:
                logger.info(u"Move cos://{src_bucket}/{src_path}   =>   cos://{dst_bucket}/{dst_path}".format(
                    ...
                    dst_bucket=self._conf._bucket,
                    dst_path=cos_path))
                self.get_qcloud_cos_client().copy(Bucket=self._conf._bucket,
                                  Key=cos_path,
                ...
                logger.info(u"Copy cos://{src_bucket}/{src_path}   =>   cos://{dst_bucket}/{dst_path}".format(
                    ...
                    dst_bucket=self._conf._bucket,
                    dst_path=cos_path))
                rt = self.get_qcloud_cos_client().copy(Bucket=self._conf._bucket,
                                       Key=cos_path,
                                       CopySource=copy_source,
                                       CopyStatus=_directive,
                                       PartSize=self._conf._part_size,
    ...
    def delete_folder(self, cos_path, **kwargs):
        ...
        if _versions:
            ...
            while IsTruncated == "true":
                ...
                for i in range(self._retry):
                    if VersionIdMarker == "null":
                        VersionIdMarker = ""
                    try:
                        rt = self.get_qcloud_cos_client().list_objects_versions(
                            Bucket=self._conf._bucket,
                            KeyMarker=KeyMarker,
                ...
                if len(deleteList['Object']) > 0:
                    rt = self.get_qcloud_cos_client().delete_objects(Bucket=self._conf._bucket,
                                                     Delete=deleteList)
                if 'Deleted' in rt:
            ...
            while IsTruncated == "true":
                ...
                for i in range(self._retry):
                    try:
                        rt = self.get_qcloud_cos_client().list_objects(
                            Bucket=self._conf._bucket,
                            Marker=NextMarker,
                            MaxKeys=1000,
                            Prefix=cos_path,
            ...
            while IsTruncated == "true":
                ...
                if len(deleteList) > 0:
                    for file in deleteList:
                        try:
                            self.get_qcloud_cos_client().delete_object(
                                Bucket=self._conf._bucket,
                                Key=file['Key'],
                                VersionId=file['VersionId'])
                            self._have_finished += 1
    ...
    def delete_file(self, cos_path, **kwargs):
        ...
        host, url = self._conf.l5_uri(path="{path}?versionId={versionId}"
                             .format(path=quote(to_printable_str(cos_path)), versionId=_versionId))
        http_headers = {}
        http_headers['Host'] = host
        try:
            logger.debug("call:{url}".format(url=url))
            rt = self._session.delete(url=url, auth=CosS3Auth(self._conf), headers=http_headers)
    ...
    def list_multipart_uploads(self, cos_path):
        ...
        while IsTruncated == "true":
            IsTruncated = 'false'
            try:
                rt = self.get_qcloud_cos_client().list_multipart_uploads(
                    Bucket=self._conf._bucket,
                    Prefix=cos_path,
                    Delimiter='',
                    KeyMarker=KeyMarker,
            ...
            while IsTruncated == "true":
                ...
                if len(abortList) > 0:
                    for file in abortList:
                        try:
                            rt = self.get_qcloud_cos_client().abort_multipart_upload(
                                Bucket=self._conf._bucket,
                                Key=file['Key'],
    ...
    def single_download(self, cos_path, local_path, _http_headers='{}', **kwargs):
        ...
        try:
            ...
            return -1
        try:
            rt = self.get_qcloud_cos_client().get_object(
                Bucket=self._conf._bucket,
                Key=cos_path,
                **http_headers
            )
    ...
    def restore_file(self, cos_path, **kwargs):
        ...
        try:
            ...
            self.get_qcloud_cos_client().restore_object(
                Bucket=self._conf._bucket,
                Key=cos_path,
