...
path = os.path.dirname(ccxt_1_55_84.__file__)
if 'site-packages' in os.path.dirname(ccxt_1_55_84.__file__):
    raise Exception("You are running test_async.py/test.py against a globally-installed version of the library! It was previously installed into your site-packages folder by pip or pip3. To ensure testing against the local folder uninstall it first with pip uninstall ccxt_1_55_84 or pip3 uninstall ccxt_1_55_84")
...
def try_all_proxies(exchange, proxies=['']):
    ...
    for num_retries in range(0, max_retries):
        ...
        except (ccxt_1_55_84.RequestTimeout, ccxt_1_55_84.AuthenticationError, ccxt_1_55_84.NotSupported, ccxt_1_55_84.DDoSProtection, ccxt_1_55_84.ExchangeNotAvailable, ccxt_1_55_84.ExchangeError) as e:
...
for id in ccxt_1_55_84.exchanges:
    if id == 'theocean':
        continue
    exchange = getattr(ccxt_1_55_84, id)
    exchange_config = {'verbose': argv.verbose}
    if sys.version_info[0] < 3:
        exchange_config.update({'enableRateLimit': True})
    if id in config:
        exchange_config = ccxt_1_55_84.Exchange.deep_extend(exchange_config, config[id])
