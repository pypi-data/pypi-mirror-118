...
FFMPEG_COMMAND: str = 'ffmpeg -i "{track}.webm" -vn -c:a copy -- {track}.ogg'
CACHE_DIRECTORY: str = os.path.join(tempfile.gettempdir(), 'yt-music-headless')
...
class Extractor(YouTube):
    ...
    async def get_stream_urls(self, response_dict: dict) -> dict[int, str]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post('https://music.youtube.com/youtubei/v1/search',
            ...
            for raw_track in results:
                ...
                try:
                    subtitle: str = ''.join(map(
                        lambda object: object['text'], raw_track['flexColumns'][1]['musicResponsiveListItemFlexColumnRenderer']['text']['runs']))
                    duration: str = subtitle.split('\u2022')[2].strip()
                    track_duration: int = (
                        int(duration.split(':')[0]) * 60 + int(duration.split(':')[1])) * 10000
                    track: dict = {
                        ...
                        'track_artists': list(map(lambda string: string.strip(), re.split(',|&', subtitle.split('\u2022')[0]))),
                        ...
                        'album_name': subtitle.split('\u2022')[1].strip(),
...
async def track(track_id: str, itag: int = 251) -> RedirectResponse:
    if track_id in track_responses:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post('https://www.youtube.com/watch',
                ...
                response_dict: dict = (await response.json())[2]['playerResponse']
                track_responses[track_id] = response_dict
    streams: dict[int, str] = await extractor.get_stream_urls(response_dict)
...
async def track(track_id: str, itag: int = 140):
    if track_id in track_responses:
        response_dict = track_responses[track_id]
    else:
        async with aiohttp.ClientSession() as session:
            async with session.post('https://www.youtube.com/watch',
                ...
                response_dict = (await response.json())[2]['playerResponse']
                track_responses[track_id] = response_dict
    if not (os.path.exists(f'{CACHE_DIRECTORY}/{track_id}.m4a') and itag == 140) or not (os.path.exists(f'{CACHE_DIRECTORY}/{track_id}.m4a') and itag == 251):
        description: list[str] = response_dict['videoDetails']['shortDescription'].split(
            '\n\n')
        track = {
            'track_id': track_id,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(streams[itag],
                ...
                if itag == 140:
                    ...
                    async with aiofiles.open(f'{CACHE_DIRECTORY}/{track_id}.webm', 'wb') as file:
                        await file.write(await response.read())
                        process = subprocess.Popen(
                            FFMPEG_COMMAND.format(
                                track=os.path.join(tempfile.gettempdir(
                                ), 'yt-music-headless') + '/' + track_id),
                            shell=True,
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.get(f'https://img.youtube.com/vi/{track_id}/maxresdefault.jpg') as response:
                                ...
                                cover_image.save(
                                    cover_bytes, format='JPEG')
                                audio_file: OggOpus = OggOpus(
                                    f'{CACHE_DIRECTORY}/{track_id}.ogg')
                                ...
                                cover.width = 720
                                cover.height = 720
                                encoded_data: bytes = base64.b64encode(
                                    cover.write())
                                ...
                                audio_file.save()
    return RedirectResponse(f'tracks/{track_id}.{"m4a" if itag == 140 else "ogg"}')
...
def main():
    ...
    app.mount(
        '/tracks', StaticFiles(directory=CACHE_DIRECTORY), name='tracks')
    uvicorn.run(app,
