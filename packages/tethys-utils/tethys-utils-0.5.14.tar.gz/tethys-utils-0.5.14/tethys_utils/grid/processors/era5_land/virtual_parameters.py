...
def calc_eto(wrf_xr):
    ...
    """
    ## Assign variables
    dew = wrf_xr['d2m'].squeeze() - 273.15
    t2 = wrf_xr['t2m'].squeeze() - 273.15
    pres = wrf_xr['sp'].squeeze()
    gamma = (0.665*10**-3)*pres/1000
    G = fix_accum(wrf_xr['slhf'].squeeze()) * 0.0036
    R_n = (fix_accum(wrf_xr['ssr'].squeeze()) + fix_accum(wrf_xr['str'].squeeze())) * 0.0036
    u10 = wrf_xr['u10'].squeeze()
    v10 = wrf_xr['v10'].squeeze()
    ...
    # Calc ETo
    ETo = ((0.408*delta*(R_n - G) + gamma*37/(t2 + 273)*ws2*(e_mean - e_a))/(delta + gamma*(1 + 0.34*ws2))) * -1

    ETo.name = 'reference_et'

    ETo = ETo.assign_coords({'height': 0})
    ETo = ETo.expand_dims('height')
