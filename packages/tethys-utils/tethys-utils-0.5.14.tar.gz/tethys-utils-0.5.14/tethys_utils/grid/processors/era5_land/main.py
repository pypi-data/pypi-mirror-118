...
import pandas as pd
import numpy as np
from tethys_utils.grid.processors.era5_land import virtual_parameters as vp
...
param_func_mappings = {'temp_at_2': ['t2m'],
                       ...
                       'reference_et_at_0': ['t2m', 'd2m', 'u10', 'v10', 'ssr', 'str', 'slhf'],
                       'pet_at_0': ['pev'],
                       'evaporation_at_0': ['e']
                       }
...
def postprocessor(ds):
    """

    """
    ## Read variables
    vars1 = list(ds.variables)
    vars1 = [v for v in vars1 if v not in dims]

    ## Determine which parameters can be converted
    vars2 = [p for p, l in param_func_mappings.items() if set(l).issubset(vars1)]
    ...
    map1 = mappings.loc[vars2]

    for p, m in map1.iterrows():
        # pass
