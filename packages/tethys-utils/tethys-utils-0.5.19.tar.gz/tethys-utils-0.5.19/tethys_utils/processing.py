...
def assign_station_ids_df(stns_df, extent=False, precision=5):
    ...
    """
    geometry = create_geometry_df(stns_df, extent, False, precision, to_wkb_hex=False)
...
def get_station_data_from_xr(data):
    ...
    stn_data1['from_date'] = from_date
    stn_data1['to_date'] = to_date
...
def stats_for_dataset_metadata(stns):
    ...
    ## time range
    trange1 = np.array([[s['stats']['from_date'], s['stats']['to_date']] for s in stns])
    ...
    dict1 = {'spatial_resolution': res1, 'extent': extent1, 'time_range': {'from_date': min_t, 'to_date': max_t}}
