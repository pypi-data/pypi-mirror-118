...
class Titan(object):
    ...
    def update_aggregates(self, threads=60):
        ...
        for ds in self.dataset_list:
            ds_new = tu.s3.put_remote_dataset(s3, self.bucket, ds)
            ds_stations = tu.s3.put_remote_agg_stations(s3, self.bucket, ds['dataset_id'], threads)

        # Aggregate all datasets for the bucket
        ds_all = tu.s3.put_remote_agg_datasets(s3, self.bucket, threads)
