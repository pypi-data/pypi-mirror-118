...
def put_remote_dataset(s3, bucket, dataset, stns, run_date=None):
    """
...
def put_remote_agg_stations(s3, bucket, dataset_id, threads=30):
    ...
    [s.update({'last_results_object_key': s['results_object_key'][-1]}) for s in output]
    [s.pop('results_object_key') for s in output]
    ...
    # Results object keys
    ro_key = key_patterns['results_object_keys'].format(dataset_id=dataset_id)
