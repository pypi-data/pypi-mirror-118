...
ds_list = self.load_grid_metadata(datasets)

grid1 = self.load_raw_grid(data_path2, lon_dim_name, lat_dim_name, time_dim_name, preprocessor=preprocessor)
...
ds = open_wrf_dataset(data_path1)
...
self.load_run_date(processing_code, run_date=None)
...
self.load_results(results, xy_size, workers, dataset_id, sum_closed, other_closed, discrete, other_attrs, other_encoding, run_date)

self.load_grids(results, xy_size, workers, dataset_id, sum_closed, other_closed, discrete, other_attrs, other_encoding, run_date)
