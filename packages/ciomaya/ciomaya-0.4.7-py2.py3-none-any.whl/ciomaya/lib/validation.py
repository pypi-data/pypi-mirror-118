...
class ValidateInstanceType(Validator):
    def run(self, layername):
        ...
        instance_type = next(
            (it for it in coredata.data()["instance_types"] if it["name"] == instance_type_name), None
        )
...
class ValidateGPU(Validator):
    ...
    def validate_arnold(self, instance_type):
        ...
        if (arnold_gpu and instance_type["gpu"]) or ((not arnold_gpu) and (not instance_type["gpu"])):
            return
...
class ValidateScoutFrames(Validator):
    ...
    def run(self, _):
        ...
        if chunk_size > 1:
            ...
            msg += " This can cause more scout frames to be rendered than you might expect. ({} scout frames).".format(scout_count)
            self.add_warning(msg)
...
class ValidateDestinationDirectory(Validator):
    def run(self, _):
        ...
        if not current_images_path == dest_directory:
            self.add_warning(
                    "The selected destination directory for output '{}' does not match the images file rule '{}'. Are you sure this is what you want?".format(dest_directory,current_images_path)
                )
        for gpath in asset_cache.data(self._submitter):
            asset_path = gpath.fslash(with_drive=False)
            if asset_path.startswith(dest_directory):
                pm.displayInfo("Some of your upload assets exist in the specified output destination directory\n. {} contains {}".format(
            ...
            if dest_directory.startswith(asset_path):
                pm.displayInfo("You are trying to upload a directory that contains your destination directory.\n. {} contains {}".format(
