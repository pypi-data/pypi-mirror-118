...
class bingbot:
    ...
    def reportDailySet(ReportingP: str, DAILYPOINTSREPORTING: str, DAILYTOTALPOINTS: int):
        try:
            ...
            with open(os.path.join(ReportingP, filename), 'a+') as d:
                DAILYPOINTSREPORTING += ["completed Daily Tasks Successfully with Total "+str(
    ...
    def reportPromotionsSet(ReportingP: str, PROMOTIONREPOTING: str, PROMOTIONPOINTS: int):
        try:
            ...
            with open(os.path.join(ReportingP, filename), 'a+') as d:
                PROMOTIONREPOTING += ["completed Promotions Tasks Successfully with Total "+str(
    ...
    def reportSearchSet(ReportingP: str, SEARCHREPORTING: str, SEARCHTOTALPOINTS: int):
        try:
            ...
            with open(os.path.join(ReportingP, filename), 'a+') as d:
                SEARCHREPORTING += ["completed Search Tasks Successfully with Total " +
                                    str(SEARCHTOTALPOINTS)+" Points \n"]
    ...
    def inputCredentials():
        ...
        if numberAccounts > 5:
            ...
            bingbot.inputCredentials()
        else:
            mydict = bingbot.email_password(numberAccounts)
    ...
    def browserSetup(Chrome_path: str, headless_mode: bool = False, user_agent: str = PC_USER_AGENT) -> WebDriver:
        ...
        options.add_argument('lang=' + bingbot. LANG.split("-")[0])
        if headless_mode:
    ...
    def login(browser: WebDriver, email: str, pwd: str, POINTS_COUNTER: int, isMobile: bool = False):
        # Access to bing.com

        try:
            browser.get('https://login.live.com/')
            # Wait complete loading
            bingbot.waitUntilVisible(browser, By.ID, 'loginHeader', 10)
            # Enter email
            ...
            # Wait complete loading
            try:
                bingbot.waitUntilVisible(browser, By.ID, 'KmsiCheckboxField', 10)
            ...
            POINTS_COUNTER = bingbot.checkBingLogin(browser, POINTS_COUNTER, isMobile)
            return POINTS_COUNTER
        except:
            POINTS_COUNTER = bingbot.login(browser, email, pwd)
            return POINTS_COUNTER


    def checkBingLogin(browser: WebDriver, POINTS_COUNTER: int, isMobile: bool = False):
        ...
        try:
            ...
            POINTS_COUNTER = bingbot.checkBingLogin(browser, isMobile)
            return POINTS_COUNTER
    ...
    def getGoogleTrends(numberOfwords: int) -> list:
        ...
        while len(search_terms) < numberOfwords:
            i += 1
            r = requests.get('https://trends.google.com/trends/api/dailytrends?hl=' + bingbot.LANG + '&ed=' +
                            str((date.today() - timedelta(days=i)).strftime('%Y%m%d')) + '&geo=' +bingbot. GEO + '&ns=15')
    ...
    def getRelatedTerms(word: str) -> int:
        r = requests.get('https://api.bing.com/osjson.aspx?query=' +
                        word, headers={'User-agent':bingbot. PC_USER_AGENT})
        return r.json()[1]
    ...
    def bingSearches(browser: WebDriver, numberOfSearches: int, POINTS_COUNTER: int, SEARCHREPORTING: str, SEARCHTOTALPOINTS: int, isMobile: bool = False):

        i = 0
        search_terms = bingbot.getGoogleTrends(numberOfSearches)
        for word in search_terms:
            i += 1
            print('[BING]', str(i) + "/" + str(numberOfSearches))
            points = bingbot.bingSearch(browser, word, isMobile)
            if points <= POINTS_COUNTER:
                relatedTerms =bingbot. getRelatedTerms(word)
                for term in relatedTerms:
                    points =bingbot. bingSearch(browser, term, isMobile)
        ...
        if isMobile:
            SEARCHREPORTING += ["done mobile "+str(
                numberOfSearches) + " search for "+str(SEARCHTOTALPOINTS)+" points \n"]
        else:
            SEARCHREPORTING += ["done desktop "+str(
                numberOfSearches) + " search for "+str(SEARCHTOTALPOINTS)+" points \n"]
    ...
    def completeDailySetSearch(browser: WebDriver, cardNumber: int, donepoints: int, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        ...
        try:
            ...
            DAILYPOINTSREPORTING += ["done daily search for " +
                                    str(donepoints)+"  \n"]
        except Exception as e:
            try:
                browser.find_element_by_xpath('//*[@id="daily-sets"]/mee-card-group[1]/div/mee-card[' + str(
    ...
    def completeDailySetSurvey(browser: WebDriver, cardNumber: int, donepoints: int, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        ...
        try:
            browser.find_element_by_id(
                "btoption" + str(random.randint(0, 1))).click()

            DAILYTOTALPOINTS += donepoints
            DAILYPOINTSREPORTING += ["done completeDailySetSurvey for " +
                                    str(donepoints)+"  \n"]
            return DAILYTOTALPOINTS, DAILYPOINTSREPORTING
        except:
            pass
    ...
    def completeDailySetQuiz(browser: WebDriver, cardNumber: int, donepoints: int, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        ...
        try:
            ...
            browser.switch_to.window(window_name=browser.window_handles[1])
            time.sleep(8)
            bingbot.waitUntilpanelshow(browser)
        ...
        if thereBtn:
            ...
            time.sleep(3)
        try:
            bingbot.waitUntilVisible(browser, By.XPATH,
            ...
            DAILYTOTALPOINTS += donepoints
            DAILYPOINTSREPORTING += ["done completeDailySetQuiz for " +
                                    str(donepoints)+"  \n"]
            return DAILYTOTALPOINTS, DAILYPOINTSREPORTING
        except Exception as e:
    ...
    def completeDailySetVariableActivity(browser: WebDriver, cardNumber: int, donepoints: int, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        ...
        try:
            ...
            try:
                ...
                DAILYTOTALPOINTS += donepoints
                DAILYPOINTSREPORTING += [
                    "done completeDailySetVariableActivityfor "+str(donepoints)+"  \n"]
                time.sleep(5)
                browser.close()
                time.sleep(2)
                browser.switch_to.window(window_name=browser.window_handles[0])
    ...
    def completeDailySetThisOrThat(browser: WebDriver, cardNumber: int, totalpoints: int, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        ...
        thereBtn = False
        loaded = False
        bingbot.waitUntilStartbuttonappear(browser)
        while(loaded == False):
            try:
                browser.find_element_by_xpath('//*[@id="rqStartQuiz"]')
                loaded = True
        ...
        try:
            ...
            for question in range(10):
                try:
                    ...
                    answer1Code = bingbot.getAnswerCode(answerEncodeKey, answer1Title)
                    answer2 = browser.find_element_by_id("rqAnswerOption1")
                    answer2Title = answer2.get_attribute('data-option')
                    answer2Code = bingbot.getAnswerCode(answerEncodeKey, answer2Title)
            ...
            DAILYPOINTSREPORTING += ["done completeDailySetThisOrThat for " +
                                    str(totalpoints)+"  \n"]
            return DAILYTOTALPOINTS, DAILYPOINTSREPORTING
        except:
            bingbot.resetTabs(browser)
    ...
    def getDashboardData(browser: WebDriver) -> dict:
        dashboard = bingbot.findBetween(browser.find_element_by_xpath('/html/body').get_attribute('innerHTML'),
                                "var dashboard = ", ";\n        appDataModule.constant(\"prefetchedDashboard\", dashboard);")
        dashboard = json.loads(dashboard)
        return dashboard


    def completeDailySet(browser: WebDriver, DAILYTOTALPOINTS: int, DAILYPOINTSREPORTING: str):
        time.sleep(10)
        ...
        d = bingbot.getDashboardData(browser)['dailySetPromotions']
            ...
            try:
                if activity['complete'] == False:
                    ...
                    if activity['promotionType'] == "urlreward":
                        ...
                        DAILYTOTALPOINTS, DAILYPOINTSREPORTING = bingbot.completeDailySetSearch(
                    ...
                    if activity['promotionType'] == "quiz":
                        if activity['pointProgressMax'] == 50:
                            ...
                            DAILYTOTALPOINTS, DAILYPOINTSREPORTING = bingbot.completeDailySetThisOrThat(
                                browser, cardNumber, activity['pointProgressMax'], DAILYTOTALPOINTS, DAILYPOINTSREPORTING)
                        elif activity['pointProgressMax'] == 40 or activity['pointProgressMax'] == 30:
                            print('[DAILY SET]',
                                'Completing quiz of card ' + str(cardNumber))
                            DAILYTOTALPOINTS, DAILYPOINTSREPORTING =bingbot. completeDailySetQuiz(
                                browser, cardNumber, activity['pointProgressMax'], DAILYTOTALPOINTS, DAILYPOINTSREPORTING)
                            ...
                            if "PollScenarioId" in filters:
                                ...
                                DAILYTOTALPOINTS, DAILYPOINTSREPORTING = bingbot.completeDailySetSurvey(
                                    browser, cardNumber, activity['pointProgressMax'], DAILYTOTALPOINTS, DAILYPOINTSREPORTING)
                            else:
                                print(
                                    '[DAILY SET]', 'Completing quiz of card ' + str(cardNumber))
                                DAILYTOTALPOINTS, DAILYPOINTSREPORTING = bingbot.completeDailySetVariableActivity(
                                    browser, cardNumber, activity['pointProgressMax'], DAILYTOTALPOINTS, DAILYPOINTSREPORTING)
    ...
    def getAccountPoints(browser: WebDriver) -> int:
        return bingbot.getDashboardData(browser)['userStatus']['availablePoints']
    ...
    def completePunchCards(browser: WebDriver):
        punchCards = bingbot.getDashboardData(browser)['punchCards']
        for punchCard in punchCards:
            try:
                if punchCard['parentPromotion'] != None and punchCard['childPromotions'] != None and punchCard['parentPromotion']['complete'] == False and punchCard['parentPromotion']['pointProgressMax'] != 0:
                    ...
                    bingbot.completePunchCard(browser, dest, punchCard['childPromotions'])
            except:
                bingbot.resetTabs(browser)
        time.sleep(2)
        browser.get('https://account.microsoft.com/rewards/')
    ...
    def completeMorePromotionSearch(browser: WebDriver, cardNumber: int, donepoints: int, PROMOTIONPOINTS: int, PROMOTIONREPOTING: str):
        ...
        PROMOTIONREPOTING += ["done completeMorePromotionSearch for " +
                            str(donepoints)+"  \n"]
        time.sleep(1)
        browser.switch_to.window(window_name=browser.window_handles[1])
        time.sleep(random.randint(13, 17))
    ...
    def completeMorePromotionQuiz(browser: WebDriver, cardNumber: int, donepoints: int, PROMOTIONPOINTS: int, PROMOTIONREPOTING: str):
        ...
        try:
            ...
            PROMOTIONPOINTS += donepoints
            PROMOTIONREPOTING += ["done completeMorePromotionQuiz for " +
                                str(donepoints)+"  \n"]
    ...
    def completeMorePromotionABC(browser: WebDriver, cardNumber: int, donepoints: int, PROMOTIONPOINTS: int, PROMOTIONREPOTING: str):
        try:
            ...
            time.sleep(5)
            PROMOTIONPOINTS += donepoints
            PROMOTIONREPOTING += ["done completeMorePromotionABC for " +
                                str(donepoints)+"  \n"]
            browser.close()
            time.sleep(2)
            browser.switch_to.window(window_name=browser.window_handles[0])
    ...
    def completeMorePromotionThisOrThat(browser: WebDriver, cardNumber: int, donepoints: int, PROMOTIONPOINTS: int, PROMOTIONREPOTING: str):
        ...
        try:
            ...
            PROMOTIONPOINTS += donepoints
            PROMOTIONREPOTING += ["done completeMorePromotionThisOrThat for " +
                                str(donepoints)+"  \n"]
    ...
    def checkifmorepromotionexist(browser: WebDriver):
        bingbot.resetTabs(browser)
        morePromotions = bingbot.getDashboardData(browser)['morePromotions']
        isfinish = True
        i = 0
    ...
    def completeMorePromotions(browser: WebDriver, PROMOTIONPOINTS: int, PROMOTIONREPOTING: str):
        ...
        for promotion in morePromotions:
            try:
                ...
                if promotion['complete'] == False and promotion['pointProgressMax'] != 0:
                    if promotion['promotionType'] == "urlreward":
                        PROMOTIONREPOTING, PROMOTIONPOINTS = bingbot.completeMorePromotionSearch(
                            browser, i, promotion['pointProgressMax'], PROMOTIONREPOTING, PROMOTIONPOINTS)
                    elif promotion['promotionType'] == "quiz":
                        if promotion['pointProgressMax'] == 10:
                            PROMOTIONREPOTING, PROMOTIONPOINTS =bingbot. completeMorePromotionABC(
                                browser, i, promotion['pointProgressMax'], PROMOTIONREPOTING, PROMOTIONPOINTS)
                        elif promotion['pointProgressMax'] == 30 or promotion['pointProgressMax'] == 40:
                            PROMOTIONREPOTING, PROMOTIONPOINTS =bingbot. completeMorePromotionQuiz(
                                browser, i, promotion['pointProgressMax'], PROMOTIONREPOTING, PROMOTIONPOINTS)
                        elif promotion['pointProgressMax'] == 50:
                            PROMOTIONREPOTING, PROMOTIONPOINTS = bingbot.completeMorePromotionThisOrThat(
                                browser, i, promotion['pointProgressMax'], PROMOTIONREPOTING, PROMOTIONPOINTS)
                    else:
                        if promotion['pointProgressMax'] == 100 or promotion['pointProgressMax'] == 200:
                            PROMOTIONREPOTING, PROMOTIONPOINTS = bingbot.completeMorePromotionSearch(
    ...
    def getRemainingSearches(browser: WebDriver):
        dashboard = bingbot.getDashboardData(browser)
        searchPoints = 1
