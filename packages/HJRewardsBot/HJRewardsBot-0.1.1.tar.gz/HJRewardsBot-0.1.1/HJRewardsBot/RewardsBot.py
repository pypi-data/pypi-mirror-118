...
def browserSetup(Chrome_path:str,headless_mode: bool = False, user_agent: str = PC_USER_AGENT) -> WebDriver:
    ...
    chrome_browser_obj = webdriver.Chrome(executable_path=Chrome_path, options=options)
...
def login(browser: WebDriver, email: str, pwd: str, POINTS_COUNTER:int,isMobile: bool = False):
  ...
  try:  
    ...
    POINTS_COUNTER= checkBingLogin(browser,POINTS_COUNTER, isMobile)
    return POINTS_COUNTER
  except:
     POINTS_COUNTER= login(browser, email, pwd)
     return POINTS_COUNTER
...
def checkBingLogin(browser: WebDriver, POINTS_COUNTER:int,isMobile: bool = False):
    ...
    try:
       ...
       POINTS_COUNTER= checkBingLogin(browser, isMobile)
       return POINTS_COUNTER    
   ...
   try:
        ...
        for promotion in morePromotions:
            try:
                ...
                if promotion['complete'] == False and promotion['pointProgressMax'] != 0:
                    if promotion['promotionType'] == "urlreward":
                        PROMOTIONREPOTING,PROMOTIONPOINTS=completeMorePromotionSearch(browser, i,promotion['pointProgressMax'],PROMOTIONREPOTING,PROMOTIONPOINTS)
                    elif promotion['promotionType'] == "quiz":
                        if promotion['pointProgressMax'] == 10:
                             PROMOTIONREPOTING,PROMOTIONPOINTS= completeMorePromotionABC(browser, i,promotion['pointProgressMax'],PROMOTIONREPOTING,PROMOTIONPOINTS)
                        elif promotion['pointProgressMax'] == 30 or promotion['pointProgressMax'] == 40:
                            PROMOTIONREPOTING,PROMOTIONPOINTS=completeMorePromotionQuiz(browser, i,promotion['pointProgressMax'],PROMOTIONREPOTING,PROMOTIONPOINTS)
                        elif promotion['pointProgressMax'] == 50:
                             PROMOTIONREPOTING,PROMOTIONPOINTS= completeMorePromotionThisOrThat(browser, i,promotion['pointProgressMax'],PROMOTIONREPOTING,PROMOTIONPOINTS)
                    else:
                        if promotion['pointProgressMax'] == 100 or promotion['pointProgressMax'] == 200:
                            PROMOTIONREPOTING,PROMOTIONPOINTS=completeMorePromotionSearch(browser, i,promotion['pointProgressMax'],PROMOTIONREPOTING,PROMOTIONPOINTS)
            except Exception as e:
                print(e)
                resetTabs(browser)
                completeMorePromotions(browser,PROMOTIONPOINTS,PROMOTIONREPOTING)
                #resetTabs(browser)
        return PROMOTIONREPOTING,PROMOTIONPOINTS
   except Exception as e:
        print(e)
