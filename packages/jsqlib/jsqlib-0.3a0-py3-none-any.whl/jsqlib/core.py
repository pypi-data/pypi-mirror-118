...
from jsqlib.helpers.common import dquote, stringify, densify
from jsqlib.helpers.types import BL_T, SIBN_T, SIBND_T, SIBNLD_T


class Builder(metaclass=ABCMeta):
    """Base JSON to SQL expressions builder."""

    def __init__(self, constants: Optional[Dict[str, SIBN_T]] = None, **kwargs) -> None:
        self._constants = constants or dict()

    def build(self, query: Dict[str, Any], dense: bool = True) -> str:
        chunks = list()
        query = query or dict()
        ...
        return densify(joined) if dense else joined

    def _quote_(self, value):
        return dquote(value) if value else value

    def _stringify_(self, value: SIBN_T) -> str:
        return stringify(value, constants=self._constants)

    def _eval_(self, value: SIBND_T) -> str:
    ...
    def _apply_(self, name: str, args: List[SIBND_T]) -> str:
        merged = ','.join((self._eval_(arg) for arg in args))

        return f'{name}({merged})'

    def _alias_(self, alias: SIBND_T) -> str:
        return f'as {self._quote_(alias)}' if alias else ''

    def _enclose_(self, value: SIBNLD_T) -> str:
    ...
    def _commute_(self, value: List[SIBND_T], op: str) -> str:
        return f' {op} '.join((self._eval_(v) for v in value))

    def _combine_(self, value: Dict[str, SIBNLD_T], type_: str) -> str:
        return f'{type_} {self.build(value, dense=False)}'

    def _calc_(self, value: List[SIBND_T], op: str) -> str:
    ...
    def _enclose(self, value: SIBNLD_T) -> str:
        return self._enclose_(value)

    def _value(self, value: SIBN_T) -> str:
        return self._stringify_(value)

    def _optional(self, value: Dict[str, SIBND_T]) -> str:
        optional = list()
    ...
    def _with(self, value: Dict[str, Any], recursive: bool = False) -> str:
        ...
        for k, v in value.items():
            if columns := v.pop('columns', ''):
                columns = self._columns_(columns)  # pyre-ignore[6]

            chunks.append(f"{self._quote_(k)}{columns} as ({self.build(v, dense=False)})")

        return f"{with_} {', '.join(chunks)}"

    def _with_recursive(self, value: Dict[str, SIBND_T]) -> str:
        return self._with(value, recursive=True)

    def _union(self, value: Dict[str, SIBNLD_T], type_: str = '') -> str:
        union = f'union {type_}' if type_ else 'union'
        return self._combine_(value, union)

    def _union_all(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._union(value, type_='all')

    def _union_distinct(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._union(value, type_='distinct')

    def _intersect(self, value: Dict[str, SIBNLD_T], type_: str = '') -> str:
        intersect = f'intersect {type_}' if type_ else 'intersect'
        return self._combine_(value, intersect)

    def _intersect_all(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._intersect(value, type_='all')

    def _intersect_distinct(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._intersect(value, type_='distinct')

    def _except(self, value: Dict[str, SIBNLD_T], type_: str = '') -> str:
        except_ = f'except {type_}' if type_ else 'except'
        return self._combine_(value, except_)

    def _except_all(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._except(value, type_='all')

    def _except_distinct(self, value: Dict[str, SIBNLD_T]) -> str:
    ...
    def _join(self, value: Dict[str, SIBND_T], type_: str = '') -> str:
        return self._from_(value, type_=f'{type_} join')

    def _inner_join(self, value: Dict[str, SIBND_T]) -> str:
        return self._join(value, type_='inner')

    def _left_join(self, value: Dict[str, SIBND_T]) -> str:
        return self._join(value, type_='left')

    def _right_join(self, value: Dict[str, SIBND_T]) -> str:
        return self._join(value, type_='right')

    def _full_join(self, value: Dict[str, SIBND_T]) -> str:
        return self._join(value, type_='full')

    def _cross_join(self, value: Dict[str, SIBND_T]) -> str:
        return self._join(value, type_='cross')

    def _where(self, value: Dict[str, BL_T]) -> str:
        return f'where {self._eval_(value)}'

    def _group_by(self, value: List[SIBND_T]) -> str:
        return f"group by {','.join(self._eval_(v) for v in value)}"

    def _having(self, value: Dict[str, SIBNLD_T]) -> str:
        return f'having {self._eval_(value)}'

    def _order_by(self, value: List[SIBND_T]) -> str:
        columns = list()

        for v in value:
            if isinstance(v, dict):
                if v.pop('desc', False):
                    columns.append(f'{self._eval_(v)} desc')
                if 'using' in v:
                    using = self._eval_(v.pop('using'))
                    columns.append(f'{self._eval_(v)} using {using}')
            else:
                columns.append(self._eval_(v))

        return f"order by {','.join(columns)}" if columns else ''

    def _limit(self, value: Dict[str, SIBND_T]) -> str:
        return f'limit {self._eval_(value)}'

    def _offset(self, value: Dict[str, SIBND_T]) -> str:
    ...
    def _eq(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='=')

    def _neq(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='!=')

    def _lt(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='<')

    def _gt(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='>')

    def _lte(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='<=')

    def _gte(self, value: List[SIBND_T]) -> str:
        return self._commute_(value, op='>=')

    def _is(self, value: List[SIBND_T], negate: bool = False) -> str:
        condition = 'is not' if negate else 'is'
        return f'{self._eval_(value[0])} {condition} {self._eval_(value[-1])}'

    def _is_not(self, value: List[SIBND_T]) -> str:
        return self._is(value, negate=True)

    def _add(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='+')

    def _sub(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='-')

    def _mult(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='*')

    def _div(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='/')

    def _pow(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='^')

    def _mod(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='%')

    def _and(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='and')

    def _or(self, value: List[SIBND_T]) -> str:
        return self._calc_(value, op='or')

    def _in(self, value: List[SIBND_T], negate: bool = False) -> str:
        op = 'not in' if negate else 'in'
        check = ','.join((self._eval_(v) for v in value[1:]))
        return f"{self._eval_(value[0])} {op} ({check})"

    def _not_in(self, value: List[SIBND_T]) -> str:
        return self._in(value, negate=True)

    def _cast(self, value: List[SIBND_T]) -> str:
        return f'cast ({self._eval_(value[0])} as {self._eval_(value[-1])})'

    def _like(self, value: List[SIBND_T], negate: bool = False) -> str:
        condition = 'not like' if negate else 'like'
        return f'{self._eval_(value[0])} {condition} {self._eval_(value[-1])}'

    def _not_like(self, value: List[SIBND_T]) -> str:
        return self._like(value, negate=True)

    def _exists(self, value: Dict[str, SIBNLD_T], negate: bool = False) -> str:
        condition = 'not exists' if negate else 'exists'
        return f'{condition} ({self.build(value, dense=False)})'

    def _not_exists(self, value: Dict[str, SIBNLD_T]) -> str:
        return self._exists(value, negate=True)

    def _between(self, value: List[SIBND_T], negate: bool = False) -> str:
    ...
    def _not_between(self, value: List[SIBND_T]) -> str:
            ...
            for entity in using:
                if isinstance(entity, dict):
                    ...
                    if values := entity.pop('values', None):
                        values = f'(values {self._values_(values)})'  # pyre-ignore[6]
    ...
    def _returning(self, value: List[SIBND_T]) -> str:
        returning = list()
...
DIALECTS = {POSTGRESQL_DIALECT: PGBuilder}
