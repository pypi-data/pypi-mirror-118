...
from typing import Match, Optional, Pattern, Tuple, Type
...
def _get_final_wheel_tag(
    ...
    for tag in WheelTag.sys_tags():
        if tag == current_tag or tag == compiled_wheel_tag:
            return tag
...
def compile_wheel(wheel_filepath: str,
    ...
    with TemporaryDirectory(dir=wheel_dirpath) as temp_dirpath:
        ...
        compiler.compile(exclude_pattern=exclude_pattern,
                         remove_src=remove_src)

        # Read WHEEL dist-info file content.
        wheel_metadata_filepath = join(dist_info_dirpath, 'WHEEL')

        with open(wheel_metadata_filepath, 'r') as f:
            wheel_metadata: Message = message_from_file(f)
        ...
        with open(wheel_metadata_filepath, 'w') as f:
            Generator(f, maxheaderlen=0).flatten(wheel_metadata)
