...
class ExposureSpecification:
    ...
    def __init__(self, pattern, ngroup, nint, nexp, tframe, nframe=1, nsample=1, nsample_skip=0, tfffr=0,
                 subarray='Full', ndrop2=0, nprerej=0, npostrej=0, nreset1=1, ndrop1=0, ndrop3=0, nreset2=1,
    ...
    def get_times_nir(self):
        ...
        A small alteration has been made: the nskip parameter is now ndrop2, to follow 
        the pending format from Holler et al. 2021, JWST-STScI-006013-A
        ...
        # Equation 4
        self.measurement_time = self.tframe * self.nint * ((self.ngroup - 1) * (self.nframe + self.ndrop2))
        # Equation 5
        if self.frame0:
        ...
        self.saturation_time = self.tfffr + self.tframe * (self.ndrop1 +
                               (self.ngroup - 1) * (self.nframe + self.ndrop2) + self.nframe)
    ...
    def get_times_mir(self):
        ...
        Note that these equations are generic, suitable for both H2RG and SiAs detectors.
    ...
    def get_groups_before_sat(self, slope, fullwell):
        ...
        if self.det_type == 'sias':
            groups_before_sat = ((time_to_saturation / self.tframe) - self.nframe - self.ndrop1) / \
                                (self.nframe + self.ndrop2) + 1 
    ...
    def slope_variance(self, rate, dark_current, readnoise, unsat_ngroups,
        ...
        slope_dc_var: ndarray
            The associated variance of the dark current only
        """
        ...
        if ((self.nprerej != 0) or (self.npostrej != 0)):
        ...
        slope_var = (6. / 5.) * (n ** 2. + 1.) / (n * (n ** 2. - 1.)) * \
            ...
            slope_rn_var
        # just the variance due to the dark current (remove poisson and readnoise)
        slope_dc_var = (6. / 5.) * (n ** 2. + 1.) / (n * (n ** 2. - 1.)) * \
            ((np.zeros_like(variance_per_pix) + dark_current) / tgroup) * \
            (1. - (5. / 3.) * (m ** 2. - 1.) / (m * (n ** 2. + 1.)) * (tframe / tgroup))
        ...
        slope_dc_var = ma.filled(slope_dc_var, fill_value=np.nan)

        return slope_var, slope_rn_var, slope_dc_var
...
class ExposureSpecificationTA(ExposureSpecification):
    ...
    def __init__(self, pattern, ngroup, ngroup_extract, nint, nexp, tframe, nframe=1, nsample=1, tfffr=0,
                 nsample_skip=1, subarray='Full', ndrop2=0, nprerej=0, npostrej=0, nreset1=1, ndrop1=0, ndrop3=0,
        ...
        ndrop2: int (optional, default 0)
            number of dropped frames per group. Only
            supported by some readout patterns.
        ...
        ExposureSpecification.__init__(self, pattern, ngroup, nint, nexp, tframe, nframe=nframe,
                 subarray=subarray, ndrop2=ndrop2, nsample_skip=nsample_skip, tfffr=tfffr,
    ...
    def slope_variance(self, rate, dark_current, readnoise, unsat_ngroups,
        ...
        # And for dark current alone:
        slope_dc_var = min_factor * (np.zeros_like(variance_per_pix)+dark_current)/textract * (1.-(1./3.)*(m**2-1)/m *
                                                                             tframe/textract)
