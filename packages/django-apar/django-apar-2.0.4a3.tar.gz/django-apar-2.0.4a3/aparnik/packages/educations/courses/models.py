...
class CourseSummary(models.Model):
    ...
    course = models.ForeignKey('BaseCourse', related_name='total_times', on_delete=models.CASCADE,
...
class Course(BaseCourse):
    ...
    cover = models.ForeignKey(FileField, related_name='course_cover', on_delete=models.CASCADE,
                              verbose_name=_('Cover Image'))
    banner = models.ForeignKey(FileField, related_name='course_banner', on_delete=models.CASCADE,
                               verbose_name=_('Banner Image'))
    # TODO: set category marboot be course
    category_obj = models.ForeignKey(Category, null=True, blank=True, on_delete=models.CASCADE,
                                     verbose_name=_('Category'))
    parent_obj = models.ForeignKey('self', null=True, blank=True, on_delete=models.CASCADE, verbose_name=_('Parent'))
    dependency_obj = models.ForeignKey('self', related_name='dependency', null=True, blank=True,
                                       on_delete=models.CASCADE, verbose_name=_('Dependency'))
    publish_date = models.DateTimeField(default=now, verbose_name=_('Publish Date'))
    publish_month = models.CharField(max_length=2, null=True, blank=True, choices=MONTH_CHOICES,
    ...
    teacher_obj = models.ManyToManyField(
        Teacher,
        blank=True,
        through='courses.TeacherCourse',
    ...
    def get_user_for_base_review_notification(self, instance, created):
        ...
        if created:
            return users | User.objects.active().filter(
                pk__in=self.teachers.active().values_list('user_obj', flat=True))
        return users
...
class CourseFile(File):
    ...
    def get_user_for_base_review_notification(self, instance, created):
        ...
        if created:
            return users | User.objects.active().filter(
                pk__in=self.course_obj.teachers.active().values_list('user_obj', flat=True))
        return users
...
def post_save_file_receiver(sender, instance, created, *args, **kwargs):
    if instance.sort == 0:
        ...
        for file in CourseFile.objects.exclude(id=instance.id).filter(course_obj=instance.course_obj,
                                                                      sort=instance.sort):
...
class TeacherCourse(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        super(TeacherCourse, self).save(*args, **kwargs)

    class Meta:
        ordering = ['priority', ]
        unique_together = [['course_obj', 'teacher_obj', ]]
        verbose_name = _('Teacher course')
        verbose_name_plural = _('Teacher course relations')

    def update_priority(self):
        if TeacherCourse.objects.exclude(pk=self.pk).filter(course_obj=self.course_obj, priority=self.priority).count() > 0:
            TeacherCourse.objects.exclude(pk=self.pk).filter(course_obj=self.course_obj, priority__gte=self.priority).update(
