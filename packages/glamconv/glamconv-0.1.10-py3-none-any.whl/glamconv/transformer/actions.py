...
class AbstractAction(object):
    ...
    def dump(cls):
        return {
            ...
            u"desc": cls.desc,
            u"applicableFor": sorted([frmt.uid for frmt in cls.applicable_for]),
            u"paramsDesc": sorted([pdef.dump() for pdef in cls.params_def]),
        }
...
class ReadAction(AbstractAction):
    ...
    category = u"Reading"
    desc = (
        u"This action reads an opened file and returns its content "
        "as a bytes string."
    )
    params_def = tuple()  # (AbstractParameter, )
...
class WriteAction(AbstractAction):
    ...
    category = u"Writing"
    desc = (
        u"This action returns the data from a binary so that it can be "
        "written into a file."
    )
    params_def = tuple()  # (AbstractParameter, )
