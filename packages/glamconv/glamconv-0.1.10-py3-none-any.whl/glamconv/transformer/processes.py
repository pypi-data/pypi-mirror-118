...
class Process(object):
    ...
    def build_steps(self, steps_specif):
        ...
        for idx, specif in enumerate(steps_specif):
            ...
            self.steps.append(
                Step(action_class(), specif[u"log_details"], specif[u"params"])
            )
    ...
    def check(self):
        ...
        if self.output_format is not None:
            ...
            assert isinstance(
                self.steps[-2].action, self.output_format.validating_class
    ...
    def run(self, input_stream, output_stream=None, progress_callback=None):
        ...
        for idx, step in enumerate(self.steps):
            ...
            try:
                ...
                logger.error(
                    u"The following error has occured and has stopped " u"the run:",
                    format_exc(),
                )
                logger.failure = True
                ...
                if idx > 1 and isinstance(self.steps[-1].action, WriteAction):
                    ...
                    logger.info(
                        u"Trying to write the latest intermediate " u"result..."
                    )
                    try:
