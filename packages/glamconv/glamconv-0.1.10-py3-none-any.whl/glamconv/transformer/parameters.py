...
class SingleParameter(AbstractParameter):
    ...
    def _convert(self, specif_val):
        try:
            ...
            raise ValueError(
                u"Can't convert to target type with {0} from: {1}"
...
class CouplesParameter(AbstractParameter):
    ...
    def _convert(self, specif_val):
        ...
        if not isinstance(specif_val, (tuple, list)):
            raise ValueError(
                u"Expects a list of couples and got: {0}" u"".format(specif_val)
            )
    ...
    def log_value(self, value):
        return (
            self.name,
            u"\n".join([u"{0}: {1}".format(elt_1, elt_2) for (elt_1, elt_2) in value]),
        )
