...
class Ead2002Validator(ValidateAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if result is False:
            error_list = [
                u"Line {0}: {1}".format(err.line, err.message)
            ...
            if err_num > 100:
                ...
                error_list.append(
                    u"... and {0:d} more errors! ..." u"".format((err_num - 100))
                )
            logger.error(
            ...
            logger.info(u"The XML data complies with the EAD-2002 standard " u"(DTD).")
        return result
...
class ApeEadValidator(ValidateAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if result is False:
            ...
            logger.info(
                u"The XML data complies with the Ape-EAD standard " u"(XML Schema)."
            )
        return result
