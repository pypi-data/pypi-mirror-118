...
class EmptyEltsEraser(TransformAction):
    ...
    desc = (
        ...
        u"Some elements accept a paragraph (<p>) with textual content. In "
        u"these elements, if they are empty but contain a textual content, a "
        u"child <p> is added to wrap the text. This action can be applied to "
        u"correct EAD trees that are not valid against the EAD-2002 format."
    )
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        xpath_req_1 = u" | ".join(
            ...
            + [
                u".//{0}[not(*) and normalize-space(text()) = '']".format(eltname)
        ...
        for elt in xml_root.xpath(xpath_req_2):
            ...
            child = etree.SubElement(elt, u"p")
            child.text = elt.text
            elt.text = None
        if count_1 > 0:
            logger.warning(
                u"{0:d} empty elements have been deleted." u"".format(count_1)
            )
            if log_details:
                logger.warning(
                    u"The following empty elements have been deleted:\n"
                    + u"\n".join(log_data_1),
                )
        if count_2 > 0:
            logger.warning(
                u"{0:d} textual contents in empty elements have "
                u"been wrapped in <p> elements."
                u"".format(count_2)
            )
            if log_details:
                logger.warning(
                    u"The texts in the following empty elements have been "
                    u"wrapped in <p> elements:\n" + u"\n".join(log_data_2),
                )
        return xml_root
