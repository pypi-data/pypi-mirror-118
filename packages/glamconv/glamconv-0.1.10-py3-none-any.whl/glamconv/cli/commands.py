...
def ead2_to_ape(input_filepath_or_tree, output_filepath, settings=None):
    ...
    if hasattr(input_filepath_or_tree, "findtext"):
        logger.debug(
            "processing %s", input_filepath_or_tree.findtext("eadheader/eadid")
        )
    else:
    ...
    try:
        ...
        if osp.getsize(temp_fname) > 0:
            ...
            log.warning(
                "transformation of %s didn't generate any output", transform_input
            )
    finally:
    ...
    for file_idx, (norm_fname, inp_dirname, inp_filename, out_dirname) in enumerate(
        ...
        if log.output_validation:
            ...
            abstract[u"ERROR"].append(
                u"{0} (in step #{1:d})".format(norm_fname, (log.failure_step_index + 1))
            )
