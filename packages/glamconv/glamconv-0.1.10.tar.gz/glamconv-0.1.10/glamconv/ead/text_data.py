...
from glamconv.ead.utils import (
    log_element,
    write_hierarchy,
    split_qname,
...
class AddressConverter(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        xpath_req = u" | ".join([u".//{0}/address".format(name) for name in parents])
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <address> elements have been converted in "
                u"<p> elements.".format(count)
            )
            if log_details:
                logger.warning(
                    u"The following elements have been converted:", u"\n".join(log_data)
                )
        return xml_root
...
class BlockquoteRemover(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            ...
            if log_details:
                logger.warning(
                    u"The following elements have been emptied and " u"deleted:",
                    u"\n".join(log_data),
                )
        return xml_root
...
class NoteRemover(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        xpath_req = u" | ".join([u".//{0}/note".format(name) for name in parents])
        ...
        if count > 0:
            ...
            if log_details:
                logger.warning(
                    u"The following elements have been emptied " u"and deleted:",
                    u"\n".join(log_data),
                )
        return xml_root
...
class ChronlistConverter(TransformAction):
    ...
    category = u"Text data"
    desc = (
        u"In Ape-EAD, the <chronlist> elements don't exist anymore. "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for list_elt in xml_root.xpath(u".//chronlist"):
            ...
            for chritm in list_elt.xpath(u"chronitem"):
                ...
                for idx, evt in enumerate(chritm.xpath(u"event|eventgrp/event")):
                    insert_text_at_element_end(item, u" : " if idx == 0 else u" ; ")
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <chronlist> elements have been converted "
...
class DefinitionListConverter(TransformAction):
    ...
    category = u"Text data"
    desc = (
        u"In Ape-EAD, <list> elements can not have <defitem> children. "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for list_elt in xml_root.xpath(u".//list[defitem]"):
            ...
            for defitm in list_elt.xpath(u"defitem"):
                ...
                if label is not None and len(label) == 0:
                    ...
                    label.text = None
                    label.append(emph)
                insert_text_at_element_end(label, (u" : " + (item.text or u"")))
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <list> elements containing <defitem> "
...
def _list_content_extractor(
    list_elt, extracted_elts, prefix=u"- ", indent=u"  ", level=0
):
...
class ListConverter(TransformAction):
    ...
    category = u"Text data"
    desc = (
        u"In Ape-EAD, <list> elements can not occur anymore in some "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if log_details:
            log_data = []
        text_only_parents = (u"event", u"extref", u"extrefloc", u"ref", u"refloc")
        parents = (
            u"controlaccess",
            u"descgrp",
            u"entry",
        ...
        xpath_req = u" | ".join([u".//{0}/list".format(name) for name in parents])
        for list_elt in xml_root.xpath(xpath_req):
            parent = list_elt.getparent()
            if split_qname(parent.tag)[1] in text_only_parents:
                texts = []
                extracted_elts = []
                _list_content_extractor(
                    list_elt, extracted_elts, prefix=u"", indent=u""
                )
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <list> elements have been converted into "
                u"<p> elements.".format(count)
            )
            if log_details:
                logger.warning(
...
class LegalstatusConverter(TransformAction):
    ...
    category = u"Text data"
    desc = (
        u"The <legalstatus> elements don't exist in Ape-EAD. This action "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <legalstatus> elements have been converted "
...
def _adjust_mixed_content_and_log(xml_elt):
    ...
    log_data.extend(list(itertools.chain.from_iterable(children_log_data.values())))
    return log_data
...
class MixedContentAdjuster(TransformAction):
    ...
    category = u"Text data"
    desc = (
        u"In Ape-EAD, the elements with a mixed content (child elements "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            ...
            if log_details:
                log_info = [
                    u"{0}\n   Removed elements: {1}" u"".format(elt, u", ".join(remov))
                    for elt, remov in log_data
                    if len(remov) > 0
                ]
                logger.warning(
                    u"The following mixed content elements have " u"been modified:",
                    u"\n".join(log_info),
                )
        return xml_root
