...
class EadNamespaceAdder(TransformAction):
    ...
    category = u"Naming"
    desc = (
        u"This action puts all the XML elements with an empty namespace "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.info(
                u"{0:d} elements with an empty namespace have been "
...
class EadNamespaceRemover(TransformAction):
    ...
    category = u"Naming"
    desc = (
        u"This action puts all the XML elements with the Ape-EAD standard "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.info(
                u"{0:d} elements with the Ape-EAD namespace have been "
