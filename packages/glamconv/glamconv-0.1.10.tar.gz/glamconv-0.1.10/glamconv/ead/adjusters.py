...
class ArchdescLevelAttribAdjuster(TransformAction):
    ...
    desc = (
        u"In the 'level' attribute of <archdesc> elements the only "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if log_details:
            log_data = []
        for arch in xml_root.xpath(u'.//archdesc[not(@level) or @level!="fonds"]'):
            msg = u""
            value = arch.get(u"level", u"")
            if (
                value not in [u"", u"otherlevel"]
...
class DscTypeAttribAdjuster(TransformAction):
    ...
    category = u"Cleansing"
    desc = (
        u"In the 'type' attribute of <dsc> elements the only possible "
...
class EmphRenderAttribAdjuster(TransformAction):
    ...
    category = u"Cleansing"
    desc = (
        u"In the 'render' attribute of <emph> elements the only possible "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if log_details:
            log_data = []
        for emph in xml_root.xpath(u'.//emph[@render!="bold" and @render!="italic"]'):
...
LNGS = (
    u"roh sco scn rom ron alg oss ale alb scc mni nld mno osa mnc scr oci "
...
SCRS = (
    u"Tfng Shaw Cirt Guru Hebr Geor Zzzz Phnx Hrkt Plrd Ogam Telu Bopo "
...
class ListAttribsAdjuster(TransformAction):
    ...
    category = u"Cleansing"
    desc = (
        u"The 'numeration' and the 'type' attributes of <list> elements "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for list_elt in xml_root.xpath(u".//list[@numeration or @type]"):
            ...
            if msg != "":
                if log_details:
                    log_data.append(
                        log_element(list_elt, msg=(u"    Deleted attributes:" + msg))
                    )
                count += 1
        if count > 0:
            logger.warning(
