...
class InternalLinkTransformer(TransformAction):
    ...
    desc = (
        u"In Ape-EAD, the elements describing an internal link (<ptr> and "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for elt in xml_root.xpath(".//ref|.//ptr"):
            ...
            count += 1
            if log_details:
                log_data.append(log_element(elt, attributes=(u"href", u"xlink:href")))
        if count > 0:
            logger.warning(
...
class ExternalLinkTransformer(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"This action transforms the <archref>, <bibref>, <extptr> and "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        legit_elt = {
            u"bibliography": u"bibref",
            u"unitid": u"extptr",
            u"descrules": u"extref",
            u"item": u"extref",
            u"p": u"extref",
            u"repository": u"extref",
        }
        xpath_req = u" | ".join([u".//{0}".format(eltname) for eltname in names])
        for elt in xml_root.xpath(xpath_req):
            ...
            elt.tag = newname
            count += 1
            lnk = elt.get(u"href") or elt.get(u"{{{xlink}}}href".format(**NS), u"")
            if newname == "extptr" and len(elt.text or u"") > 0:
                if log_details:
                    log_data.append(
                        log_element(
                            elt,
                            msg=u"    Text content will be deleted",
                ...
                if log_details:
                    log_data.append(
                        log_element(
                            elt,
                            msg=u"    href will be added as text content",
                            attributes=(u"href", u"xlink:href"),
                        )
                    )
            elif log_details:
                log_data.append(
                    log_element(
                        elt, attributes=(u"href", u"xlink:href"), text_content=True
                    )
                )
        if count > 0:
            logger.warning(
...
class DaodescEraser(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"<daodesc> elements don't exist anymore in Ape-EAD. This "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <daodesc> elements have been deleted." u"".format(count)
            )
            if log_details:
                logger.warning(
                    u"The following elements have been deleted:", u"\n".join(log_data)
                )
        return xml_root
...
class DaoContentEraser(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"In Ape-EAD, <dao> elements must be empty (no text, not even "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            ...
            if log_details:
                logger.warning(
                    u"The following <dao> elements have been " u"emptied.",
                    u"\n".join(log_data),
                )
        return xml_root
...
class DaogrpTransformer(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"Ape-EAD doesn't have any element for describing the extended "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for daogrp in xml_root.xpath(u".//daogrp"):
            for daoloc in daogrp.xpath(u"daoloc"):
                ...
                if log_details:
                    log_data1.append(
                        log_element(daoloc, attributes=(u"href", u"xlink:href"))
                    )
            if len(daogrp) > 0:
                count2 += 1
                if log_details:
        ...
        if count1 > 0:
            ...
            if log_details:
                logger.warning(
                    u"The following <dao> elements have been " u"defined:",
                    u"\n".join(log_data1),
                )
        if count2 > 0:
            logger.warning(
            ...
            if log_details:
                logger.warning(
                    u"The following non-empty <daogrp> elements " u"have been deleted:",
                    u"\n".join(log_data2),
                )
        return xml_root
...
class ArchdescCDaoMover(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"In Ape-EAD, the <dao> elements can't be directly inserted into "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for dao in xml_root.xpath(u".//c/dao | .//archdesc/dao"):
            ...
            count += 1
            if log_details:
                log_data.append(log_element(dao, attributes=(u"href", u"xlink:href")))
        if count > 0:
            logger.warning(
            ...
            if log_details:
                logger.warning(
                    u"The following <dao> elements have " u"been moved:",
                    u"\n".join(log_data),
                )
        return xml_root
...
class OddDaoTransformer(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"In Ape-EAD, the <dao> elements can't exist inside an <odd> "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        for odd in xml_root.xpath(u".//odd[dao]"):
            ...
            for dao in odd.xpath(u"dao"):
                dao.tag = u"extref"
                lnk = dao.get(u"href") or dao.get(u"{{{xlink}}}href".format(**NS), u"")
                title = dao.get(u"title") or dao.get(
                    u"{{{xlink}}}title".format(**NS), u""
                )
                if title != u"":
                    dao.text = title
                else:
...
class LinkgrpEraser(TransformAction):
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        if count > 0:
            logger.warning(
                u"{0:d} <linkgrp> elements have been deleted." u"".format(count)
            )
            if log_details:
                logger.warning(
                    u"The following elements have been deleted:", u"\n".join(log_data)
...
class XLinkAttribSetter(TransformAction):
    ...
    category = u"Links & Refs"
    desc = (
        u"This action transforms the attributes of <bibref>, <extprt>, "
    ...
    def _execute(self, xml_root, logger, log_details):
        ...
        actuate_conv = {
            ...
            u"actuatenone": u"none",
            u"actuateother": u"other",
        }
        show_conv = {
            u"new": u"new",
            u"embed": u"embed",
...
class DaoCharacterReplacer(TransformAction):
    ...
    params_def = (
        SingleParameter(
            u"old",
            u"Old",
            u"Expression to be found in the link and to be replaced by " u"the new one",
            u"Text",
            text_type,
            u"",
        ),
