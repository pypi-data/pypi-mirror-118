...
any state :math:`\rho`.
Since both $A_k^\dagger A_k$ and $\rho$ are positive semidefinite, the probabilities are guaranteed to be nonnegative.
...
.. note::
   If one does not care about the state after the measurement, one can throw away this information by defining 
   the positive operator-valued measure (POVM) operators $E_k := A_k^\dagger A_k$. For projective measurements they are the same as $A_k$,
   but generally they only provide the probabilities of the various outcomes.
