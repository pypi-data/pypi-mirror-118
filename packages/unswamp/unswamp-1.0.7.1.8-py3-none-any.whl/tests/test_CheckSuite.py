...
class TestCheckSuite(TestCase):
    def test_CheckSuite_properties(self):
        ...
        dataset_name = TestHelpers.str_random()
        checks = ["newkey", "newvalue"]
        suite = CheckSuite(suite_id, dataset_name, checks)

        TestHelpers.test_property(self, suite, "id", suite_id)
        TestHelpers.test_property(self, suite, "dataset_name", dataset_name)
    ...
    def test_CheckSuite_run(self):
        ...
        checks = [check]
        suite = CheckSuite(suite_id, name, checks)

        dataset = TestHelpers.get_dataset()
        run_name = TestHelpers.str_random()
        check_run = suite.run(dataset, run_name)
        self.assertTrue(
