...
class BaseLSTDataset(Dataset):
    ...
    def __init__(self, hdf5_file_path, camera_type, group_by, targets=None, particle_dict=None, use_time=False, train=True,
        ...
        assert self.camera_type == 'LST_LSTCam'
        ...
        lstcam = CameraGeometry.from_name('LSTCam')
        ...
        with tables.File(hdf5_file_path, 'r') as hdf5_file:
            ...
            if self.simu:
                ...
                self.trig_tels[:, np.searchsorted(layout['tel_id'], self.dl1_params['tel_id'])] = True  # TODO fix when real data has several tels
            # Select sub-subarray
...
class MemoryLSTDataset(BaseLSTDataset):
    ...
    def __init__(self, hdf5_file_path, camera_type, group_by, targets=None, particle_dict=None, use_time=False, train=True,
        ...
        with tables.File(hdf5_file_path, 'r') as hdf5_file:
            ...
            if self.use_time:
                self.times = hdf5_file.root[dl1_images_lstcam_key].col('peak_time').astype(np.float32)[self.filtered_indices]
...
class FileLSTDataset(BaseLSTDataset):
    ...
    def _get_image_data(self, idx):
        ...
        if isinstance(image_id, np.ndarray):
            ...
            for i, ind in enumerate(image_id):
                if ind > -1:
                    data[i] = np.nan_to_num(self.hdf5_file.root[dl1_images_lstcam_key].col('image')[ind].astype(np.float32))
                    if self.use_time:
                        time[i] = np.nan_to_num(self.hdf5_file.root[dl1_images_lstcam_key].col('peak_time')[ind].astype(np.float32))
            data = (data,)
            if self.use_time:
                data += time
        else:
            ...
            if self.use_time:
                time = np.nan_to_num(self.hdf5_file.root[dl1_images_lstcam_key].col('peak_time')[image_id].astype(np.float32))
                data += time,
        return data
    ...
    def _open_file(self):
        if self.hdf5_file is None:
            ...
            self.times = np.nan_to_num(self.hdf5_file.root[dl1_images_lstcam_key].col('peak_time')[self.filtered_indices])
