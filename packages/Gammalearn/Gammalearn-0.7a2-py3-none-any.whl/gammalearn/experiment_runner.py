...
from pytorch_lightning.callbacks import ModelCheckpoint, GPUStatsMonitor, LearningRateMonitor
...
# dictionnary of profiling method
profile_dict = {
    'simple': {'profiler': SimpleProfiler, 'options': {'extended': True}},
    'advanced': {'profiler': AdvancedProfiler, 'options': {}},
    'pytorch': {'profiler': PyTorchProfiler, 'options': {'use_cuda':True}},
}
...
class LitGLearnModule(LightningModule):
    ...
    def __init__(self, experiment):
        ...
        # self.camera_parameters = load_camera_parameters(self.experiment.dataset_parameters['camera_type'])
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        self.manual_backward(loss, optimizers)
        ...
        self.log('Loss_weighted', {'training': loss.item()}, on_step=False, on_epoch=True)
        # log other metrics
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('Loss_validating', loss.item())
        self.log('Loss_weighted', {'validating': loss.item()})
        # log other metrics
        for task, all_metrics in self.metrics.items():
            for name, metric in all_metrics.items():
                if task == 'class':
...
def main():
    ...
    parser.add_argument('--fast_debug', help='log useful information for debug purpose',
                        action='store_true')
    parser.add_argument('--profile', type=str, choices=list(profile_dict.keys()),
                        help='log useful information for debug purpose')
    ...
    # TODO find a solution for stereo with different camera types
    geometries = []
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=os.path.join(experiment.main_directory, experiment.experiment_name),
                                          ...
                                          period=experiment.save_every, save_top_k=-1)
    ...
    if profile is not None:
        profiler = profile_dict[profile]['profiler'](os.path.join(experiment.main_directory+'/'
                                                                  + experiment.experiment_name,
                                                                  experiment.experiment_name + '.prof'),
                                                     **profile_dict[profile]['options'])
    else:
        profiler = None

    # Run !
    if fast_debug:
        trainer = pl.Trainer(fast_dev_run=True, gpus=-1, accelerator=experiment.ddp, profiler=profiler)
        trainer.fit(gl_lightning_module,
                    train_dataloader=gl_data_module.train_dataloader(),
                    val_dataloaders=gl_data_module.val_dataloader())
        trainer.test(gl_lightning_module,
                     test_dataloaders=gl_data_module.test_dataloader())
    else:
        trainer = pl.Trainer(
            default_root_dir=os.path.join(experiment.main_directory, experiment.experiment_name),
            gpus=experiment.gpus, accelerator=experiment.ddp, max_epochs=experiment.max_epochs,
        ...
        if experiment.train:
            ...
            for dataloader in gl_data_module.test_dataloaders():
                trainer.test(gl_lightning_module, test_dataloaders=dataloader)
                gl_lightning_module.reset_test_data()
