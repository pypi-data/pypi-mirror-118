...
class Bot:
    ...
    def __init__(self, token, sequence = 0, session_id = None, bot_identifier = "python/discordbot/v1.1", logger = default_logger):
        ...
        async def check_death(dt):
            if self._die:
                ...
                self._ws_close_code = 1001
                try:
                    await self._ws_conn.close(self._ws_close_code)
                except:
                    pass
                self._ws_conn = None
        ...
        while not self._stop_periodics:
            for d in self._periodics:
                ...
                try:
                    ...
                    # make sure it will not run for at least 5 seconds
                    d[1] = time.time() + max(0, 5 - call_rate)
    ...
    async def _send_payload(self, pl):
        ...
        try:
            ...
            self._ws_close_code = self._ws_conn.close_code
            self._ws_conn = None
            raise e
    ...
    async def _attempt_full_connect(self):
        ...
        try:
            if self._ws_conn != None:
                self._ws_close_code = 1006
                await self._ws_conn.close(1006) # will reconnect
        except:
        ...
        if self.session_id == None:
            if self._ws_close_code != None:
                # this is the second (or more) time that i've identified
                # there must be a 1-5 second wait
            ...
            if self._ws_close_code == None: # first connection
                self.self._ws_first_ever_connect = False
        ...
        while not self._die:
            ...
            try:
                ...
                if util.error_caused_by(e, websockets.ConnectionClosedError):
                    if self._ws_close_code == 4004:
                        p_err = error.DiscordError("Invalid bot token.")
                        p_err.__cause__ = e
                        self.log_error(p_err)
                        return False
                    elif self._ws_close_code == 4009:
                        self.session_id = None
                    else:
                        self.log("Bot will reconnect due to websocket closing with code %d." % self._ws_close_code)
                    self._ws_reconnect = True
