...
# ==============================================================================
from research.segment3d.models import randlanet
import torch
...
def conv1d(in_channels, out_channels, kernel_size, stride, padding, bn=True, activation=True):
  return nn.Sequential(
      ...
      nn.LeakyReLU(0.2, True) if activation else nn.Identity())
...
class DilatedResidualBlock(nn.Module):
  ...
  def __init__(self, d_out):
    ...
    # self.mlp3 = conv2d(d_out // 2, d_out // 2, 1, 1, 0)
    # self.att_pooling_3 = AttentionPooling(d_out // 2 * 3 + d_out, d_out)
  ...
  def forward(self, xyz, feature, neigh_idx):
    ...
    # # [bs, 2*ndim, num_points, num_neighbor]
    # f_concat = torch.cat([f_neighbours, f_xyz3, f_xyz2, f_xyz1], dim=1)

    # # [bs, 2*dim, num_points, 1]
    # f_pc_agg = self.att_pooling_3(f_concat)
...
class RandLANet(nn.Module):
  ...
  def __init__(self, num_classes=13, num_layers=6, feature_dims=[16, 64, 128, 256, 512],
               stem_in=6, stem_out=8):
    super(RandLANet, self).__init__()
    self.num_layers = num_layers
    self.num_classes = num_classes

    # mapping xyz-rgb to feature space
    self.stem = conv1d(stem_in, stem_out, 1, 1, 0)
  ...
  def forward(self, xyz, neigh_idx, sub_idx, interp_idx, features):
    ...
    return f_out, base_features.squeeze().transpose(2, 1)
...
class RandLANetStack(nn.Module):

  def __init__(self, num_classes=13, num_layers=6, feature_dims=[16, 64, 128, 256, 512], num_stacks=2):
    super(RandLANetStack, self).__init__()
    assert num_stacks >= 2, 'require at least two RandLANet.'
    nets = [RandLANet(num_classes=num_classes,
                      num_layers=num_layers,
                      feature_dims=feature_dims,
                      stem_in=6,
                      stem_out=8)]
    for _ in range(num_stacks - 1):
      net = RandLANet(num_classes=num_classes,
                      num_layers=num_layers,
                      feature_dims=feature_dims,
                      stem_in=32,
                      stem_out=16)
  ...
  def forward(self, xyz, neigh_idx, sub_idx, interp_idx, features):
    """consecutive two unet
    """
    outs = []
    for _, net in enumerate(self.nets):
      out, features = net(xyz, neigh_idx, sub_idx, interp_idx, features)
      outs.append(out)
    return tuple(outs)
