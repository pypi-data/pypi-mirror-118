...
SENT_EMAILS=[]
EMAIL_BLACKLIST=eXyrY()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",eXyrF(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=eXyrG,from_name=eXyrG,smtp_host=eXyrG,smtp_user=eXyrG,smtp_pass=eXyrG,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if eXyru(recipients,eXyrS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=eXyrG):
 ...
 result["From"]=formataddr((eXyrF(Header(from_name,"utf-8")),from_email))
 return result
