...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return RLXcW if e else RLXcY
 def attach(self,cli:LocalstackCli)->RLXcI:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(RLXcI)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:RLXct):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,RLXcI)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:RLXct):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,RLXcI)
def print_table(columns:Dict[RLXct,RLXct],rows:List[Dict[RLXct,Any]],formatters:Dict[RLXct,Callable[[Any],RLXct]]=RLXcI):
 from rich.table import Table
 if formatters is RLXcI:
  formatters=RLXcS()
 t=Table()
