...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[rDfms])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[rDfms])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[rDfms])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @rDfmk
 def fetch_details(cls,api_id,rDfmt,method,uri=rDfms):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return rDfms
  integrations=[r for r in resp["Items"]if rDfmt==r.get("IntegrationType")and uri in[rDfms,r.get("IntegrationUri")]and method in[rDfms,r.get("IntegrationMethod")]]
  return(integrations or[rDfms])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or rDfms
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
