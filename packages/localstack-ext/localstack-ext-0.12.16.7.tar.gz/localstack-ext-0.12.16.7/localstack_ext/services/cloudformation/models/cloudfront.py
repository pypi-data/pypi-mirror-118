...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xLzcb
 @xLzci
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xLzcI(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xLzcJ
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xLzcN(origins,xLzcp)else origins.get("Items")
  origins_to_create=xLzcs([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xLzcN(aliases,xLzcp)else aliases.get("Items")
  aliases_to_create=xLzcs(aliases)
  for distr in distrs:
   origins=xLzcs([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xLzcs(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xLzcQ)
   config["Enabled"]=xLzcP(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xLzcN(custom_errors,xLzcD)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xLzcP(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xLzcQ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xLzcP(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xLzcb):
    parent=config if parent is xLzcb else parent
    items=parent.get(key)or[]
    if xLzcN(items,xLzcD)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xLzcv(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xLzcN(cache_behavs,xLzcD)else cache_behavs)
