...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ltOrs(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ltOrU
   return result
  except ltOrK:
   props=self.props
   ...
   if parts[0]in candidates and ltOrB(parts)>1:
