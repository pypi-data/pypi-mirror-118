...
class CognitoUserPool(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return SuJRc(CognitoUserPool,self).get_cfn_attribute(attribute_name)
 ...
 def fetch_details(cls,pool_name):
  ...
  return([p for p in pools if p["Name"]==pool_name]or[SuJRo])[0]
 def get_deploy_templates():
  def get_user_pool_params(params,**kwargs):
   ...
   if(policies.get("PasswordPolicy")or{}).get("MinimumLength"):
    policies["PasswordPolicy"]["MinimumLength"]=SuJRm(policies["PasswordPolicy"]["MinimumLength"])
   if SuJRg(result.get("AutoVerifiedAttributes"),SuJRL):
   ...
   for schema_attr in result.get("Schema")or[]:
    ...
    if attr_constr:
     attr_constr["MinLength"]=SuJRL(attr_constr.get("MinLength",1))
     attr_constr["MaxLength"]=SuJRL(attr_constr.get("MaxLength",50))
   return result
...
class IdentityPool(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   return SuJRc(IdentityPool,self).get_cfn_attribute(attribute_name)
  except SuJRr:
...
class CognitoUserPoolClient(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  if attribute_name in REF_ID_ATTRS:
   return self.get_physical_resource_id(attribute_name)
  return SuJRc(CognitoUserPoolClient,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
 ...
 def fetch_details(cls,pool_id,client_name):
  ...
  return([c for c in clients if c["ClientName"]==client_name]or[SuJRo])[0]
...
class CognitoIdentityPoolRoleAttachment(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if roles:
   roles["_deployed"]=SuJRs
  return roles or SuJRo
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
