...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or yeHJG
 @yeHJX
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return yeHJh(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @yeHJM
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if yeHJE(origins,yeHJg)else origins.get("Items")
  origins_to_create=yeHJW([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if yeHJE(aliases,yeHJg)else aliases.get("Items")
  aliases_to_create=yeHJW(aliases)
  for distr in distrs:
   origins=yeHJW([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=yeHJW(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",yeHJf)
   config["Enabled"]=yeHJn(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if yeHJE(custom_errors,yeHJU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=yeHJn(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":yeHJf}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=yeHJn(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=yeHJG):
    parent=config if parent is yeHJG else parent
    items=parent.get(key)or[]
    if yeHJE(items,yeHJU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":yeHJd(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if yeHJE(cache_behavs,yeHJU)else cache_behavs)
