...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=GidrU(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not Gidra
   return result
  except Gidrm:
   props=self.props
   ...
   if parts[0]in candidates and GidrK(parts)>1:
