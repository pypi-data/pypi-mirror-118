...
SENT_EMAILS=[]
EMAIL_BLACKLIST=BsJwo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",BsJwK(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=BsJwF,from_name=BsJwF,smtp_host=BsJwF,smtp_user=BsJwF,smtp_pass=BsJwF,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if BsJwr(recipients,BsJwD)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=BsJwF):
 ...
 result["From"]=formataddr((BsJwK(Header(from_name,"utf-8")),from_email))
 return result
