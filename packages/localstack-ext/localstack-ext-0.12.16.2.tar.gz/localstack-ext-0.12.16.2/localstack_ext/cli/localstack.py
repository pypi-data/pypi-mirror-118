...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return poBkb if e else poBkO
 def attach(self,cli:LocalstackCli)->poBkX:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(poBkX)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:poBkR):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,poBkX)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:poBkR):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,poBkX)
def print_table(columns:Dict[poBkR,poBkR],rows:List[Dict[poBkR,Any]],formatters:Dict[poBkR,Callable[[Any],poBkR]]=poBkX):
 from rich.table import Table
 if formatters is poBkX:
  formatters=poBkG()
 t=Table()
