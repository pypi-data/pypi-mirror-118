...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return qdnMm if e else qdnMW
 def attach(self,cli:LocalstackCli)->qdnMR:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(qdnMR)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:qdnME):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,qdnMR)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:qdnME):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,qdnMR)
def print_table(columns:Dict[qdnME,qdnME],rows:List[Dict[qdnME,Any]],formatters:Dict[qdnME,Callable[[Any],qdnME]]=qdnMR):
 from rich.table import Table
 if formatters is qdnMR:
  formatters=qdnMT()
 t=Table()
