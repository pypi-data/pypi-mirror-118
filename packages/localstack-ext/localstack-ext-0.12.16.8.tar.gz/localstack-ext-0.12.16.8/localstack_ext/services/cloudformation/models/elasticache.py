...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=qKJyz(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not qKJyh
   return result
  except qKJyx:
   props=self.props
   ...
   if parts[0]in candidates and qKJyC(parts)>1:
