...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or LEOCN
 @LEOCq
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return LEOCe(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @LEOCp
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if LEOCQ(origins,LEOCR)else origins.get("Items")
  origins_to_create=LEOCd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if LEOCQ(aliases,LEOCR)else aliases.get("Items")
  aliases_to_create=LEOCd(aliases)
  for distr in distrs:
   origins=LEOCd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=LEOCd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",LEOCn)
   config["Enabled"]=LEOCi(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if LEOCQ(custom_errors,LEOCU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=LEOCi(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":LEOCn}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=LEOCi(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=LEOCN):
    parent=config if parent is LEOCN else parent
    items=parent.get(key)or[]
    if LEOCQ(items,LEOCU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":LEOCA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if LEOCQ(cache_behavs,LEOCU)else cache_behavs)
