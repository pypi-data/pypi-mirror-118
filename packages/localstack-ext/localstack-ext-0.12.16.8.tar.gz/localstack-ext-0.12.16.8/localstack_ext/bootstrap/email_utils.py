...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xnJQz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",xnJQb(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xnJQg,from_name=xnJQg,smtp_host=xnJQg,smtp_user=xnJQg,smtp_pass=xnJQg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xnJQt(recipients,xnJQR)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xnJQg):
 ...
 result["From"]=formataddr((xnJQb(Header(from_name,"utf-8")),from_email))
 return result
