...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return VyHTI if e else VyHTN
 def attach(self,cli:LocalstackCli)->VyHTM:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(VyHTM)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:VyHTc):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,VyHTM)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:VyHTc):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,VyHTM)
def print_table(columns:Dict[VyHTc,VyHTc],rows:List[Dict[VyHTc,Any]],formatters:Dict[VyHTc,Callable[[Any],VyHTc]]=VyHTM):
 from rich.table import Table
 if formatters is VyHTM:
  formatters=VyHTU()
 t=Table()
