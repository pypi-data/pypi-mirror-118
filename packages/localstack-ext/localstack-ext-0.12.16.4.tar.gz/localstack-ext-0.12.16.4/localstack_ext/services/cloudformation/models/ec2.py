...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=PTIcE,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if PTIcC(perm["IpProtocol"])!=PTIcC(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",PTIcE)
   vpc_id=PTIcE
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or PTIcE
    vpc_id=groups and groups[0].get("VpcId")or PTIcE
   dst_group_id=result.pop("DestinationSecurityGroupId",PTIcE)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=PTIcC(result.get("IpProtocol"))
   description=result.pop("Description",PTIcE)
   cidr_ipv6=result.pop("CidrIpv6",PTIcE)
