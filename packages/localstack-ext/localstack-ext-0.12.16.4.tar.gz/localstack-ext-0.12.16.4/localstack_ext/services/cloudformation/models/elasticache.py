...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tqJlO(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tqJlK
   return result
  except tqJlo:
   props=self.props
   ...
   if parts[0]in candidates and tqJlk(parts)>1:
