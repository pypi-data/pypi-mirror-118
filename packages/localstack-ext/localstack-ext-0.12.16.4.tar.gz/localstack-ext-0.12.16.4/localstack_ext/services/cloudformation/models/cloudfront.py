...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or AkrGN
 @AkrGs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return AkrGd(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @AkrGt
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if AkrGF(origins,AkrGH)else origins.get("Items")
  origins_to_create=AkrGK([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if AkrGF(aliases,AkrGH)else aliases.get("Items")
  aliases_to_create=AkrGK(aliases)
  for distr in distrs:
   origins=AkrGK([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=AkrGK(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",AkrGw)
   config["Enabled"]=AkrGi(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if AkrGF(custom_errors,AkrGR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=AkrGi(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":AkrGw}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=AkrGi(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=AkrGN):
    parent=config if parent is AkrGN else parent
    items=parent.get(key)or[]
    if AkrGF(items,AkrGR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":AkrGP(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if AkrGF(cache_behavs,AkrGR)else cache_behavs)
