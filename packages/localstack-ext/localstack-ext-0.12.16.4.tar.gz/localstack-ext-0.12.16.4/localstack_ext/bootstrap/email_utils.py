...
SENT_EMAILS=[]
EMAIL_BLACKLIST=hBVst()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",hBVsG(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=hBVsY,from_name=hBVsY,smtp_host=hBVsY,smtp_user=hBVsY,smtp_pass=hBVsY,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if hBVsH(recipients,hBVsJ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=hBVsY):
 ...
 result["From"]=formataddr((hBVsG(Header(from_name,"utf-8")),from_email))
 return result
