...
SENT_EMAILS=[]
EMAIL_BLACKLIST=IDQrE()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",IDQrU(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=IDQrW,from_name=IDQrW,smtp_host=IDQrW,smtp_user=IDQrW,smtp_pass=IDQrW,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if IDQrP(recipients,IDQrY)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=IDQrW):
 ...
 result["From"]=formataddr((IDQrU(Header(from_name,"utf-8")),from_email))
 return result
