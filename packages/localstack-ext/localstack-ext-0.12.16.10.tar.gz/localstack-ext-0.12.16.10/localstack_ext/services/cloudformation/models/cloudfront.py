...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or rMEat
 @rMEaP
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return rMEaW(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @rMEax
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if rMEad(origins,rMEaA)else origins.get("Items")
  origins_to_create=rMEaF([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if rMEad(aliases,rMEaA)else aliases.get("Items")
  aliases_to_create=rMEaF(aliases)
  for distr in distrs:
   origins=rMEaF([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=rMEaF(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",rMEaK)
   config["Enabled"]=rMEaQ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if rMEad(custom_errors,rMEai)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=rMEaQ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":rMEaK}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=rMEaQ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=rMEat):
    parent=config if parent is rMEat else parent
    items=parent.get(key)or[]
    if rMEad(items,rMEai)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":rMEaJ(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if rMEad(cache_behavs,rMEai)else cache_behavs)
