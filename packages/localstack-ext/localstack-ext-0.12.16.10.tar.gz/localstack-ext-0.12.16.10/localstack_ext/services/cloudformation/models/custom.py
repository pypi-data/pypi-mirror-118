...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   if elSQz(service_token).startswith("arn:aws:lambda"):
   ...
   result=elSQY
   try:
    result=retry(fetch_result,retries=elSQm(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except elSQr:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,elSQT(bucket_objects.keys())))
    raise
