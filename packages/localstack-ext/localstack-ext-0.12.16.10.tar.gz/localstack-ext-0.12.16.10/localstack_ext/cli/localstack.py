...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return xnlIs if e else xnlIU
 def attach(self,cli:LocalstackCli)->xnlIY:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(xnlIY)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:xnlIA):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,xnlIY)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:xnlIA):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,xnlIY)
def print_table(columns:Dict[xnlIA,xnlIA],rows:List[Dict[xnlIA,Any]],formatters:Dict[xnlIA,Callable[[Any],xnlIA]]=xnlIY):
 from rich.table import Table
 if formatters is xnlIY:
  formatters=xnlIB()
 t=Table()
