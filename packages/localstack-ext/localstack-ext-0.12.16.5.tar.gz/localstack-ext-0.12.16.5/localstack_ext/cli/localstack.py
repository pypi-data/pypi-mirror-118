...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return NdKFL if e else NdKFy
 def attach(self,cli:LocalstackCli)->NdKFr:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(NdKFr)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:NdKFA):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,NdKFr)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:NdKFA):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,NdKFr)
def print_table(columns:Dict[NdKFA,NdKFA],rows:List[Dict[NdKFA,Any]],formatters:Dict[NdKFA,Callable[[Any],NdKFA]]=NdKFr):
 from rich.table import Table
 if formatters is NdKFr:
  formatters=NdKFx()
 t=Table()
