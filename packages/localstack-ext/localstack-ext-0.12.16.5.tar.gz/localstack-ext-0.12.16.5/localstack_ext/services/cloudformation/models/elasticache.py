...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=SRqXY(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not SRqXM
   return result
  except SRqXs:
   props=self.props
   ...
   if parts[0]in candidates and SRqXQ(parts)>1:
