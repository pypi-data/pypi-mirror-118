...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or peyYn
 @peyYJ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return peyYL(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @peyYv
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if peyYK(origins,peyYD)else origins.get("Items")
  origins_to_create=peyYO([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if peyYK(aliases,peyYD)else aliases.get("Items")
  aliases_to_create=peyYO(aliases)
  for distr in distrs:
   origins=peyYO([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=peyYO(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",peyYV)
   config["Enabled"]=peyYP(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if peyYK(custom_errors,peyYE)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=peyYP(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":peyYV}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=peyYP(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=peyYn):
    parent=config if parent is peyYn else parent
    items=parent.get(key)or[]
    if peyYK(items,peyYE)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":peyYB(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if peyYK(cache_behavs,peyYE)else cache_behavs)
