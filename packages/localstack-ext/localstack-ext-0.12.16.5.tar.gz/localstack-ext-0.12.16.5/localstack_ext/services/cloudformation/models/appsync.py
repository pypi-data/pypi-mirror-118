...
class AppSyncResolver(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,type_name,field_name,data_source_name):
  ...
  return(result or[YXLTf])[0]
 def get_deploy_templates():
  def get_resolver_params(params,**kwargs):
   params=keys_to_lower(clone(params))
   req_template=params.pop("requestMappingTemplateS3Location",YXLTf)
   res_template=params.pop("responseMappingTemplateS3Location",YXLTf)
   if req_template:
    tmpl=aws_utils.download_s3_object(req_template,as_str=YXLTu)
    if tmpl:
     params["requestMappingTemplate"]=tmpl
   if res_template:
    tmpl=aws_utils.download_s3_object(res_template,as_str=YXLTu)
    if tmpl:
...
class GraphQLSchema(GenericBaseModel):
 ...
 @YXLTs
 def fetch_details(cls,api_id,definition=YXLTf,s3_location=YXLTf):
  client=aws_stack.connect_to_service("appsync")
  try:
   schema=client.get_introspection_schema(apiId=api_id,format="SDL")["schema"]
   schema=schema.read()if YXLTU(schema,"read")else schema
...
class AppSyncApiKey(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YXLTj(AppSyncApiKey,self).get_cfn_attribute(attribute_name)
 def fetch_state(self,stack_name,resources):
  props=self.props
  api_id=props.get("ApiId")
...
class AppSyncDataSource(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return([s for s in sources if s["name"]==ds_name]or[YXLTf])[0]
class AppSyncFunctionConfig(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YXLTj(AppSyncFunctionConfig,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
...
class GraphQLAPI(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  if not props.get("apiId"):
   ...
   if not details:
    raise YXLTn('Unable to find GraphQL API named "%s"'%props.get("Name"))
  ...
  if attribute_name=="GraphQLUrl":
   return appsync_api.get_graphql_url(props["apiId"],use_domain_name=YXLTu)
  return YXLTj(GraphQLAPI,self).get_cfn_attribute(attribute_name)
 def get_physical_resource_id(self,attribute,**kwargs):
  props=self.props
 ...
 def fetch_details(cls,api_name):
  ...
  return([a for a in apis if a["name"]==api_name]or[YXLTf])[0]
