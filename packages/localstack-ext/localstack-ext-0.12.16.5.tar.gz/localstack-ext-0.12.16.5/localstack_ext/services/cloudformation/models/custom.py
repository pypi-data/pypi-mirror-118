...
class CustomResource(GenericBaseModel):
 ...
 def get_deploy_templates():
  def create_custom_resource(resource_id,resources,resource_type,func,stack_name,*args):
   ...
   if MgOLA(service_token).startswith("arn:aws:lambda"):
   ...
   result=MgOLd
   try:
    result=retry(fetch_result,retries=MgOLi(CUSTOM_RESOURCES_RESULT_POLL_TIMEOUT/2),sleep=2)
    result=json.loads(to_str(result))
   except MgOLl:
    bucket_objects=map_all_s3_objects(buckets=[CUSTOM_RESOURCES_RESULTS_BUCKET])
    LOG.info("Unable to fetch CF custom resource result from s3://%s/%s . Existing keys: %s"%(CUSTOM_RESOURCES_RESULTS_BUCKET,result_key,MgOLG(bucket_objects.keys())))
    raise
