...
SENT_EMAILS=[]
EMAIL_BLACKLIST=FavRy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",FavRW(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=FavRp,from_name=FavRp,smtp_host=FavRp,smtp_user=FavRp,smtp_pass=FavRp,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if FavRI(recipients,FavRB)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=FavRp):
 ...
 result["From"]=formataddr((FavRW(Header(from_name,"utf-8")),from_email))
 return result
