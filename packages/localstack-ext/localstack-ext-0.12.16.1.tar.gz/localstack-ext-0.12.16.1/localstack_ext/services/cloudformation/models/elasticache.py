...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=uvhUA(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not uvhUb
   return result
  except uvhUI:
   props=self.props
   ...
   if parts[0]in candidates and uvhUs(parts)>1:
