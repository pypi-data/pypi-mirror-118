...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or UItqX
 @UItqw
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return UItqY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @UItqD
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if UItqc(origins,UItqu)else origins.get("Items")
  origins_to_create=UItqa([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if UItqc(aliases,UItqu)else aliases.get("Items")
  aliases_to_create=UItqa(aliases)
  for distr in distrs:
   origins=UItqa([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=UItqa(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",UItqE)
   config["Enabled"]=UItqv(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if UItqc(custom_errors,UItqO)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=UItqv(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":UItqE}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=UItqv(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=UItqX):
    parent=config if parent is UItqX else parent
    items=parent.get(key)or[]
    if UItqc(items,UItqO)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":UItqR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if UItqc(cache_behavs,UItqO)else cache_behavs)
