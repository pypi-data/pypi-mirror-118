...
SENT_EMAILS=[]
EMAIL_BLACKLIST=idGKw()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",idGKL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=idGKb,from_name=idGKb,smtp_host=idGKb,smtp_user=idGKb,smtp_pass=idGKb,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if idGKO(recipients,idGKT)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=idGKb):
 ...
 result["From"]=formataddr((idGKL(Header(from_name,"utf-8")),from_email))
 return result
