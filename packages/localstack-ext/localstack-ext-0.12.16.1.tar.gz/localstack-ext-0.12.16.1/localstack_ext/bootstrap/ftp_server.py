...
def apply_patches():
 ...
 extended_proto_cmds.update({"SITE ADDUSER":RDYbi(perm="m",auth=RDYbQ,arg=RDYbQ,help="Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.")})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=RDYbO)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=RDYbQ)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=RDYbQ):
...
def main():
 do_start_ftp(RDYby(sys.argv[1]),asynchronous=RDYbO)
