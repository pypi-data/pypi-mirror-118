...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return lYwFr if e else lYwFA
 def attach(self,cli:LocalstackCli)->lYwFa:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(lYwFa)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:lYwFg):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,lYwFa)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:lYwFg):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,lYwFa)
def print_table(columns:Dict[lYwFg,lYwFg],rows:List[Dict[lYwFg,Any]],formatters:Dict[lYwFg,Callable[[Any],lYwFg]]=lYwFa):
 from rich.table import Table
 if formatters is lYwFa:
  formatters=lYwFM()
 t=Table()
