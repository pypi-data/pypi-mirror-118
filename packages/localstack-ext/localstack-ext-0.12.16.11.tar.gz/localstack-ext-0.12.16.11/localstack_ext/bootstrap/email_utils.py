...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rBsyS()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rBsyL(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rBsyK,from_name=rBsyK,smtp_host=rBsyK,smtp_user=rBsyK,smtp_pass=rBsyK,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rBsyM(recipients,rBsyf)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rBsyK):
 ...
 result["From"]=formataddr((rBsyL(Header(from_name,"utf-8")),from_email))
 return result
