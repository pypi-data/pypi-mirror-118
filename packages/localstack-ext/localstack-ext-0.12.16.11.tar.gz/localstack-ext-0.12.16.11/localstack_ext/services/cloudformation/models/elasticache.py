...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=oHStx(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not oHStF
   return result
  except oHStU:
   props=self.props
   ...
   if parts[0]in candidates and oHSti(parts)>1:
