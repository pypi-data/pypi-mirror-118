...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=STQqD(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not STQqN
   return result
  except STQqR:
   props=self.props
   ...
   if parts[0]in candidates and STQqi(parts)>1:
