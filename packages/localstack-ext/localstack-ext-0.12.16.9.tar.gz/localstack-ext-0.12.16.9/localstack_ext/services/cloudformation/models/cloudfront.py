...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or pYWiF
 @pYWid
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return pYWiI(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @pYWiv
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if pYWie(origins,pYWiy)else origins.get("Items")
  origins_to_create=pYWig([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if pYWie(aliases,pYWiy)else aliases.get("Items")
  aliases_to_create=pYWig(aliases)
  for distr in distrs:
   origins=pYWig([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=pYWig(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",pYWib)
   config["Enabled"]=pYWiz(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if pYWie(custom_errors,pYWiV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=pYWiz(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":pYWib}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=pYWiz(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=pYWiF):
    parent=config if parent is pYWiF else parent
    items=parent.get(key)or[]
    if pYWie(items,pYWiV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":pYWiG(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if pYWie(cache_behavs,pYWiV)else cache_behavs)
