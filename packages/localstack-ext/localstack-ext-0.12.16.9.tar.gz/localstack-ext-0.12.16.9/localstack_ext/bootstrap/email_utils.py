...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xNgWj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",xNgWk(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xNgWA,from_name=xNgWA,smtp_host=xNgWA,smtp_user=xNgWA,smtp_pass=xNgWA,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xNgWD(recipients,xNgWm)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xNgWA):
 ...
 result["From"]=formataddr((xNgWk(Header(from_name,"utf-8")),from_email))
 return result
