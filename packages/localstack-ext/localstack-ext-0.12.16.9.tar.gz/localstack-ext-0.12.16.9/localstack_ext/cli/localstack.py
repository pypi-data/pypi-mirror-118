...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return vFfIX if e else vFfIr
 def attach(self,cli:LocalstackCli)->vFfIo:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(vFfIo)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:vFfIg):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,vFfIo)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:vFfIg):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,vFfIo)
def print_table(columns:Dict[vFfIg,vFfIg],rows:List[Dict[vFfIg,Any]],formatters:Dict[vFfIg,Callable[[Any],vFfIg]]=vFfIo):
 from rich.table import Table
 if formatters is vFfIo:
  formatters=vFfIl()
 t=Table()
