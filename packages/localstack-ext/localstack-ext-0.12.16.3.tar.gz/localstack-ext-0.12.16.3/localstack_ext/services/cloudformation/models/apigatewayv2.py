...
class ApiGatewayV2DomainName(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["DomainName"]==domain_name]or[tFHPo])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Authorizer(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  result=([a for a in apis if a["Name"]==auth_name]or[tFHPo])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Api(GenericBaseModel):
 ...
 def fetch_details(cls,api_name):
  ...
  result=([a for a in apis if a["Name"]==api_name]or[tFHPo])[0]
  return result
 def get_physical_resource_id(self,attribute,**kwargs):
...
class ApiGatewayV2Integration(GenericBaseModel):
 ...
 @tFHPn
 def fetch_details(cls,api_id,tFHPG,method,uri=tFHPo):
  client=aws_stack.connect_to_service("apigatewayv2")
  resp=client.get_integrations(ApiId=api_id)
  if "Items" not in resp:
   return tFHPo
  integrations=[r for r in resp["Items"]if tFHPG==r.get("IntegrationType")and uri in[tFHPo,r.get("IntegrationUri")]and method in[tFHPo,r.get("IntegrationMethod")]]
  return(integrations or[tFHPo])[0]
...
class ApiGatewayV2Stage(GenericBaseModel):
 ...
 def fetch_details(cls,api_id,stage_name):
  client=aws_stack.connect_to_service("apigatewayv2")
  return client.get_stage(ApiId=api_id,StageName=stage_name)or tFHPo
 def get_physical_resource_id(self,attribute,**kwargs):
  if attribute in REF_ID_ATTRS:
