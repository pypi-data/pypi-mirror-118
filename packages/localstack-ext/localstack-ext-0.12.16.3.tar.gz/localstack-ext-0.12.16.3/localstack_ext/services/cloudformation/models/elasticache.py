...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tXnIY(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tXnIG
   return result
  except tXnIV:
   props=self.props
   ...
   if parts[0]in candidates and tXnIs(parts)>1:
