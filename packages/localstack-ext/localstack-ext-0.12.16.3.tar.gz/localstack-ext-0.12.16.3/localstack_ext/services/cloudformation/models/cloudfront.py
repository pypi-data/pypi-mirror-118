...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or fjkQG
 @fjkQy
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return fjkQp(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @fjkQB
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if fjkQJ(origins,fjkQi)else origins.get("Items")
  origins_to_create=fjkQa([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if fjkQJ(aliases,fjkQi)else aliases.get("Items")
  aliases_to_create=fjkQa(aliases)
  for distr in distrs:
   origins=fjkQa([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=fjkQa(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",fjkQS)
   config["Enabled"]=fjkQl(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if fjkQJ(custom_errors,fjkQU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=fjkQl(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":fjkQS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=fjkQl(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=fjkQG):
    parent=config if parent is fjkQG else parent
    items=parent.get(key)or[]
    if fjkQJ(items,fjkQU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":fjkQs(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if fjkQJ(cache_behavs,fjkQU)else cache_behavs)
