...
SENT_EMAILS=[]
EMAIL_BLACKLIST=RGyuO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",RGyuY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=RGyuz,from_name=RGyuz,smtp_host=RGyuz,smtp_user=RGyuz,smtp_pass=RGyuz,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if RGyuj(recipients,RGyuK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=RGyuz):
 ...
 result["From"]=formataddr((RGyuY(Header(from_name,"utf-8")),from_email))
 return result
