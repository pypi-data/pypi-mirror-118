...
SENT_EMAILS=[]
EMAIL_BLACKLIST=NLvVT()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",NLvVp(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=NLvVM,from_name=NLvVM,smtp_host=NLvVM,smtp_user=NLvVM,smtp_pass=NLvVM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if NLvVO(recipients,NLvVX)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=NLvVM):
 ...
 result["From"]=formataddr((NLvVp(Header(from_name,"utf-8")),from_email))
 return result
