...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return rKhAl if e else rKhAP
 def attach(self,cli:LocalstackCli)->rKhAQ:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(rKhAQ)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:rKhAE):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,rKhAQ)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:rKhAE):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,rKhAQ)
def print_table(columns:Dict[rKhAE,rKhAE],rows:List[Dict[rKhAE,Any]],formatters:Dict[rKhAE,Callable[[Any],rKhAE]]=rKhAQ):
 from rich.table import Table
 if formatters is rKhAQ:
  formatters=rKhAV()
 t=Table()
