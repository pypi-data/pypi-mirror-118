...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or PFYCt
 @PFYCJ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return PFYCv(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @PFYCT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if PFYCA(origins,PFYCy)else origins.get("Items")
  origins_to_create=PFYCi([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if PFYCA(aliases,PFYCy)else aliases.get("Items")
  aliases_to_create=PFYCi(aliases)
  for distr in distrs:
   origins=PFYCi([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=PFYCi(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",PFYCR)
   config["Enabled"]=PFYCG(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if PFYCA(custom_errors,PFYCe)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=PFYCG(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":PFYCR}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=PFYCG(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=PFYCt):
    parent=config if parent is PFYCt else parent
    items=parent.get(key)or[]
    if PFYCA(items,PFYCe)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":PFYCW(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if PFYCA(cache_behavs,PFYCe)else cache_behavs)
