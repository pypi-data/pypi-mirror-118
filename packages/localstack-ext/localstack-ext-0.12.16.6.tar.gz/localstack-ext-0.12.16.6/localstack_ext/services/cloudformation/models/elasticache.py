...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xitMC(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xitMO
   return result
  except xitMe:
   props=self.props
   ...
   if parts[0]in candidates and xitMv(parts)>1:
