...
from data_verifier.exceptions.exception import VerificationError, SessionRetrieveException
...
class AhrefsSiteDetails(BaseResposne):
    ...
    def login_attempt(self,hidden):
        ...
        try:
            ...
            print("ERROR 0: ",response.text,response.url,response.headers,self.headers,response.status_code)
            raise VerificationError(response.status_code)
    ...
    def main_site(self):
        ...
        print("ERROR 1: ",response.text,response.url,response.headers,self.headers,response.status_code)
    ...
    def get_tokens(self):
        ...
        try:
            ...
            print("ERROR 2: ", response.text, response.url, response.headers, self.headers, response.status_code)
            raise SessionNotActive
    ...
    def do_login(cls, db_ob, **kwargs):
        ...
        except (SessionExpired,SessionNotActive):
            print("failed taking another attempt...")
...
def test():
    ...
    return AhrefsSiteDetails.do_login(applicant,persistor=persistor,details=True)
