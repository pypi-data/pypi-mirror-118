...
class UnleashAdmin:
    ...
    def create_feature_toggle(self, input):
        ...
        try:
            ...
            data = None
            try:
                data = self.get_feature_toggle(input["name"])
            except Exception:
                if data:
                    print(f"Feature Toggle '{input['name']}' already existing..")
                    return data
                else:
                    res = requests.post(
                        url=url, headers=self.headers, data=json.dumps(input)
                    )
...
class UnleashClient:
    def __init__(self, address, auth_token, app_name):
