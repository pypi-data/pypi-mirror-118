...
def run_threaded(func: Callable,
                 ...
                 optional_iterable: Optional[Iterable] = None,
                 **kwargs):
    def _func(_it, _it_optional=None):
        if _it_optional:
            func(_it, _it_optional, **kwargs)
        else:
            func(_it, **kwargs)

    if not disable_threading and not optional_iterable:
    ...
    elif not disable_threading and optional_iterable:
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            list(
                tqdm(
                    executor.map(
        ...
        for it in tqdm(iterable, desc=desc, disable=disable_tqdm):
            _func(it)
