...
class Configs:
    ...
    def get_from_s3(self, statics):
        ...
        try:
            self.download(s3_bucket, s3_folder + "/" + filename, os.path.join(zip_path, "tmp.zip"))
    ...
    def s3_path_finder(self, load_file: str):
        if load_file.startswith("s3://"):
            ...
            logging.info(f"{Colors.BOLD}{Colors.OKBLUE}{conf_name} file will be downloading from S3{Colors.ENDC}\n")
            self.download(bucket_name, filepath, conf_name)
            return conf_name
        else:
            logging.info(f"{Colors.BOLD}{Colors.OKBLUE}{load_file} file will be loading from local.{Colors.ENDC}\n")
            return load_file
    ...
    def launch(self):
        ...
        if response:
            if response["statusCode"] == 200:
                ...
                logging.error(f"{Colors.FAIL}Experiment failed, {response.get('body')}{Colors.ENDC}")
        else:
            logging.error("Experiment could not start")
        ...
        while True:
            if status != "finished" and status != "error":
                ...
                if status == "running":
                    ...
                    logging.info(f"{Colors.OKBLUE}Status: {status}, Progress: {progress}{Colors.ENDC}")
                elif status == "pending":
                    logging.info(f"Status: {status}, experiment id: {id}, running environment: {env}")
                elif status == "finished":
                    print(f"{id}")
                    logging.info(f"{Colors.OKGREEN}{Colors.UNDERLINE}Experiment finished!{Colors.ENDC}")
                elif status == "error" or status == "stopped":
                    logging.error(f"{Colors.FAIL}Experiment finished with Error.")
                    logging.error(f"{response['body'].get('msg', 'unknown')}{Colors.ENDC}")
                    # download experiment
                ...
                # download experiment
                self.client.download(id, folder, self.path)
                logging.info(f"Experiment with id: {id} downloaded in outputs folder as ZIP")
                return True
...
def run():
    ...
    if config:
        ...
        if not config.keys():
            logging.error(f"{Colors.FAIL}No experiment parameter is defined.{Colors.ENDC}")
            sys.exit(0)
