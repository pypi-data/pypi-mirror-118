...
class QueryWizzard:
    ...
    def get_model(self, model: Type[Model] or str) -> Type[Model]:
    ...
    def get_m21_value(self, relation, filter_column, value) -> int or None:
        if relation.load_all:
            if not value:
                return None
            return int(value)
    ...
    def insert_query(self, model, data):
        ...
        for column, value in data.items():
            ...
            if field is None:
                ...
                relation = model.get_relation(column)
                if isinstance(relation, ManyToOne):
                    fk_id = self.get_m21_value(relation, relation.insert_search_column, value)
                    if fk_id:
    ...
    def create(self, model: Type[Model] or str, data, return_created_object=True, commit=True) -> dict or int:
        if not data:
            raise ValueError('Invalid argument data: should not be empty')
        model = self.get_model(model)
    ...
    def update(self, model, filter_condition, instance_id, prev_data, data):
        ...
        for column, value in data.items():
            ...
            if isinstance(relation, ManyToOne):
                fk_id = self.get_m21_value(relation, relation.update_search_column, value)
                if fk_id:
