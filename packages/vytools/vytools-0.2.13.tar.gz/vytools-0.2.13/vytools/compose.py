  ...
  for tag in ordered: # SORTED TO ENSURE THIS ORDER DEFINITION, ARGUMENT, STAGE, ARTIFACT
    ...
    if val.startswith('definition:'):
      ...
      if len(splitname) == 2 and len(artname) > 0:
        ...
        if build_level == -1 and '..' not in artname and eppath and os.path.exists(eppath):
          ...
          if arttyp in ['artifact','file']:
            if os.path.isdir(artifactpath):
              logging.error('The {} "{}" already exists as a directory. You will need to delete it to continue with this compose file'.format(arttyp,artifactpath))
              return False
        ...
        if arttyp == 'vydir':
          replace_obj[tag] = os.path.relpath(Path(artname).resolve(), start=Path(eppath).resolve())
    ...
    if type(obj[k]) == str:
      ...
      for key in keys:
        try:
          ...
          logging.error('Failed to substitute anchor {} in vy compose "{}". {}'.format(key,cfile,exc))
          return False
