...
class LinuxLocalOperation():
    def start_mock_server(self,mock_server_port):
        self.stop_port_process(mock_server_port)

    def stop_port_process(self,mock_server_port):
...
class MockServerClass:
    ...
    def __init__(self,mock_server_port,mock_server_log_path="request_data.txt",remote_mock_server_machine=None):
        """
        ...
        self.remote_mock_server_machine = remote_mock_server_machine
        self.mock_server_port = mock_server_port
        self.mock_server_log_path = mock_server_log_path

        if self.remote_mock_server_machine == None:
            if platform.system() == "Windows":
                self.system_operation = WindoxLocalOperation()
            elif platform.system() == "Linux":
                self.system_operation = LinuxLocalOperation()
        else:
            self.system_operation = LinuxRemoteOperation(remote_mock_server_machine)

        mock_server_listen_port = mock_server_port
        mock_server_ip = self.remote_mock_server_machine["ip"] if self.remote_mock_server_machine else "127.0.0.1"
        self.query_request_data_interface_url = f"http://{mock_server_ip}:{mock_server_listen_port}/get_mock_server_all_request_data"
        self.clear_request_data_interface_url = f"http://{mock_server_ip}:{mock_server_listen_port}/clear_mock_server_request_data"
    ...
    def add_route(self,route_path,response_date="{}",response_status='200',response_headers='{}',processing_time = '0'):
        ...
        route_code_list_len = len(self.route_code_list)
...
if __name__=="__main__":
        ...
        self.system_operation.start_mock_server(self.mock_server_port)
        sleep(4)

    #此函数用于,启动模拟服务之前,先把特定端口号的进程kill调
    def stop_port_process(self):
        self.system_operation.stop_port_process(self.mock_server_port)

    def stop_mock_server(self):
        self.system_operation.stop_mock_server()
    ...
    def get_mock_server_request_data_list(self):
        ...
        mock_server_request_data_json = requests.get(self.query_request_data_interface_url).json()
    ...
    def clear_mock_server_request_data(self):
        requests.get(self.clear_request_data_interface_url)
