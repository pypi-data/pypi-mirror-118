...
class LogMidddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        if context.get('x-cloud-trace-context'):
            request.state.trace=f"projects/{PROJECT_ID}/traces/{context.get('x-cloud-trace-context')}"
        else:
            request.state.trace=f"projects/{PROJECT_ID}/traces/{context.get('X-Request-ID')}"
...
class logging():
    ...
    def get_trace() -> str:
        if context.get('x-cloud-trace-context'):
            return f"projects/{PROJECT_ID}/traces/{context.get('x-cloud-trace-context')}"
