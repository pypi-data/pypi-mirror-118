...
def make_tormicro_app(
    ...
    handlers = [
        (IndexRequestHandler.URI_PATTERN, IndexRequestHandler, dict(config=config, logger=logger)),
        (InfoRequestHandler.URI_PATTERN, InfoRequestHandler, dict(config=config, logger=logger)),
    ]
