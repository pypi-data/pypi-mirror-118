...
class DjangoQueuePool(QueuePool):
    def __init__(
        ...
        if self.logging_name and kwargs.get("echo"):
    ...
    def status(self):
        pid = os.getpid()
        status = (
            f"Mysql Connection Pool of {pid}; "
            f"Pool size: {self.size()}; Connections in pool: {self.checkedin()}; "
            f"Current Overflow: {self.overflow()}; Current Checked out: {self.checkedout()}"
        )
