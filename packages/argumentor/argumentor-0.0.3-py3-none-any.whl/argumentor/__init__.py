...
class Arguments:
    ...
    def __generate_help_page(self) -> str:
        ...
        for option in self.options:
            if self.options[option]['group'] is None:
                line = ''
                if self.options[option]['short_operation'] is not None:
                    line += f'  {self.options[option]["short_option"]}'
                line += f'  {option}'
                if len(line) > 15:
                    line += f'\t\t\t{self.options[option]["description"]}\n'
    ...
    def add_operation(self, operation: str, value_type, description: str = None,
        ...
        if operation in self.operations.keys():
            ...
            raise InvalidOptionError(f'short operation can only be one-character long when no dash, not "{short_operation}"')
        elif short_operation is not None and len(short_operation) > 2:
            raise InvalidOptionError(f'short operation can contain a dash and a character maximum, not "{short_operation}"')
        else:
    ...
    def add_operation_group(self, group_name: str) -> None:
        ...
        if group_name in self.operation_groups:
            ...
            self.operation_groups[group_name] = {'operations': []}
    ...
    def parse(self) -> [dict, dict]:
        ...
        for argument in self.arguments:
            ...
            if argument in self.operations.keys():
                ...
                if not self.operations[self.active_operation]['value_type'] == bool:
        ...
        # Check if all active operations that require a list have at least one value
