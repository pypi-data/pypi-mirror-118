...
class CertoraBuildGenerator:
    ...
    def generate_function_finder(self, calleeName: Optional[str], f: Func) -> Optional[Tuple[str, str, str]]:
        debug_print(f"Generating function finder for signature {f.source_code_signature()}")
        """
    ...
    def add_auto_finders(self, build_arg_contract_file: str, out: str, contracts: List[ContractInSDC],
        ...
        for c in sorted(contracts, key=lambda c: c.original_file):
            debug_print(f"Adding auto finders for {c.name}")
            ...
            if is_lib:
                ...
                if len(colliding_refs) == 0:
                    ...
                    library_functions_to_add \
                        .extend([self.generate_library_function_finder(c.name, f) for f in
                                 sorted([g for g in c.internal_funcs])
                ...
                for c in contracts_in_file_sorted:
                    ...
                    if c.name == primary_contract:
                        contract_functions_to_add = [self.generate_contract_function_finder(f)
                                                     for f in
                                                     # sort for deterministically generated solidity files
                                                     sorted([g for g in c.internal_funcs.union(c.public_funcs)])]
