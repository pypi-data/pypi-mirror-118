...
class TrainApi(BaseApi):
    ...
    @arg_parse_validation(
        arg_setup=train_arg_setup, validate_setup=train_validate_setup
    )
    @adapt_to_http_response
    @api_logger("train")
    def post(self, args):
        return alg_manager.train(
            args["algorithm"],
            args["model_path"],
            args["data"],
            args["parameter"],
...
class StatusApi(BaseApi):
    ...
    @arg_parse_validation(
        arg_setup=status_arg_setup, validate_setup=status_validate_setup
    )
    @adapt_to_http_response
    @api_logger("status")
    def get(self, args):
...
class PredictApi(BaseApi):
    ...
    @arg_parse_validation(
        arg_setup=predict_arg_setup, validate_setup=predict_validate_setup
    )
    @adapt_to_http_response
    @api_logger("predict")
    def post(self, args):
        return dispatcher.dispatch_predict(
            args["model_id"], args["features"], args["uuid"], args["params"]
        )
...
class LoadApi(BaseApi):
    ...
    @api_logger("load")
    def post(self, args):
        return dispatcher.dispatch_load(
            args["algorithm"], args["model_id"], args["model_path"]
        )
...
class UnloadApi(BaseApi):
    ...
    @arg_parse_validation(
        arg_setup=unload_arg_setup, validate_setup=unload_validate_setup
    )
    @adapt_to_http_response
    @api_logger("unload")
    def post(self, args):
...
class DeleteApi(BaseApi):
    ...
    @arg_parse_validation(
        arg_setup=delete_arg_setup, validate_setup=delete_validate_setup
    )
    @adapt_to_http_response
    @api_logger("delete")
    def post(self, args):
        dispatcher.dispatch_unload(args["algorithm"], args["model_id"])
...
class NodeInfoApi(BaseApi):
    api_url = "/node_info"

    @arg_parse_validation()
    @adapt_to_http_response
    def get(self, args):
        return node_info.statistics()
