...
class Config():
    ...
    datadict_info = { # stores the structure of the config (consists of a section name and its containing fields; the field also stores its type)
        'info': {'name': {"data": 'ABC', "type": 'str'},
                 'number': {"data": '999', "type": 'int'},
                 },
        }
    path_info = "config.ini" # standard path to save the config file
    list_convert_type = {"int": convertype.conv_int, # supported data types
    ...
    def __init__(self,
        ...
        if datadict_: self.datadict = datadict_
        else: self.datadict = self.datadict_info
        if path_: self.path = path_
        else: self.path = self.path_info
        self.load()
    ...
    def get(self,
        ...
        if head:
            if field:
                if data: return self.datadict[head][field][data]
                else: return self.list_convert_type[self.datadict[head][field]["type"]](self.datadict[head][field]["data"])
            else: return self.datadict[head]
        else: return self.datadict
    ...
    def set(self,
        ...
        if type_ == None: type_ = convertype.type_list[str(type(var))]
    ...
    def delete(self,
        ...
        if field:
            if data: del self.datadict[head][field][data]
            else: del self.datadict[head][field]
        else: del self.datadict[head]
        self.save()
