...
class RelatedView(BaseView):
    ...
    def get(
        ...
        **kwargs,
        ...
        try:
            if not related_obj:
                related_obj, related_mapping = self.injection_data.related_data("GET")
            if not m_to_m_obj:
                m_to_m_obj, m_to_m_mapping = self.injection_data.m_to_m_data("GET")
            if results.data and results.data.result:
                result = self.relate.extend_result_field(
                    obj_serializer, related_obj, m_to_m_obj, related_mapping, m_to_m_mapping, current_page_queryset
                )
    ...
    def post(
        ...
        **kwargs,
        ...
        if not related_obj:
            related_obj, related_mapping = self.injection_data.related_data("POST")
        if not m_to_m_obj:
            m_to_m_obj, m_to_m_mapping = self.injection_data.m_to_m_data("POST")
