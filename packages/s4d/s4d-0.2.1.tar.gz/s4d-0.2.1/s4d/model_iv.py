...
class ModelIV:
    ...
    def debug_model(self):
        ...
        if self.norm_mean is not None:
            print('norm_mean: ', self.norm_mean.shape)
            print('norm_cov: ', self.norm_cov.shape)
    ...
    def train_per_speaker(self, show, feature_server, idmap, input_feature_filename=None, normalization=True):
        """
        ...
        """
        stat = StatServer(idmap, distrib_nb=self.ubm.distrib_nb(), feature_size=self.ubm.dim())
        stat, _ = stat.sum_stat_per_model() # just used to reduce the size of the statserver

        if input_feature_filename is None:
            input_feature_filename = idmap.rightids[0]

        stat.accumulate_stat_per_speaker(ubm=self.ubm,
                                         input_feature_filename=input_feature_filename,
                                         idmap=idmap,
