...
class DER:
    ...
    def confusion2pandas(self):
        """Generate a matrice version of the confusion matrix
        ...
        mat = self.conf/100
        pd_mat = pandas.DataFrame(data=mat, columns=self.hyp_cluster_list, index=self.ref_cluster_list)
        return pd_mat

         
    #def confusion2notebook(self):
    #    df = self.confusion2pandas()
    ...
    def assignment(self):
        ...
        self.not_assigned_ref = copy.deepcopy(self.ref_cluster_list)
        ...
        for i in range(self.assigned.shape[0]):
            ...
            if s > 0:
                ...
                self.not_assigned_ref.remove(r)
                name_assigned[(r, h)] = s
                self.match += s
            else:
                self.assigned[i, 0] = -1
        ...
        return name_assigned, self.not_assigned, self.not_assigned_ref

    def assignment2pandas(self):
        """Generate a matrice version of the confusion matrix
        ...
        mat = self.conf / 100
        l = list()
        name_assigned, not_hyp, not_ref = self.assignment()
        for key, value in name_assigned.items():
            l.append((key[0], key[1], value/100))
        for r in not_ref:
            idx = self.ref_cluster_list.index(r)
            l.append((r, 'None', mat[idx,:].sum()))
        for h in not_hyp:
            idx = self.hyp_cluster_list.index(h)
            l.append(('None', h,  mat[:,idx].sum()))        
        
        return pandas.DataFrame(data=l, columns=['ref', 'hyp','confusion'])
        
    #def assignment2notebook(self):
    #    display(HTML(self.assignment2pandas().to_html()))
    ...
    def get_table(self, name=None, time=True, rate=True, overlap=False):
        ...
        if time:
            if not add_name:
                ...
                lst += self.time(name=None, overlap=overlap)
...
def get_header(speech=True, overlap=False, speaker=True):
    return DER_result.header(speech=True, overlap=False, speaker=True)
