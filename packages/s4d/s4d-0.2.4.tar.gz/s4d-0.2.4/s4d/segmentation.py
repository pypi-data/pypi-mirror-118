...
def get_borders(distances, start, stop, show='empty', win=250):
    """
    ...
    # find local maximal at + or - win size
    borders = scipy.signal.argrelmax(distances, order=win)[0].tolist()
    # append the first and last
    borders = [0] + borders + [stop-start]

    diarization_out = Diar()
    spk = 0
    for i in range(0, len(borders) - 1):
        diarization_out.append(show=show, start=start+borders[i],
                               stop=start+borders[i + 1], cluster='S' + str(spk))
        spk += 1
    return diarization_out
