...
class Namespace:
    ...
    def create_tree(self, src):
        match = re.search(
            r"namespace\s*([a-zA-Z\.]*)\s*\{\s*([\s\S]*)\s*}", src)
        if match is None:
            return self
        self.Name = match.group(1)
        self.Code = match.group(2)[:-1]
        ...
        for char in self.Code:
            ...
            if close_brace_count == open_brace_count and close_brace_count > 0:
                match = re.search(
                    r"[^//](public|internal|)\s*class\s*([a-zA-Z]*)", self.Code)
                if match is None:
                    continue
                self.Classes.append(
                    Class(Code=_class_code, Name=match.group(2)).create_tree())
                open_brace_count = 0
                close_brace_count = 0
                _class_code = ''
        return self
