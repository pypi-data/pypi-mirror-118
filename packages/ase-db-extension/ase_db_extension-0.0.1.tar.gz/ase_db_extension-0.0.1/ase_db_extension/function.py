...
def db_from_ase_csv(csv_file: Union[str, pd.DataFrame], new_database_name="new_database.db", index_col=0)->Database:
...
def db_from_structure_csv(csv_file: Union[str, pd.DataFrame], new_database_name="new_database.db", index_col=0,
    ...
    with database:
        for k in data.keys():
            datak = data[k]
            structure = datak.pop(name)
            structure = eval(structure)
            st = Structure.from_dict(structure)
...
def db_rename(database: Database,name_pair=(("old_name1","new_name1"),), check=True ,selection=None, **kwargs)->Database:
    if check:
