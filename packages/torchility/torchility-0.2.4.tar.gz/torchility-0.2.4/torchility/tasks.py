...
class GeneralTaskModule(LightningModule):
    ...
    def training_step(self, batch, batch_nb):               # 训练步
        ...
        self.log('train_loss', loss.detach(), prog_bar=True, on_step=self.on_step, on_epoch=self.on_epoch)
    ...
    def test_step(self, batch, batch_nb):                   # 测试步
        loss, preds, targets = self.do_forward(batch)
        self.log('test_loss', loss.detach(), prog_bar=True, on_step=self.on_step, on_epoch=self.on_epoch)
    ...
    def do_metric(self, preds, targets, state, log):               # 指标计算
        for metric in self.metrics:
            ...
            log(f"{state}_{name}", result.detach(), prog_bar=True, on_step=on_step, on_epoch=on_epoch)
