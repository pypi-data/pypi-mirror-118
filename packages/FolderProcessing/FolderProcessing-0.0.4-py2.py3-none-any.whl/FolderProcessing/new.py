...
def new_del(command, path, name):
    import shutil
    """del File"""
    if command == 'del':
        path_del = path
        name_del = name
        if os.path.isdir(path_del+name_del):
            os.remove(path_del+name_del)
            print(f"{name_del} File deleted !")
        else:
            print(f"file {name_del} does not exist !")
    if command == 'del_folder': 
        path_folder = path
        name_folder = name
        shutil.rmtree(f'{path_folder}{name_folder}')
        print(f"Deleted folder{path_folder}{name_folder} ！")

def new_input(file, name, message=''):
    input_name = name
    input_message = message
    input_file = file
    input_file
    input_file = open(f"{input_file}{input_name}","w")
