        ...
        if quotes is not None:
            for item in quotes:
                for k in ['ask_p', 'ask_v', 'bid_p', 'bid_v']:
                    item[k]=item.get(k, 0.0 if k.endswith('_p') else 0)  # 以 _p 结尾的都是浮点型。如果没有值的时候，默认值赋值为0
...
def run(strategy_id='', filename='', mode=MODE_UNKNOWN, token='',
    ...
    context.on_l2transaction_fun_v2 = getattr(fmodule, 'on_l2transaction_v2', None)
    ...
    context.on_algo_order_status_fun = getattr(fmodule, 'on_algo_order_status', None)
    context.on_algo_order_status_fun_v2 = getattr(fmodule, 'on_algo_order_status_v2', None)
    ...
    if _all_not_none(context.on_algo_order_status_fun, context.on_algo_order_status_fun_v2):
        gmsdklogger.warning('{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_algo_order_status'))
    ...
    if _all_not_none(context.on_l2transaction_fun, context.on_l2transaction_fun_v2):
        gmsdklogger.warning('{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_l2transaction_fun'))
