...
class Context(object):
    ...
    def data(symbol, frequency, count=1, fields='', bar_df=True):
        ...
        if frequency == DATA_TYPE_L2TRANSACTION:
            gmsdklogger.warning('context.data 不支持 frequency={}, 返回空数据', DATA_TYPE_L2TRANSACTION)
            if bar_df:
                return pd.DataFrame([])
            else:
                return []
        ...
        if frequency == DATA_TYPE_TICK:
            if symbol in context.tick_data_cache:
                ...
                if count > len(symbol_tick_queue):
                    ...
                    adjust_end_time = context.now.strftime('%Y-%m-%d %H:%M:%S') if context.is_live_model() else context.backtest_end_time
                ...
                if len(fields) == 0:
                    ...
                    if len(out_fields)>0 and fields !='':
                        gmsdklogger.warning(','.join(out_fields)+'不在合法的字段列表中，合法字段:'+','.join(all_fields))
    ...
    def _get_account_info(self, account_id, account_name):
        ...
        if result:
            ...
            for position in positions.data:
                ...
                if properties is not None:
                    position['credit_position_sellable_volume'] = properties.get('credit_position_sellable_volume', 0)
        ...
        if result:
            ...
            if len(acc_statuses.data) > 0:
                conn_status = acc_statuses.data[0].status  # type: Union[ConnectionStatus, None]

        account = Account(account_id, account_name, cash, positions_infos, conn_status)
