...
def merge_aligned_near_boxes_by_label (boxes, size, closed = 10):
...
def union_boxes_by_label (labels, overlap_thresh = 0.5):
    if len (labels) == 1:
        return labels
    ...
    return np.concatenate ([boxes [pick].astype ("int"), labels [pick]], axis = 1)
