    ...
    def start_live_data(self, resolved_contracts, req_id=None):  # TODO: Prevent reuse of req_ids
        ...
        for i , req_id in enumerate(req_ids):
            ...
            if self.live_websocket.subscription_type == 'tick':
                ...
                elif req_id not in self.live_data.keys() or type(self.live_data[req_id]) != TickLiveData:
                ...
                if resolved_contract in self.symbol_mkt_id_map.keys():
                    ...
                    self.live_data[req_id] = TickLiveData(resolved_contract)
                if resolved_contract in self.symbol_mkt_id_map.keys():
                    self.min_live_data[req_id] = self.min_live_data[list(self.symbol_mkt_id_map[resolved_contract])[0]]
                elif req_id not in self.min_live_data.keys() or type(self.min_live_data[req_id]) != MinLiveData:
                    self.min_live_data[req_id] = MinLiveData(resolved_contract)
            elif 'min' in self.live_websocket.subscription_type:
                if resolved_contract in self.symbol_mkt_id_map.keys():
                    self.live_data[req_id] = self.live_data[list(self.symbol_mkt_id_map[resolved_contract])[0]]
                elif req_id not in self.live_data.keys() or type(self.live_data[req_id]) != MinLiveData:
