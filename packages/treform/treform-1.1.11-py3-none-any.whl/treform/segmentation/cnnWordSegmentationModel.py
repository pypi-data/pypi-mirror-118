...
def train(model_path='./models/checkpoint-{epoch}.ckpt',
          log_dir='./logs',
          train_file='',
          dev_file='',
          training_config='',
    ...
    train_dataset = (
        tf.data.TextLineDataset(tf.constant([train_file]))
            .shuffle(25000000)
            .map(
    ...
    dev_dataset = (
        tf.data.TextLineDataset(tf.constant([dev_file]))
            .shuffle(100000)
            .map(
            string_to_example(vocab_table),
            num_parallel_calls=tf.data.experimental.AUTOTUNE,
...
if __name__ == "__main__":
    ...
    train(model_path=model_path, log_dir=log_dir,
          train_file=train_file, dev_file=dev_file,
          training_config=training_config, char_file=char_file)
