...
class JobDataStructure(MainDataBase):
    ...
    # def _get_experiment_run_ids_from_period(self, period_start):
    #     """
    #     This function receives period start in timestamp format (int)
    #     Then it returns the unique run_id of the jobs that fall in this period.
    ...
    #             cur = self.conn.cursor()
    #             cur.execute(
    #                 "select run_id from job_data where start >= ? or finish >= ? GROUP BY run_id", (period_start, period_start))
    #             rows = cur.fetchall()
    #             print(type(rows))
    #             if len(rows) > 0:
    #                 return rows
