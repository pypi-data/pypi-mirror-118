...
class Algorithm(ABC):
    ...
    def adjust(self,original,img,index=None, value=None):
        plt_close()
...
def plt_show(b=None, g=None, r=None, av=None):
    ...
    if av!=None:
        #print('show av')
        plt.plot(x, av,  color='black', label="Ave")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.ylim(0, 255)
    plt.legend()
...
#        result = cv2.resize(img,dsize=None,fx=4,fy=4)
    ...
    def filt_width(self, original_list, number,std_list, max_std):
...
##        height, width = image.shape[:2]
##        top    = image[0:int(height/3), 0:width]
##        mid    = image[int(height/3):int(height*2/3), 0:width]
##        bottom = image[int(height*2/3):height, 0:width]
##
##        #print(top.shape[:2],mid.shape[:2],bottom.shape[:2])
##        
##        top_mn, top_std = self.block_mean_std(top,i)
##        mid_mn, mid_std = self.block_mean_std(mid,i)        
##        btm_mn, btm_std = self.block_mean_std(bottom,i)
##
##
##        mn_list = [top_mn, mid_mn, btm_mn]
##        max_mn = max(mn_list)
##        m_index = mn_list.index(max_mn)
##
##        std = [top_std, mid_std, btm_std][m_index]
##
##        return max_mn, std
...
##                            offset+width: offset+width+(width - reduce)]
##
##        up_mn    =[self.av_mean(left), self.av_mean(mid), self.av_mean(right)]
##        up_block =[left,mid,right]
##        print(up_mn)
##        
##        mid       = image[h_h: height,
##                            offset: offset+width]                            
##        left      = image[h_h: height,
##                            offset-(width-reduce): offset]                            
##        right     = image[h_h: height,
##                            offset+width: offset+width+(width - reduce)]
##        down_mn    =[self.av_mean(left), self.av_mean(mid), self.av_mean(right)]
##        down_block =[left,mid,right]
##        print(down_mn)
##
##        rtn =False
##        if up_mn[1] == min(up_mn) and down_mn[1] == min(down_mn):
##            if(min([up_mn[1], down_mn[1]])>90):
##                rtn =True
##        return rtn , [left,mid,right]
    ...
    def calculation(self, image):
        ...
        thresh, std = self.threshold(gray)
        ...
        block_offset =142
        block_width  = calibration.IDEAL_BAR_WIDTH*2+ int(calibration.IDEAL_BAR_WIDTH/2)
        ...
        #filt_list  = self.filt(mn_list, -29)
        #std_list = self.filt(std_list)

        std_filter = 6
        mn_filter  = 100
        width_filter = 10      # 0.0, 13,13
        number_of_low_mn = 20#30 #calibration.IDEAL_BAR_WIDTH
        

##        std_filter = 50
##        mn_filter  = 100
##        width_filter = 10      # 22, 5,19
##        number_of_low_mn = 30 #calibration.IDEAL_BAR_WIDTH


        #filt_width_list_no_std = self.filt_width_no_std(mn_list, number_of_low_mn)

        filt_width_list = self.filt_width(mn_list, number_of_low_mn, std_list, std_filter)
