...
def main():
    ...
    if os.path.exists('lastopen.tvg'):
        root.update()
        ask = messagebox.askyesno('TreeViewGui', 'Want to open previous file?', parent = root)
        if ask:
    ...
    if filename:
        ...
        if os.path.exists(f'{filename}_tvg'):
            try:
        ...
        messagebox.showwarning('File', 'No File Name!', parent = self.root)
        root.destroy()