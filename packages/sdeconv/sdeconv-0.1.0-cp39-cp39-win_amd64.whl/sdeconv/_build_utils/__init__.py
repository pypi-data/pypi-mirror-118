...
def cythonize_extensions(top_path, config):
    ...
    # check simple compilation with OpenMP. If it fails sdeconv will
    ...
    # sdeconv module (only at build-time) to be used twice:
    # - First to set the value of sdeconv_OPENMP_PARALLELISM_ENABLED, the
    ...
    sdeconv._OPENMP_SUPPORTED = check_openmp_support()
    ...
    config.ext_modules = cythonize(
        ...
        nthreads=n_jobs,
        compile_time_env={
            'sdeconv_OPENMP_PARALLELISM_ENABLED': sdeconv._OPENMP_SUPPORTED},
