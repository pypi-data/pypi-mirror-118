...
# This function is to get the positions in given arry
                ...
                for row in csv_reader:
                    if row_count == 0:
                        ...
                        if experience_data and first_value_record:
                            ...
                            time_value = (float(row[t_pos]) * 1000) - first_time_value
                            y_value = float(row[y_pos])
...
def get_value_from_array(data_arr_input, low_li, up_li):
    ...
    if length_array == 1:
        ...
        for d in new_arr:
            total_time += d[0]
            total_value += d[1]
        avg_time = total_time / len(new_arr)
        avg_value = total_value / len(new_arr)
    ...
    if draw_able and data:
        ...
        for info in series_array:
            ...
            if parent_index is not None:
                ...
                if str(display_type).lower() == "pen":
                    ...
                    number_of_time_interval = math.ceil(max_video_length / time_interval_for_data)
                    lower_range = 0
                    upper_range = round((lower_range + time_interval_for_data), 2) * 10
                    width_check = int(t_w) * 1000
                    box_data_array = []
                    for i in range(1, (number_of_time_interval + 1), 1):
                        raw_data_array = []
        ...
        for info in series_array:
            ...
            if parent_index is not None:
                ...
                t_name = info["t_label"]
                y_name = info["y_label"]
                y_name_x = info["y_label_x"]
                y_name_y = info["y_label_y"]
                ...
                y_label_x_input = axes_pos_x + y_name_x
                y_label_y_input = axes_pos_y - int(axes_pos_h / 2) + y_name_y
                                    ...
                                    if type(num[1]) == str:
                                        ...
                                        if not value_to_draw:
                                            ...
                                            if pen_draw:
                                                ...
                                                if need_to_draw_pointer and round(num[0], 0) >= round(check_timer, 0):
                                                    draw_after = True
                                                    pointer_x = end_x
                                                    pointer_y = end_y
                                                start_x = end_x
