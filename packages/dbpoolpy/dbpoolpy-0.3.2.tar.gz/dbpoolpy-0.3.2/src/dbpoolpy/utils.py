    ...
    def _(*args, **kwargs):
        ...
        try:
            ...
            print(
                ...
                conn.pool.max_conn, conn._transaction,
            ...
            if settings.DEBUG:
                ...
                if settings.POOL_TYPE == POOLTYPE.SIMPLE:
                    print(
                        ...
                        self._name, dbcf.get('user', ''),
                        dbcf.get('host', ''), dbcf.get('port', 0),
                        dbcf.get('database', ''),
                        len(self.pool._idle_cache),
                        len(self.pool._idle_using),
                        self.pool._maxconnections, 1 if self._transaction else 0,
                        int((endtm - starttm) * 1000000),
                        str(ret), num,
                        sql, err)
                else:
                    print(
                        ...
                        getattr(self, '_server_id', '0'), getattr(self, '_conn_id', 0) % 10000,
