...
class Pager:
    ...
    def todict(self):
        '''返回pagedata数据转换为字典'''
        self.split(True)
        #print('data:%s', self.pagedata.data)
    ...
    def range(self, maxlen=10):
        ...
        ret = range(max(self.page - maxlen, 1),
                    min(self.page + maxlen, pagecount) + 1)
        #print("range:", ret)
        return ret
...
class PageDataDB (PageDataBase):
    ...
    def load(self, cur, pagesize, isdict=False):
        ...
        print('PageDataDB load sql:%s', sql)
    ...
    def count(self, pagesize):
        ...
        #print("PageDataDB count sql:%s", self.count_sql)
        ret = self.db.query(self.count_sql)
        row = ret[0]
        self.records = int(row['count'])
        print("PageDataDB count:%s", self.records)
