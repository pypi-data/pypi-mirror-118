...
class unsupervisedInterface:
    def __init__(self,inifile):
        ...
        # performDR = LabelFrame(tab3, text="3.Perform Dimensionality Reduction", font=("Helvetica", 12, 'bold'), padx=10,
        #                        pady=10, fg='black')
        # self.importFeatureDataset = FileSelect(performDR, "Import feature dataset .pkl")
        # self.algorithm = DropDownMenu(performDR,'Dimensionality Algorithm',drOptions,'15')
        # self.algorithm.setChoices(drOptions[0])
        #
        # self.UMAPhyperparameters = LabelFrame(performDR, text="UMAP Hyperparameters", padx=5, pady=5)
        # self.distance1 = EntryBox(self.UMAPhyperparameters, "Distance", '10')
        # self.neighbors1 = EntryBox(self.UMAPhyperparameters, "Neighbors", '10')
        # self.spread1 = EntryBox(self.UMAPhyperparameters, "Spread", '10')
        # self.dimensionsUMAP = EntryBox(self.UMAPhyperparameters, "Dimensions", '10')
        #
        # btnSaveDR = Button(performDR, text="Save dimensionality reduction .npy", command=self.perform_DR)
        # btnVisualizeDR = Button(performDR, text="Save visualizations",
        #                         command=sampleButton)  # change name to update with algorithm selection
    ...
    def unsupervisedCreate(self):
        self.unsupervisedfolderpath = os.path.join(os.path.dirname(self.configini),'unsupervised')
    ...
    def generate_dataset(self):
        filesFolder = self.importDatasets.folder_path
        features2removeFile = self.importFeatureRemove.file_path
        classifierName = self.classifier.entry_get
        outputPath = os.path.join(self.unsupervisedFolder, r'create_dataset')
    ...
    def perform_DR(self):
        #targetGroup = self.targetGroupEntry.entry_get  # cluster RI males with target group, based on groupings from previous script, user specifies
        file = self.importFeatureDataset.file_path
        outputDirectory = os.path.join(self.unsupervisedFolder,'dimensionality_reduction')

        algorithm1 = self.clicked.get()
        print(algorithm1)
