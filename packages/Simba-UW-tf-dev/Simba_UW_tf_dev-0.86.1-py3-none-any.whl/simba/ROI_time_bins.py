...
def roi_time_bins(inifile, inputcsv, binLength=int):
    ...
    for filecounter, i in enumerate(filesFound):
        ...
        try:
            ...
            print(
                'The FPS / pixels per millimeter for ' + CurrentVideoName + ' could not be found in the project/logs/video_info.csv file, or multiple entries for ' + CurrentVideoName + ' exist in your  project/logs/video_info.csv file. Make sure each video in your project is represented once in your project/logs/video_info.csv file')
        ...
        for bincounter, current_bin in enumerate(currListDf[:setBins]):
                ...
                for rectangle in range(noRectangles):
                    ...
                    for bodyparts in range(len(currentPoints)):
                        if ((((topLeftX - 10) <= currentPoints[bodyparts][0] <= (bottomRightX + 10)) and (
                            ...
                            rectangleTimes[rectangle][bodyparts] = round(
                                (rectangleTimes[rectangle][bodyparts] + (1 / currFps)), 2)
                ...
                for polygon in range(noPolygons):
                    ...
                    for bodyparts in range(len(currentPoints)):
                        ...
                        if (polyGonStatus == True) and (current_probability_list[bodyparts] > probability_threshold):
                            polygonTime[polygon][bodyparts] = round((polygonTime[polygon][bodyparts] + (1 / currFps)),
                                                                    2)
            ...
            for value, rectangle_counter in enumerate(zip(rectangleTimes, rectangleEntries)):
                for animal in range(len(multiAnimalIDList)):
                    out_row_time.append([CurrentVideoName, bincounter, start_time, end_time, multiAnimalIDList[animal],
                                         Rectangles.loc[value]['Name'], rectangleTimes[value][animal]])
                    out_row_entries.append(
                        [CurrentVideoName, bincounter, start_time, end_time, multiAnimalIDList[animal],
                         Rectangles.loc[value]['Name'], rectangleEntries[value][animal]])
