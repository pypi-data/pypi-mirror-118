...
class PESGM(torch.optim.Optimizer):
    '''
    Momentum version for PESG (still in progress)
    Reference:
        https://arxiv.org/abs/2012.03173
        https://arxiv.org/abs/2006.06889 
    '''
    def __init__(self, model, a=None, b=None, alpha=None, imratio=0.1, margin=1.0, lr=0.1, gamma=500, momentum=0, clip_value=1.0, weight_decay=1e-5, **kwargs):
      ...
      # TODO! 
        ...
        super(PESGM, self).__init__(self.params, self.defaults)
    ...
    def update_regularizer(self, decay_factor=None):
        ...
        for i, param in enumerate(self.model_ref):
            self.model_ref[i].data = self.model_acc[i].data/self.T
        for i, param in enumerate(self.model_acc):
            self.model_acc[i].data = torch.zeros(param.shape, dtype=torch.float32, requires_grad=False).to(self.device)
        self.T = 0
