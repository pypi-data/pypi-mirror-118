...
class CaseSensitiveDict(dict):
    ...
    def load(self, postman_enviroment_file_path):
        with open(postman_enviroment_file_path,
                  encoding='utf8') as postman_enviroment_file:
...
class PostPython:
    def __init__(self, postman_collection_file_path):
        with open(postman_collection_file_path,
                  encoding='utf8') as postman_collection_file:
        ...
        if item in self.__folders:
            ...
            if len(similar_folders) > 0:
                ...
                raise AttributeError(
                    '%s folder does not exist in Postman collection.\n'
...
class PostCollection:
    ...
    def __getattr__(self, item):
        if item in self.__requests:
            ...
            similar_requests = difflib.get_close_matches(item,
                                                         post_requests,
                                                         cutoff=0.0)
            if len(similar_requests) > 0:
                similar = similar_requests[0]
                raise AttributeError(
                    '%s request does not exist in %s folder.\n'
                    'Did you mean %s' % (item, self.name, similar))
            else:
                raise AttributeError(
                    '%s request does not exist in %s folder.\n'
                    'Your choices are: %s' %
                    (item, self.name, ", ".join(post_requests)))
...
class PostRequest:
    def __init__(self, post_python, data):
        ...
        if 'body' in data and data['body']['mode'] == 'raw' and 'raw' in data[
                'body']:
            self.request_kwargs['json'] = extract_dict_from_raw_mode_data(
                data['body']['raw'])
            if not self.request_kwargs['json']:
                self.request_kwargs['data'] = data['body']['raw']
        if 'body' in data and data['body'][
                'mode'] == 'formdata' and 'formdata' in data['body']:
            self.request_kwargs['data'], self.request_kwargs[
                'files'] = extract_dict_from_formdata_mode_data(
                    data['body']['formdata'])
