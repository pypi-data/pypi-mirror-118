...
def get_kafka_secrets(
    ...
    if parsed_uri.scheme == "dbfs":
        ...
        s3 = boto3.client("s3")
        with tempfile.NamedTemporaryFile(suffix="keystore.jks", delete=False) as f:
            s3.download_fileobj(parsed_uri.netloc, parsed_uri.path.lstrip("/"), f)
            local_keystore_loc = f.name
