...
def command_list():
    ...
    if materializable:
        printer.safe_print("Automatic Materialization Enabled:")
    ...
    if nonmaterializable:
        printer.safe_print("Automatic Materialization Disabled:")
...
def command_show():
    ...
    is_enabled_str = "Enabled" if workspace.capabilities.materializable else "Disabled"
    printer.safe_print(f"{workspace_name} (Automatic Materialization {is_enabled_str})")
...
def command_create(args):
    ...
    # create
    create_workspace(args.workspace, args.automatic_materialization_enabled)
...
def command_delete(args):
    ...
    is_materializable = is_materializable_workspace(args.workspace)
...
def build_parser(root_parser):
    ...
    for c in COMMANDS:
        ...
        if c.command == "create":
            p.add_argument(
                "--automatic-materialization-enabled", action="store_true", help="enable automatic materialization"
            )
        if c.command == "delete":
