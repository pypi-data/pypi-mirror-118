...
class DataConnection(BaseDataConnection):
    ...
    def read_indices(self, **kwargs):
        training_details = self._wml_client.training.get_details(self.auto_pipeline_params['run_id'])
        # TODO indices location should be read from response, than constructed
        try:
            holdout_data_indices_location = training_details['entity']['status']['metrics'][0]['context']['intermediate_model']['holdout_data_indices_location']

        except KeyError:
            holdout_data_indices_location = \
            training_details['entity']['status']['metrics'][0]['context']['intermediate_model']['schema_location']
            holdout_data_indices_location = '/'.join(
                holdout_data_indices_location.split('/')[:-3]) + '/holdout_data_indices.tar.gz'
        ...
        if self.type == DataConnectionTypes.S3:
            ...
            try:
                data = self._download_indices_from_cos(cos_client=cos_client,
                                                       location_path=holdout_data_indices_location)
            ...
            try:
                ...
                data_location = self.location.to_dict().copy()
                data_location.update({'type': self.type, 'path': holdout_data_indices_location})
            ...
            data = self._download_indices_from_file_system(location_path=holdout_data_indices_location)
            ...
            if self._check_if_connection_asset_is_s3():
                ...
                try:
                    ...
                    flight_service = FlightService(
                        wml_client=self._wml_client,
                        params=self.auto_pipeline_params,
                        data_location={'location': holdout_data_indices_location,
                                       'type': self.type,
    ...
    def read(self,
        ...
        if with_holdout_split:
            try:
                ...
                print("\"with_holdout_split\" option is deprecated.")
                warn("\"with_holdout_split\" option is deprecated.")
        ...
        # note: this code is deprecated, user information isa provided at the beginning.
        if with_holdout_split:
            ...
            is_classification = self.auto_pipeline_params['prediction_type'] == PredictionType.BINARY \
            ...
            if indices is None:
                x, x_holdout, y, y_holdout = train_test_split(
                    ...
                    random_state=33,
                    stratify=data[self.auto_pipeline_params['prediction_column']].values if is_classification else None)
            else:
                x_holdout = data.iloc[indices.values.flatten()].drop(self.auto_pipeline_params['prediction_column'],
                                                                     axis=1)
                y_holdout = data.iloc[indices.values.flatten()][self.auto_pipeline_params['prediction_column']].values
                x = data.loc[set(data.index) - set(indices.values.flatten())].drop(
                    self.auto_pipeline_params['prediction_column'], axis=1)
                y = data.loc[set(data.index) - set(indices.values.flatten())][
                    self.auto_pipeline_params['prediction_column']].values
