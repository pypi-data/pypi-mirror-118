...
def get_user_config():
    """Retrieves the user config from an env variable or path"""
    return (
        os.environ.get("LANCET_TEST_USER_CONFIG") or
        os.path.expanduser(os.path.join("~", LOCAL_CONFIG))
    )


def get_project_config():
    """Retrieves the project config from an env variable or path"""
    return (
        os.environ.get("LANCET_TEST_PROJECT_CONFIG") or
        os.path.join(os.path.realpath("."), LOCAL_CONFIG)
    )
...
def load_config(path=None, defaults=None):
    ...
    if defaults is None:
        defaults = [DEFAULT_CONFIG, SYSTEM_CONFIG, get_user_config()]
