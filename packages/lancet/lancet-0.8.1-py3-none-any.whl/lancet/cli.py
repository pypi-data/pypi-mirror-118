...
IGNORED_EXCEPTIONS = set([bdb.BdbQuit])
    ...
    def resolve_command(self, ctx, args):
        ...
        if cmd_name in self.get_configured_aliases():
            if cmd_name in self.list_commands(ctx):
                ...
                click.secho(
                    '"{}" references an existing command. I am '
...
def main(ctx, debug):
    ...
    ctx.obj = Lancet(
        config, integration_helper, call_on_close=ctx.call_on_close
    )
    ...
    if sentry_dsn and not debug:
        if not sentry_sdk:
            click.secho(
                "You provided a Sentry DSN but the Sentry SDK is "
            ...
            try:
                ...
                click.secho(
                    'Please provide a valid sentry_dsn value in "{}"'.format(project_config),
                    fg="yellow",
                    bold=True,
                )
            else:
