...
class Control:
    ...
    mass_atom : dict = None, CELL : list = None, dtype = np.float32):
        ...
        * The Last one is CELL, same like mass_atom the XYZ files don't have the values for the box sizes but you may difine them here.  
        ...
        M_array shows that this sytem has four different atoms, for example (C, N, H, Cl) split in two categories given by keyword mol_type
    ...
    dtype = np.float32, keytrj : int = 0):
        ...
        * trajec_num: used * input_name : is your lammps input name but warning don't forget to put the lammps extentions [.lammps or .LAMMPS] of file at the end of your input file name.
        It means that if your input file name is <<coordinates>> then you may write <<coordinates.lammps or coordinates.LAMMPS>>.to specify how many trajectories does your input file has. 
        * mass_atom: is contening atoms names, atoms id and theirs masses. as you know the must LAMMPS outputs files don't contain the atomic masses, nor even charges,
        that why in this code you've the possibility to add yourself these values. If it doesn't contain any values the default 
        values will be 1. for all atoms 
        * M_array: is a kernel used to select atoms properly in your input file, this keyword specifies how would you like 
        to select and classify atoms. 
        * nsit_M: is different from the nsit keyword used in Classical_MD, the structure is different, you use this keyword to specify how many atoms does 
        your system has per molecules. 
        * keytrj: used to specify what informations does your LAMMPS output file has. 
        ...
        * dtype: used to specify the max float
        * multi_treatment: used to read several files at the same time but doesn't work yet for this version.
        * CELL: used to define the default values of the simulations box.
        ...
        M_array shows that this sytem has four different atoms, for example (C, N, H, Cl) split in two categories given by keyword mol_type
        and each lists corresponding to each atom, it means that:
        ...
        >>> let's take the case of carbon
        >>>'C' = symbol 
        >>>[1, 12.011] = 1 is index of the carbon in the lammps ouput and 12.011 is its atomic mass 
                ...
                for val in mass_list:
                    if type(mass_atom[val]) != type(t1):
                        ...
                        if len(mass_atom[val]) != 2:
                            ...
                            if type(id1) != type(t3) and type(id2) != type(t2):
                                @decor
                                def print_error():
                                    print('InputError 😊')
                                    print(mass_atom[val], 'not in ', [type(t3), type(t2)] )
                                    print("Correct the mistake and try again")
                                exit()
        else:
            @decor
    ...
    label_encoder : dict = None):
        ...
        """
        You can use this function to get the names of all atoms in yours molecular systems. 
        If you want, you may use this function to make encoding by activating the keyword << encoding >> on True,
        then use the label_encoder to specify how would you like to do this. 
        
        WARNING: if you want to compute the ElectronDensity or ElectronDensityNeutral the encoding characters should be identical with 
        the elements of the periodic table, else if the code couldn't run. 
     
        It means that, if in your molecular system the carbons are [CR, C1, CW] when you're making the encoding, all these values become [C]
    ...
    def TimeSimulation(self, timestep : float = 1e-3, tri : any = False, step : int = None):
        ...
        timestep: identical with the timestep used for yours simulations 
        tri: is a boolean type, generally use to make a selection of trajectories 
        step: is an integer, it specifies the range between two timesteps when the keyword <<tri>> is defined on <True>>
