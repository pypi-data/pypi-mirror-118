...
DEFAULT_INPUT_FILE = 'pyproject.template.toml'
DEFAULT_OUTPUT_FILE = 'pyproject.toml'
DEFAULT_VERSION_VARIABLE = 'VERSION'
...
def _text_replace(lines, variables):
    for variable, value in variables.items():
        lines = lines.replace(f'${{{variable}}}', value)
    return lines


def tag_file(args, parser: ScmParser = None) -> str:
    parser = parser if parser is not None else parser_factory()
    version = parser.version(pre_commit=args.pre_commit)
    variables = {args.version: version}
    if args.version_type is not None:
        variables[args.version_type] = parser.version_type().name
    with open(args.input.name, 'r') as input:
        lines = _text_replace(input.read(), variables)
    with open(args.output.name, 'w') as output:
        output.write(lines)
    return f'tagged {args.output.name} from {args.input.name}'
