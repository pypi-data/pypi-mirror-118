...
class PortfolioOptimization:
    ...
    def __init__(self, stats_list: List[StrategyBacktestStats], shown_corr_plot=False, **kwargs):
        ...
        self.available_keys = {
            ...
            if SymbolsInfo.get_or_create_curr_symbols(_.symbols_info.symbols).symbols == _.symbols_info.symbols
        }
        # 目前仅考虑 CTA 的情况
    ...
    def _fit_by_group_keys(self, key_weight_dic, group: InstrumentClassificationEnum, key_set: set):
        if self.goal == PortfolioOptimizeGoalEnum.equal_mdd:
            indicator = self._kwargs.get('equal_mdd_indicator', "max_drawdown")
            key_mdd_dic = {
            ...
            _key_weight_dic = {key: weights / tot_weight * self.group_weight_dic[group]
                               for key, weights in _key_weight_dic.items()}
            key_weight_dic.update(_key_weight_dic)
        elif self.goal == PortfolioOptimizeGoalEnum.black_litterman:
            balance_df = pd.DataFrame({
                key: df['balance']
                for key, df in self.key_balance_df_dic.items() if key in key_set and df is not None}).ffill().dropna()
            if self.start_date:
                balance_df = balance_df[balance_df.index >= pd.to_datetime(self.start_date)]
            if self.end_date:
                balance_df = balance_df[balance_df.index <= pd.to_datetime(self.end_date)]

            cov_matrix = risk_models.sample_cov(balance_df)
            mcaps = balance_df.iloc[-1, :]
            delta = market_implied_risk_aversion(balance_df, risk_free_rate=0).to_numpy()
            # delta = None
            ...
            ef = EfficientFrontier(
                mu, cov_matrix,
                weight_bounds=self._kwargs.get('weight_bounds', (0, 0.3) if count > 4 else (0, 1)),
                solver=self.solver)
            ...
            _key_weight_dic = {
                ...
                for key, weight in (raw_weights if raw_weights else ef.clean_weights()).items()
            }
            ...
            if self.end_date:
                balance_df = balance_df[balance_df.index <= pd.to_datetime(self.end_date)]
            # 将 balance_df 其实点回归 capital 值
            for key, balance_s in list(balance_df.items()):
                stats: StrategyBacktestStats = self.key_stats_dic[key]
                balance_s -= balance_s.iloc[0] + stats.capital
                balance_df[key] = balance_s
            ...
            ef = EfficientFrontier(
                ...
                weight_bounds=self._kwargs.get('weight_bounds', (0, 0.3) if count > 4 else (0, 1)),
                solver=self.solver)
            raw_weights = getattr(ef, self.goal.value)()
        ...
        missing_key_set = key_set - set(_key_weight_dic.keys())
...
class PortfolioOptimizationGroupFitting(PortfolioOptimization):
    ...
    def fit(self):
        ...
        # msg = "\n".join(
        #     [f"{group_name:50s} {fitting_key_group.weights * 100:.2f}%"
        #      for group_name, fitting_key_group in group_fitting_key_group_dic.items()])
        df = pd.DataFrame({
            ...
            for group_name, fitting_key_group in group_fitting_key_group_dic.items()},
            index=['weights(%)', "stg count", 'goal']).T
        self.logger.info(f"各分组权重如下：\n{df.to_markdown()}\n")
        ...
        for num, (key, weights) in enumerate(self.key_weight_dic.items(), start=1):
            vt_symbol = self.key_vt_symbol_dic[key]
            # self.logger.info(f"{num:2d}) {vt_symbol:11s} {key:130s} --> {weights * 100:.2f}%")
            msg_dic_list.append({
                "group_name": self.key_group_dic[key],
                "vt_symbol": vt_symbol,
                "key": key,
        ...
        for num, vt_symbol in enumerate(vt_symbols, start=1):
            ...
            msg_dic_list.append({
                ...
                "weights(%)": np.round(tot_weight * 100, 2),
            })
    ...
    def _fit_by_group_keys(self, fitting_keys_group: FittingKeysGroup):
        ...
        key_weight_dic = {}
        if len(fitting_keys_group.key_set) == 1:
            key = list(fitting_keys_group.key_set)[0]
            key_weight_dic[key] = fitting_keys_group.weights
            self.logger.warning(f"{fitting_keys_group.name} 仅有一个策略 {key} 权重 {fitting_keys_group.weights * 100}%")
            return key_weight_dic

        if fitting_keys_group.goal == PortfolioOptimizeGoalEnum.equal_mdd:
            indicator = fitting_keys_group.kwargs.get('equal_mdd_indicator', "max_drawdown")
            key_mdd_dic = {
                key: _.indicator_dic.get("daily_stats_dic", {}).get(indicator, None)
                for key, _ in self.key_stats_dic.items()
                if (key in key_set and _.indicator_dic and
            ...
            _key_weight_dic = {key: benchmark_capital / np.abs(v) for key, v in key_mdd_dic.items()}
            tot_weight = sum(_key_weight_dic.values())
            _key_weight_dic = {key: weights / tot_weight * fitting_keys_group.weights
                               for key, weights in _key_weight_dic.items()}
            key_weight_dic.update(_key_weight_dic)
        elif fitting_keys_group.goal == PortfolioOptimizeGoalEnum.black_litterman:
            balance_df = pd.DataFrame({
                key: df['balance']
            ...
            ef = EfficientFrontier(
                mu, cov_matrix,
                weight_bounds=fitting_keys_group.kwargs.get('weight_bounds', (0, 0.3) if count > 4 else (0, 1)),
                solver=fitting_keys_group.solver)
            absolute_views = ef.min_volatility()
            bl = BlackLittermanModel(
                cov_matrix, absolute_views=absolute_views,
                # pi='equal',
            ...
            # clean_weights = bl.clean_weights()
            _key_weight_dic = {
                key: weight * fitting_keys_group.weights
                # for key, weight in ef.clean_weights().items()
                for key, weight in (raw_weights if raw_weights else ef.clean_weights()).items()
            }
            for num, (key, weight) in enumerate(raw_weights.items(), start=1):
            ...
            ef = EfficientFrontier(
                ...
                solver=fitting_keys_group.solver)
            try:
                raw_weights = getattr(ef, fitting_keys_group.goal.value)()
            except OptimizationError as exp:
                self.logger.error(f"{fitting_keys_group.name}[{len(fitting_keys_group.key_set)}] "
                                  f"{fitting_keys_group.goal} solver={fitting_keys_group.solver} 优化异常")
                raise exp from exp
            # ef.save_weights_to_file("weights.csv")  # saves to file
            # ef.portfolio_performance(verbose=True)  #
            _key_weight_dic = {
            ...
            key_weight_dic.update(_key_weight_dic)
            self.logger.info(f"{group_name} has {len(raw_weights)} keys")
            for num, (key, weight) in enumerate(raw_weights.items(), start=1):
                self.logger.info(f"{num:2d}) {self.key_vt_symbol_dic[key]:11s} {key:130s} --> {weight * 100:.2f}%")
...
def portfolio_optimization_by_account(
    ...
    if 'fit_by_group' in kwargs:
        po = PortfolioOptimization.build_by_account(
            user_name, broker_id, shown_corr_plot=shown_corr_plot,
        )
        # po.set_goal(
