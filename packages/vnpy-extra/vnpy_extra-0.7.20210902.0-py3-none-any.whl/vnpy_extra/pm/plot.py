...
def plot_with_weights(
        ...
        vt_symbol_market_value_dic: Optional[dict] = None, vt_symbol_group_dic: Optional[dict] = None,
):
...
def plot_with_base_position(
    ...
    plot_with_weights(
        stats_list=stats_list, weight_dic=base_position_dic, legend=legend, capital=capital,
        key_vt_symbol_dic=key_vt_symbol_dic,
    )
...
def plot_by_account(user_name=None, broker_id=None, description=None, capital=3_000_000):
    stats_list = AccountStrategyMapping.get_stats_by_account(
        user_name=user_name, broker_id=broker_id, available_only=True)

    plot_with_base_position(stats_list, legend=False, capital=capital)


def _test_plot_by_account():
    plot_by_account(user_name="11859087", broker_id="95533")
