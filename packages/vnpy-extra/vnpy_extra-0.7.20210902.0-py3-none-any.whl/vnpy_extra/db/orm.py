...
class AccountStrategyMapping(Model):
    ...
    def delete_if_no_position(*, stg_info_id, symbols_info_id, id_name, short_name,
        ...
        logger.info(f"{del_count} data of {user_name}[{broker_id}] {short_name} on "
                    f"stg_info_id={stg_info_id}, symbols_info_id={symbols_info_id}, id_name={id_name} has been deleted")
        return del_count
...
class StrategyBacktestStats(Model):
    ...
    def get_stats_key_str(self) -> str:
        return f'{self.stg_info.strategy_class_name}[{self.stg_info_id}]_' \
               f'{self.symbols_info.symbols}[{self.symbols_info_id}]_' \
               f'{self.id_name}_{self.cross_limit_method}'
    ...
    def set_base_position(
        ...
        if stop_opening_pos_if_base_position_zero and base_position == 0:
            ...
            logger.warning(f"{self.short_name} {strategy_settings[BASE_POSITION]} {STOP_OPENING_POS_PARAM} == 1")
        else:
            strategy_settings[BASE_POSITION] = int(base_position)
        ...
        update_count = AccountStrategyMapping.update(
            ...
            AccountStrategyMapping.short_name == self.short_name,
            AccountStrategyMapping.user_name == user_name,
            AccountStrategyMapping.broker_id == broker_id,
        ...
        #         strategy_settings=strategy_settings
        #     ).where(
        #         AccountStrategyMapping.id_name == self.id_name,
        #         AccountStrategyMapping.user_name == user_name,
        #         AccountStrategyMapping.broker_id == broker_id,
        #         AccountStrategyMapping.stg_info == self.stg_info,
        #         AccountStrategyMapping.symbols_info == self.symbols_info,
        ...
        # logger.info("更新策略 base_position %d 条: %s : %s", update_count, self.short_name, strategy_settings)
        logger.info(f"更新策略 {BASE_POSITION} {self.short_name} : {strategy_settings}")
