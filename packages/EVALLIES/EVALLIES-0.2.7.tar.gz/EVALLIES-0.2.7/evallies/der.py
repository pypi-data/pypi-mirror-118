...
This module provides the methods to compute all kinds of Diarization Error Rate.  The datatypes used are:
    * Reference and hypothesis.  A named tuple with three fields, each an array, with a record being the nth element of each array:
        ** speaker: array of speaker names
        ** start_time: array of times at which the speaker started talking
        ** end_time: array of times at which the speaker stopped talking
    * UEM. A named tuple with two fields each an array, with a record being the nth element of each array:
        ** start_time: array of starting times for the sound range to work on
        ** start_time: array of ending times for the sound range to work on
    * Speaker map: A dict with reference_speker_name -> hypothesis speaker name

All the DER methods take reference(s), hypothese(s) and uem(s) as fixed parameters, and have as optinal parameters:
    * fixed_map: A speaker map with a possibly partial forced association between reference and hypothesis speakers (default empty)
    * collar: The collar in seconds around the reference speakers start/stop times (default 0)
    * use_nist_collar_definition: Bool selecting the LNE or NIST version of collar application (default True)

The collar is the expected imprecision around the reference speaker start and end times.  It's usually set at 0.25s. There are two main methodologies to take it into account:
    * LNE: When a reference and an hypothesis speaker are mapped together, an error less or equal to the collar size for the boundary of a segment is not counted as error.  Evaluation on the other speakers are not influenced.
    * NIST: The period of time [-collar, +collar] around each frontier of the reference speakers is removed form the evaluation

Note that the NIST method can remove a lot of time in case of debates with a lot of back-and-forth and speech superposition, hence the alternative method.

The DER methods available are:
    * der_single(ref, hyp, uem): Computes the DER for one ref/hyp/uem triplet
    * der_multi(array of ref, array of hyp, array of uem): Computes the DER for multiple ref/hyp/uem triplets independently, accumulate the results
    * der_cross(array of ref, array of hyp, array of uem): Computes the DER for multiple ref/hyp/uem triplets simultaneously, with one common speaker mapping, accumulate the results

Each method returns:
    * der
    * total missed time
    * total false alarm time
    * total confusion time
    * total error time (e.g. missed + false alarm + confusion)
    * total speech time (der = 100 * error time / speech time)
    * speaker map (array of speaker maps in the der_multi case)
...
def drop_collar_transition_from_uem(auem, aref, collar):
    uemi = namedtuple('uemi', ['start_time', 'end_time'])
    nuem = []
    for i in range(len(auem)):
        rtlist = []
        for f in aref[i].start_time:
            rtlist.append(f)
        for f in aref[i].end_time:
            rtlist.append(f)
        rtlist = sorted(rtlist)
        ouem = auem[i]
        uem = uemi(start_time = [], end_time = [])
        for j in range(len(ouem.start_time)):
            st = ouem.start_time[j]
            en = ouem.end_time[j]
            while True:
                lim = en
                for f in rtlist:
                    ...
                    if f+collar > st:
                        st = f+collar
                if st < lim:
                    uem.start_time.append(st)
                    uem.end_time.append(lim)
                    st = lim
                else:
                    break
        nuem.append(uem)
...
def der_cross(aref, ahyp, auem, fixed_map = {}, collar = 0.0, use_nist_collar_definition = True):
    ...
    if use_nist_collar_definition and collar != 0:
        auem = drop_collar_transition_from_uem(auem, aref, collar)
    ...
    if use_nist_collar_definition or collar == 0:
        aref_frontiers_collar = aref_frontiers
    else:
    ...
    for i1 in range(0, opt_size):
        i = map1[i1]
        j = map2[i1]
        if i < solved_ref_count and j < solved_hyp_count:
            r = solved_ref_back[i]
            h = solved_hyp_back[j]
            if (de[r, h] > 0 and tc[r, h] > 0):
                map_rh[r] = h
                map_hr[h] = r
    ...
    return 100. * error / totaltime, fa, miss, conf, error, totaltime, newspkmap

def der_single(ref, hyp, uem, fixed_map = {}, collar = 0.0, use_nist_collar_definition = True):
    return der_cross([ref], [hyp], [uem], fixed_map, collar, use_nist_collar_definition)

def der_multi(aref, ahyp, auem, fixed_map = {}, collar = 0.0, use_nist_collar_definition = True):
    tot_fa = 0.
    tot_miss = 0.
    tot_conf = 0.
    tot_error = 0.
    tot_time = 0.
    maps = []
    assert(len(aref) == len(ahyp))
    assert(len(aref) == len(auem))

    for i in range(len(aref)):
        _, fa, miss, conf, error, totaltime, spkmap = der_single(aref[i], ahyp[i], auem[i], fixed_map, collar, use_nist_collar_definition)
        tot_fa += fa
        tot_miss += miss
        tot_conf += conf
        tot_error += error
        tot_time += totaltime
        maps.append(spkmap)
    return 100. * tot_error / tot_time, tot_fa, tot_miss, tot_conf, tot_error, tot_time, maps
