...
def test_installation():
    im1 = Image.open("../../example/test_im1.tif")
    im2 = Image.open("../../example/test_im2.tif")
...
def rename_config_ini(old_ini_name, new_ini_name):
    ...
    if os.path.isfile("../../config/" + old_ini_name):
        shutil.move("../../config/" + old_ini_name,"../../config/" + new_ini_name)
    return True
...
def update_config_ini_from_exisiting_ini(to_ini, from_ini):
    ...
    foc_params_from.read("../../config/" + from_ini)
    ...
    with open("../../config/" + to_ini, 'w') as configfile:
        foc_params_to.write(configfile)

def list_all_config_ini():
    all_inis = [os.path.basename(cini) for cini in glob.glob("../../config/*.ini")]
    return all_inis
