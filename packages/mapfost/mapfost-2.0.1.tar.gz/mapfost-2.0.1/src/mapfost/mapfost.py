import numpy as np
import src.mapfost.costLib as ctl
import src.mapfost.mtfLib as mtl
import src.mapfost.imageLib as iml
...
def est_aberr(test_ims, pix_size_um = None, crop_ref=None, config_ini="default.ini"):
    ...
    foc_params.read(cf.__path__[0] +"/" + config_ini )
...
def test_installation():
    im1 = Image.open(ex.__path__[0] + "/test_im1.tif")
    im2 = Image.open(ex.__path__[0] + "/test_im2.tif")
...
def rename_config_ini(old_ini_name, new_ini_name):
    ...
    if os.path.isfile(cf.__path__[0] +"/"+ old_ini_name):
        shutil.move(cf.__path__[0] +"/" + old_ini_name,cf.__path__[0] +"/"+ new_ini_name)
    return True
...
def update_config_ini_from_exisiting_ini(to_ini, from_ini):
    ...
    foc_params_from.read(cf.__path__[0] +"/"+ from_ini)
    ...
    with open(cf.__path__[0] +"/" + to_ini, 'w') as configfile:
        foc_params_to.write(configfile)

def list_all_config_ini():
    all_inis = [os.path.basename(cini) for cini in glob.glob(cf.__path__[0] +"/" + "*.ini")]
    return all_inis
