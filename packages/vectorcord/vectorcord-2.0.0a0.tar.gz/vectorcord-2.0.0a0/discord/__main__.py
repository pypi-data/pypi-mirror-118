...
def show_version():
    ...
    version_info = vectorcord.version_info
    entries.append('- vectorcord v{0.major}.{0.minor}.{0.micro}-{0.releaselevel}'.format(version_info))
    if version_info.releaselevel != 'final':
        pkg = pkg_resources.get_distribution('vectorcord')
        if pkg:
            entries.append(f'    - vectorcord pkg_resources: v{pkg.version}')
...
_cog_template = '''from vectorcord.ext import commands
import vectorcord
...
def parse_args():
    parser = argparse.ArgumentParser(prog='discord', description='Tools for helping with vectorcord')
