...
def create_fasta(familyname, data_name, superfamily=None):
    ...
    infileext = None
    if not superfamily:
        path = Path(app.config["DATA"]) / familynameapp.log
    else:
...
def do_pgml_run(out_path, err_path, cwd, status, rundir):
    """Convert a Stockholm-format alignment file to FASTA.
...
def queue_pgml(rundir):
    ...
    #
    align_queue = rq.get_queue(app.config["ALIGNMENT_QUEUE"])
    jobpath = Path("/var/rayon/0.1/userdata")
    rundir_path = jobpath/rundir
    status_path = rundir_path/"status.txt"
    align_job = align_queue.enqueue(
        run_subprocess_with_status,
        args=(
            jobpath,
...
def post_run():
    """POST ProteinGraphML run.
    :return: Success text.
    """
    data = dict(request.form)
    if 'email' not in data:
        app.logger.error("email not in calculate data request.")
        abort(400)
    else:
        email = data['email']
    if 'title' not in data:
        app.logger.error("title not in calculate request.")
        abort(400)
    else:
        title = data['title']
    if 'positives' not in request.files:
        app.logger.error("positives not in calculate request.")
        abort(400)
    else:
        positives = request.files['positives'].read().decode('utf-8')
    # Create run directory
    run_id = str(bytes.hex(os.urandom(5)))
    path = Path(app.config["USERDATA"]) / (run_id +"_dir")
    if path.exists() and not path.is_dir():
        app.logger.warning("Removing existing file in data path name.")
        path.unlink()
    if not path.is_dir():
        app.logger.debug("Creating directory %s.", path)
        path.mkdir()
    # put input list in directory
    try:  # parse gene names
        positives_list = positives.split()
        #gene_dict = pd.read_csv()
        positives_len = len(positives_list)
        if positives_len < 1:
            raise ValueError
    except (ValueError, RuntimeError):
        app.logger.error(
            f'Unparseable/empty positives list {positive_list} requested for run {run_id}.')
        abort(406)
    positives_frame = pd.DataFrame({"Symbol": positives_list})
    positives_frame["Label"] = 1
    positives_frame.to_csv(path/"inputs.csv", index=False)
    # put email in directory
    email_regex = re.compile(r"[^@]+@[^@]+\.[^@]+")
    if email_regex.match(email):
        with (path/'email.txt').open('w') as email_fh:
            email_fh.write(f"{email}\n")
    else:
        app.logger.error(
                'Bad email format "{email}" requested for run {run_id}.')
        abort(406)
    # Job title, must be sanitized as it will be a directory name
    if title == "":
        title = "default"
    title = sanitize(title)
    with (path / 'title.txt').open('w') as title_fh:
        title_fh.write(f"{title}\n")
    shutil.chown(path, 'www', 'www')
    #queue_pgml(run_id)
    return Response(f"Run {run_id} with title {title} and {positives_len} submitted.\n", mimetype=TEXT_MIMETYPE)


@app.route("/results/<runid>/results.zip", methods=["GET"])
def get_results(runid):
    ...
    """
    zipfile_path = Path(app.config["USERDATA"]) /(runid +"_dir") / "results.zip"
    if not zipfile_path.exists():
            abort(404)
    return Response(zipfile_path.open('rb').read(), mimetype=ZIP_MIMETYPE)
