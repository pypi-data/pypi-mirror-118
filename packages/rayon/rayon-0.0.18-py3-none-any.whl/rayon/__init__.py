...
def post_run():
    ...
    :return: Success text.
    """
    app.logger.error("here's a request")
    app.logger.error(f"{request}")
    app.logger.error("data")
    app.logger.error(f"{request.data}")
    app.logger.error(f"{request.headers}")
    ...
    if 'positives' not in request.files:
        ...
        positives = request.files['positives'].read()
    ...
    try:  # parse gene names
        positive_list = positives.split()
        #gene_dict = pd.read_csv()
        positives_len = len(positive_list)
        if positive_len < 1:
            raise ValueError
    except (ValueError, RuntimeError):
        app.logger.error(
        ...
        abort(406)
    positives_frame = pd.DataFrame({"Symbol", positive_list})
    ...
    return Response(f"Run {run_id} with title {title} and {positives_len} submitted", mimetype=TEXT_MIMETYPE)
