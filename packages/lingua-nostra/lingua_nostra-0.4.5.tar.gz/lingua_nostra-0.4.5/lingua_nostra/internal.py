...
from lingua_nostra import config
...
def _set_active_langs(langs=None, override_default=True):
    ...
    if not isinstance(langs, list):
        raise(TypeError("lingua_nostra.internal._set_active_langs expects"
                        " 'str' or 'list'"))
...
def load_language(lang):
    """Load `lang` and its functions into memory. Will only import those
       ...
       lingua_nostra.parse loaded, but *not* lingua_nostra.format,
       ...
       `import lingua_nostra.parse`, will only import those functions
    ...
    if not isinstance(lang, str):
        raise TypeError("lingua_nostra.load_language expects 'str' "
                        "(got " + type(lang) + ")")
...
def unload_language(lang):
    """Opposite of load_language()
       ...
       `lingua_nostra.get_active_langs()` to become the default.
...
def __get_primary_lang_code_deprecation_warning(lang=''):
    ...
    Args:
        lang(str, optional): A BCP-47 language code
                              ...
                              `lingua_nostra.get_default_lang()`)
    ...
    if not lang:
        ...
        raise(TypeError("lingua_nostra.get_primary_lang_code() expects"
...
def localized_function(run_own_code_on=[type(None)]):
    ...
    defined in the top-level modules. See lingua_nostra.format or .parse for
    ...
    will locate and call
        ...
        lingua_nostra.lang.parse_es.extract_number_es('uno')
    ...
    def localized_function_decorator(func):
        ...
        def _call_localized_function(func, *args, **kwargs):
            ...
            _module = import_module(".lang." + _module_name +
                                    "_" + lang_code, "lingua_nostra")
            # The nonsense above gets you from lingua_nostra.parse
            # to lingua_nostra.lang.parse_xx
            if _module_name not in _localized_functions.keys():
                raise ModuleNotFoundError("Module lingua_nostra." +
            ...
            # lingua_nostra.parse.extract_datetime_en
...
def populate_localized_function_dict(lf_module, langs=get_active_langs()):
    ...
    Note:
        ...
        `lingua_nostra.internal._localized_functions`,
    ...
    for lang_code in langs:
        ...
        function_names = getattr(import_module("." + lf_module, "lingua_nostra"),
