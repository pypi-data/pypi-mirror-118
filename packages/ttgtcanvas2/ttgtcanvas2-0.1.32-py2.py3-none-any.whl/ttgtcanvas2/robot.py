...
class Robot():
    ...
    def read_message(self, wait_For=1):
    ...
    def put(self):
        ...
        if len(self.last_picked) > 0:
            ...
            if collection is not None and o_name in collection:
                ...
                k = "{},{}".format(self.x,self.y)
                obj_here = self.world_model.objects.get(k, {}).get(o_name, 0) + 1
                self.world_model.add_object(self.x, self.y, o_name, obj_here, self.world.js_call)
    ...
    def take(self, obj_type=None):
        ...
        if self.max_capacity is not None and self.max_capacity <= len(self.last_picked):
            raise RuntimeError(f"Can't Carry more than {self.max_capacity} items")
