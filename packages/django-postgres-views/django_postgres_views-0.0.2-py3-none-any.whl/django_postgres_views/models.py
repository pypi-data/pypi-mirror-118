...
from django_postgres_views.view import create_view, View, MaterializedView
...
class ViewSyncer(object):
    ...
    def run(self, force, update, **options):
        ...
        if loop >= 10:
            logger.warn('django_postgres_views dependencies hit limit. Check if your model dependencies are correct')
        else:
    ...
    def run_backlog(self, models, force, update):
        ...
        for view_cls in models:
            ...
            if skip is True:
                backlog.append(view_cls)
                logger.info('Putting django_postgres_views at back of queue: %s', name)
                continue # Skip
