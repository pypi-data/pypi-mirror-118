...
def interbank_bond_deal(num,option='1'):
    ...
    if option=='2':
        df.sort_values(by=['债券简称'],ascending=[True],inplace=True) 
        optiontxt=texttranslate("Issue date: Early first")
    if option=='3':
        df.sort_values(by=['债券简称'],ascending=[False],inplace=True) 
        optiontxt=texttranslate("Issue date: Late first")
    if option=='4':
    ...
    df.rename(columns={'债券简称':'Bond','成交净价':'Price (Net)','最新收益率':'Current Yield','涨跌':'Change'},inplace=True)
    
    print("\n"+texttranslate("Inter-Bank Bond Market: Deal Price (")+optiontxt+texttranslate(", Top ")+str(num)+')')
    import pandas as pd
    pd.set_option('display.unicode.ambiguous_as_wide', True)
    pd.set_option('display.unicode.east_asian_width', True)
    pd.set_option('display.width', 180) # 设置打印宽度(**重要**)
    ...
    import datetime
    today = datetime.date.today().strftime("%Y-%m-%d")
    footnote1="\n"+texttranslate("Notes: Yield in %, Change in BP.")
    footnote2=" "+texttranslate("Data source: NAFMII, ")+today  
...
def exchange_bond_deal(num,option='1'):
    ...
    其他选项按照默认排列。
    """
    print("Searching internet, it may take time, please wait ...")
    #定义标准输出关闭类
    import os, sys
    class HiddenPrints:
        def __enter__(self):
    ...
    #其他选项均作为默认选项
    if not option in ['1','2','3','4','5','6','7','8','9']: option='1'    
    if option=='1':
    ...
    if option=='8':
        df1.sort_values(by=['trade'],ascending=[False],inplace=True)
        optiontxt=texttranslate("按照成交价从高到低排序")
    if option=='9':
        df1.sort_values(by=['trade'],ascending=[True],inplace=True)
        optiontxt=texttranslate("按照成交价从低到高排序")
    ...
    print("\n***",texttranslate("沪深交易所债券市场现券即时成交价（")+optiontxt+texttranslate(", Top ")+str(num)+texttranslate(") ***"))
    import pandas as pd
    pd.set_option('display.unicode.ambiguous_as_wide', True)
    pd.set_option('display.unicode.east_asian_width', True)
...
def bond_malkiel5(aytm,yper,c,fv=100,mterm=1, \
    ...
    p0=bond_eval(aytm,yper,c,fv,mterm)
    s=pd.Series({'bp':0,'c':c,'Price':p0,'xLabel':str(c*100.00)+'%'})
    df=df.append(s, ignore_index=True)
