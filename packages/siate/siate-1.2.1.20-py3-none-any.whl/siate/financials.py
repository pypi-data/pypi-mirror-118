...
def compare_snapshot(tickers,itemk,datatag=True,tag_offset=0.01,graph=True,axisamp=1.3):
    ...
    if graph:
        ...
        titletxt=texttranslate("企业横向对比: 业绩快照（TTM）")
        import datetime; today=datetime.date.today()
...
def compare_tax(tickers,graph=True,axisamp=1.3):
    ...
    if graph:
        print("...Calculating and drawing graph, please wait ...")
        colname='tax rate%'
        titletxt=texttranslate("企业横向对比: 实际税负")
        import datetime; today=datetime.date.today()
        itemk="实际所得税率%"
        footnote=ectranslate(itemk)+" -->\n"+ \
            texttranslate("数据来源: 雅虎财经, ")+str(today)
...
def compare_dupont(tickerlist,fsdate='latest',scale1 = 10,scale2 = 10,hatchlist=['.', 'o', '\\']):
    ...
    for t in tickerlist:
        ...
        if fsdate == 'latest': 
            try:
                ...
                print("  #Warning(compare_dupont): pandas problem in some accounting items for",t)               
                continue
    ...
    plt.ylabel(texttranslate("杜邦分析分解项目"))
    ...
    if name5types > 1:
        ...
        for i in range(num):
            if linenum % 3 == 2:
                footnote=footnote+'\n'
            footnote=footnote+codetranslate(tickerlist[i])+"："+fsdatelist[i]+"，"+texttranslate(fstypenamelist[i])
            if linenum < num -1:
                footnote=footnote+'；'
            linenum=linenum + 1
    else:
        footnote=footnote+fsdatelist[0]+"，"+texttranslate(fstypenamelist[0])
        
    import datetime; today=datetime.date.today()
    #footnote1=footnote+'\n'+texttranslate("【图示放大比例】")+ectranslate(name1)+'：x'+str(scale1)+'，'+ectranslate(name2)+'：x'+str(scale2)
    footnote1=texttranslate("【图示放大比例】")+ectranslate(name1)+'：x'+str(scale1)+'，'+ectranslate(name2)+'：x'+str(scale2)
    footnote2=footnote1+'\n'+texttranslate("数据来源：雅虎财经，")+str(today)
    plt.xlabel(footnote2)
    
    plt.legend(loc='best')
    plt.title(texttranslate("杜邦分析对比图"))
    ...
    for i in range(num):
        code=df.loc[i,'财报类型']
        if code == '3M': df.loc[i,'财报类型']=texttranslate('季报')
        else: df.loc[i,'财报类型']=texttranslate('年报')
    df.rename(columns={"公司": "Company", "销售净利率": "PM"},inplace=True)
    df.rename(columns={"总资产周转率": "TAT", "权益乘数": "EM"},inplace=True)
    df.rename(columns={"净资产收益率": "ROE", "财报日期": "Date", "财报类型": "Type"},inplace=True)
