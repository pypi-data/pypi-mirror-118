...
def portfolio_cumret(portfolio,today,pastyears=1):
    ...
    if prices is None:
        print("\n  #Error(portfolio_cumret): failed to get stock prices")
    ...
    plotsr.plot(label=texttranslate('投资组合'))
    plt.axhline(y=0,ls=":",c="red")
    plt.title(texttranslate("投资组合: 日收益率的变化趋势"))
    plt.ylabel(texttranslate("日收益率%"))
    plt.xlabel(texttranslate("数据来源: 雅虎财经, ")+str(today))
    plt.legend()
    plt.show()
    #..........................................................................
    ...
    # 计算累积的组合收益，并绘图
    name_list=["Portfolio"]
    titletxt=texttranslate("投资组合: 累计收益率的变化趋势")
    ylabeltxt=texttranslate("累计收益率")
    xlabeltxt=texttranslate("数据来源: 雅虎财经, ")+str(today)
    #由函数去处理百分号放大
    ...
    titletxt=texttranslate("投资组合: 累计收益率的比较")
    cumulative_returns_plot(StockReturns,name_list,titletxt,ylabeltxt,xlabeltxt)
...
def portfolio_corr(pf_info):
    ...
    sns.heatmap(correlation_matrix,annot=True,cmap="YlGnBu",linewidths=0.3,
            annot_kws={"size": 8})
    plt.title(texttranslate("投资组合: 成分股收益率之间的相关系数"))
    plt.ylabel(texttranslate("股票"))
    plt.xlabel(texttranslate("数据来源: 雅虎财经, ")+str(today))
...
def portfolio_es(pf_info,simulation=1000):
    ...
    plt.title(texttranslate("投资组合: 马科维茨可行集"))
    plt.ylabel(texttranslate("预期收益"))
    plt.xlabel(texttranslate("预期风险")+"-->\n\n"+texttranslate("数据来源: 雅虎财经, ")+str(today))
    plt.show()
...
def portfolio_MSR_GMV(es_info,RF=0):
    ...
    plt.title(texttranslate("投资组合: 马科维茨可行集的夏普比率分布"))
    plt.ylabel(texttranslate("预期收益"))
    plt.xlabel(texttranslate("预期风险")+"-->\n\n"+ \
               texttranslate("观察期：")+hstart+" to "+hend+ \
               "\n"+texttranslate("数据来源: 雅虎财经, ")+str(today))     
    plt.show()
    ...
    plt.title(texttranslate("投资组合: MSR点和GMV点的位置"))
    plt.ylabel(texttranslate("预期收益"))
    plt.xlabel(texttranslate("预期风险")+"-->\n\n"+ \
               texttranslate("观察期：")+hstart+" to "+hend+ \
               "\n"+texttranslate("数据来源: 雅虎财经, ")+str(today))    
    ...
    titletxt=texttranslate("投资组合: 累计收益率的比较")
    ylabeltxt=texttranslate("累计收益率")
    xlabeltxt=texttranslate("观察期：")+hstart+" to "+hend+ \
              "\n"+texttranslate("数据来源: 雅虎财经, ")+str(today)      
    cumulative_returns_plot(StockReturns,namelist,titletxt,ylabeltxt,xlabeltxt)    

    #输出MSR投资组合构成比例
    print("\n=====",texttranslate("投资组合的构造方式: 最高夏普比率(MSR)"),"=====")
    print(texttranslate("成分股："),translate_tickerlist(tickerlist))
    MSR_weights_new=ratiolist_round(MSR_weights,3)
    print(texttranslate("权重："),MSR_weights_new)
    print(texttranslate("观察期："),hstart+" to "+hend)
    print(texttranslate("预期收益")+":",round(MSR_y*100.0,2),'\b%')
    print(texttranslate("预期风险")+":",round(MSR_x*100.0,2),'\b%')

    #import datetime as dt; today=dt.date.today()    
    print(texttranslate("数据来源：雅虎财经，"),str(today))

    #输出GMV投资组合构成比例
    print("\n====="+texttranslate("投资组合的构造方式: 风险最小化(GMV)"),"=====")
    print(texttranslate("成分股："),translate_tickerlist(tickerlist))
    GMV_weights_new=ratiolist_round(GMV_weights,3)
    print(texttranslate("权重："),GMV_weights_new)
    print(texttranslate("观察期："),hstart+" to "+hend)
    print(texttranslate("预期收益")+":",round(GMV_y*100.0,2),'\b%')
    print(texttranslate("预期风险")+":",round(GMV_x*100.0,2),'\b%')    
    print(texttranslate("数据来源：雅虎财经，"),str(today))
...
#==============================================================================
def portfolio_ef_1(stocks,fromdate,todate):
    """
    ...
    plt.title(texttranslate('投资组合：马科维茨有效边界'))
    
    import datetime as dt; today=dt.date.today()    
    
    footnote1=texttranslate('预期风险')+'-->\n\n'
    footnote2=texttranslate("投资组合由")+str(n_stock)+' stocks: \n'+str(translate_tickerlist(stocks))
    footnote3="\n"+texttranslate('观察期：')+str(fromdate)+' to '+str(todate)
    footnote4="\n"+texttranslate("数据来源：雅虎财经，")+str(today)
    ...
    """
    plt.figtext(x_left,y_left,texttranslate("投资组合由")+str(n_stock)+' stocks: ')
    plt.figtext(x_left,y_left-0.05,' '+str(stocks))
    plt.figtext(x_left,y_left-0.1,texttranslate('观察期：')+str(fromdate)+' to '+str(todate))
    """
    
    plt.plot(out_df_ef.index,out_df_ef['mean'],color='r',ls='--',lw=2,label=texttranslate('有效边界'))
    plt.plot(out_df_ief.index,out_df_ief['mean'],color='k',ls=':',lw=2,label=texttranslate('无效边界'))
    plt.plot(out_std_min,out_mean_min,'g*-',markersize=16,label=texttranslate('风险最低点'))
