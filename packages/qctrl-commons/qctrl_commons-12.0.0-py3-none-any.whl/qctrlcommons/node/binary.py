...
def _create_flexible_binary_node_data(
    ...
    check_argument(
        not (
            isinstance(x, node_data.PwcNodeData)
            and isinstance(y, node_data.StfNodeData)
        )
        and not (
            isinstance(x, node_data.StfNodeData)
            and isinstance(y, node_data.PwcNodeData)
        ),
    ...
    if isinstance(x, node_data.StfNodeData) and isinstance(y, node_data.StfNodeData):
        ...
        return node_data.StfNodeData(
            _operation, value_shape=value_shape, batch_shape=batch_shape
        )

    # operation(Pwc, Pwc)
    if isinstance(x, node_data.PwcNodeData) and isinstance(y, node_data.PwcNodeData):
    ...
    if isinstance(x, node_data.PwcNodeData) or isinstance(y, node_data.PwcNodeData):

        if isinstance(x, node_data.PwcNodeData):
            f_batch_shape, f_value_shape = validate_batch_and_value_shapes(x, "x")
            f_name = "x"
...
class Addition(Node):
    ...
    rtype = Union[types.Tensor, types.Pwc, types.Stf]
...
class Matmul(Node):
    ...
    name = "matmul"
    args = [
        forge.arg("x", type=Union[np.ndarray, types.Tensor, types.Pwc, types.Stf]),
        forge.arg("y", type=Union[np.ndarray, types.Tensor, types.Pwc, types.Stf]),
    ]
    rtype = Union[types.Tensor, types.Pwc, types.Stf]
