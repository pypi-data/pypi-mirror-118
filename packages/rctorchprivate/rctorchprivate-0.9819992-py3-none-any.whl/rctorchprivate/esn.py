...
class EchoStateNetwork(nn.Module):
    ...
    def train_state_feedback(self, t, X, state, y, output = False, retain_grad = False):
        ...
        return next_state, None#self.LinOut(cat([X, next_state], axis = 0).view(self.n_outputs,-1))
    ...
    def train_states_supervised(self, X, y, states, calc_grads = True, outputs = True):
        ...
        with no_grad():
            ...
            for t in range(1, X.shape[0]):
                ...
                input_t =  X[t, :].T
                state_t, outputs = self.train_state(t, X = input_t, state = states[t-1,:], y = y[t-1,:])
        ...
        with no_grad():
            ...
            with torch.set_grad_enabled(self.track_in_grad):
                ...
                if not preloaded_states_dict:
                    ...
                    if self.ODE_order:
                        y_extended = torch.ones((self.X_extended.shape[0], self.n_outputs), **self.dev)

                        y_extended = torch.vstack((y_extended[0], y_extended))
                ...
                if not backprop_f:
                    ...
                    for i, weight in enumerate(self.weights_list):
                        ...
                        if not eq_system:
                            ...
                            yfit[:, 0] = yfit[:,0] + init_conds[0]
                            for i, cond in enumerate(1, init_conds):
                                yfit[:, i] = yfit[:,i] + cond
                        ys.append(yfit)
    ...
    def unsupervised_val_states(self, n_samples, inputs, states, outputs):
        for t in range(n_samples):
            ...
            input_t, state_t = inputs[t, :], states[t,:]

            state_t, _  = self.train_state(t, X = input_t, state = state_t, y = None)
            states = cat([states, state_t.view(-1, self.n_nodes)], axis = 0)

            #extended_state_spec = cat([input_t, state_t]).view(-1, 1)
