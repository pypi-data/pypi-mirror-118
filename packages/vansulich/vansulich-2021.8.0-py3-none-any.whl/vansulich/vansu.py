...
def get_vansu():
  ...
  tomorrowa = datetime.now() + timedelta(days = 1)
  arr_tomo = tomorrowa.strftime("%Y-%m-%d").split('-')
  ngaymai = S2L(int(arr_tomo[2]), int(arr_tomo[1]), int(arr_tomo[0]))
  ngay_mai = str(ngaymai[0])+"/"+str(ngaymai[1])+"/"+str(ngaymai[2])
  print(ngay_mai)


  return {'amLich': amLich, 'saoTot': saoTot ,'saoXau': saoXau, 'gioTot': gioTot, 'gioXau': gioXau ,'ngayTotXau': ngayTotXau, 'truc':truc, 'tuoiHop':tuoiHop, 'tuoiXung':tuoiXung, 'ngay_homnay':onlyDay, 'ngay_mai' :ngay_mai }
...
def khoang_cach_AL_bk(mm = 3, dd =10):
    ...
    print(lu_date)
    #kiem tra thang am co nhuan khong
    thang_nhuan = int(str(lu_date[3]))

    # chuyen am sang duong
...
def khoang_cach_AL(mm = 3, dd =10):
    ...
    #ngay am hom nay
    al_today = S2L(today.day, today.month, today.year)

    # ngay am can tinh
    if int(al_today[1] < int(mm)):
      # thang  hien tai nho hon thang can tinh thi lay nam am hien tai
      nam_am = al_today[2]
    elif int(al_today[1]) == int(mm):
      # thanh hien tai = thang can tinh 
      # Ngay hien tại < ngay can tinh thi nam hien tai
      if int(al_today[0]) <= int(dd):
        nam_am = al_today[2]
      else:
        nam_am = al_today[2] +1
    else:
    ...
    #duong can tinh
    sun_date = L2S(dd, mm, nam_am, 0)

    # tu ngay can tinh - ngay hien tai
    dayxx = from_to_day(sun_date[2], sun_date[1], sun_date[0] , today.year, today.month, today.day)
...
def tachhtml(str):
    ...
    str = str.replace('</strong><div class="giohh-cs"> h-01h) , ','')
    return str.strip()
