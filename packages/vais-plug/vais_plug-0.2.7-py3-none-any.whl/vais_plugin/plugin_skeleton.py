...
class PluginSkeleton(object):
    ...
    def registry_plugin(self, file_exec, func_exec, version_code, version_name, config):
        ...
        try:
            ...
            code = 500
        logging.info(f"Result register plugin code: {code}")
        return code
    ...
    def log_worker_info(self):
        if self.worker is not None:
            self.worker.refresh()
            logging.info(f'Info: state: {self.worker.state}, working_time: {self.worker.current_job_working_time}, failed_count: {self.worker.failed_job_count}, ok_count: {self.worker.successful_job_count}')
        else:
            logging.info(f"Worker has not been initialized")
    ...
    def run(self, file_exec, func_exec, version_code, ignore_register=False):
        ...
        self.scheduler.add_job(
            ...
            IntervalTrigger(seconds=int(os.environ.get('INTERVAL_REGISTER_SECONDS', 1)) * 60, timezone="UTC")
        )

        self.scheduler.add_job(
            self.log_worker_info,
        ...
        if register_status == 201 or ignore_register:
            ...
            self.worker = Worker([queue], connection=PluginSkeleton.redis, exception_handlers=[PluginSkeleton.handler_error])
            self.worker.work(logging_level="DEBUG")
        else:
    ...
    def push_result_status(current_job, doc_index, doc_id, result,
        ...
        plugin_code = os.environ.get('PLUGIN_CODE_PUSH_RESULT', os.environ.get('PLUGIN_CODE'))
        global START_PROCESS_TIME
        if "START_PROCESS_TIME" not in globals():
            START_PROCESS_TIME = -1
        handle_process_time = time.time() - START_PROCESS_TIME if START_PROCESS_TIME > 0 else None
