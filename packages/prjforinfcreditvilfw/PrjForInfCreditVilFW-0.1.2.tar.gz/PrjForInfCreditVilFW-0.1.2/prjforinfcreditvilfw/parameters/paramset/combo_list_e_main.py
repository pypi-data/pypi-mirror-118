...
Main point of entry for testing at least, to make sure full code work, in a more
streamlined fashion for R&R purposes. 
'''
...
def get_combo_list(combo_type=['e', '20200901'], compesti_specs=None):
    ...
    main_type_str_list = ['20201025', '19E1']
    if any([main_type_str in sub_type for main_type_str in main_type_str_list]):
        ...
        if "_1j7" in sub_type:
            ...
            # model_t = '20180701'
            model_t = st_common_subtype
        else:
            model_t = st_common_subtype
            logger.info('no specific sub_type relevant for model, assume _7jAll')

        # D. Generate list of param combos and create combo_list
        if len(combo_type) >= 5:
            # There is a fifth element, this is post mpoly estimation, Use existing JSON File estimates
            combo_list = paramloop_json.combo_list_auto(
        ...
        elif len(combo_type) >= 3 and combo_type[2] is not None:
    ...
    if any([main_type_str in sub_type for main_type_str in main_type_str_list]):
        ...
        if len(combo_type) >= 5:
            ...
            combo_list = paramloop_json.combo_list_auto(
                ...
                minmax_f='a', minmax_t=st_common_subtype,
                data_f='a', data_t=st_common_subtype,
                esti_f='a', esti_t=sub_type.split('_')[0],
                model_f='a', model_t=model_t,
                grid_f='a', grid_t=st_common_subtype,
                interpolant_f='a', interpolant_t=st_common_subtype,
                dist_f='a', dist_t=st_common_subtype)
