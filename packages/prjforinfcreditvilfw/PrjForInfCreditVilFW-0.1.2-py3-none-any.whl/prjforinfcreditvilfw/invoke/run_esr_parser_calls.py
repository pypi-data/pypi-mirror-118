...
dc_it_execute_type = {'model_assumption': 1,
                      'compute_size': 1, 'esti_size': 2, 'esti_param': 2,
...
for st_regions in ar_regions:
    if esr_run in [1, 3, 5, 7]:
        if esr_run == 3:
            # longest running time allowed for mpoly iteration 3
            it_attemptDurationSeconds = 60 * 60
        elif esr_run == 7:
            # three hours max
            it_attemptDurationSeconds = 60 * 60 * 3
        else:
            # 60 minutes max for 1 and 5, which are model evaluations
            it_attemptDurationSeconds = 60 * 60
            ...
            response = aws_batch.submit_job(
                ...
                containerOverrides={"vcpus": int(it_vcpus),
                                    ...
                                    "command": ["python",
                                                ...
                                                "-f", esrf]},
                timeout={
                    'attemptDurationSeconds': it_attemptDurationSeconds
                })
