...
class GenerateInterfaceTest:
    ...
    def __init__(self, output_path):
        ...
        self.__template_folder = Path(__file__).parent / "./template/interface"
    ...
    def generate_test_case(self, filename):
        ...
        if not struct_data:
            ...
            return None
        global_json_path = self.output_path / self.config_folder / "global.json"
        write_json_file(global_json_path, struct_data["config_data"])

        config_util_path = self.output_path / self.config_folder / "config_util.py"
        ...
        for test_case_name in struct_data["testcases"]:
            ...
            cur_file_path = self.output_path / self.testcase_folder / filename
            write_file(cur_file_path, output)
            console_output("create test case %s success" % cur_file_path)
    ...
    def generate_aw(self, filename):
        ...
        for tag in struct_data["awTags"]:
            ...
            file_name = self.output_path / self.aw_folder / py_name
            output = template.render({"tagData": struct_data["awTags"][tag]})
            write_file(file_name, output)
            console_output("create aw %s file success" % file_name)
