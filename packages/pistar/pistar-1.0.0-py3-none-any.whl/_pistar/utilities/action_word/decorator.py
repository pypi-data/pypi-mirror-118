...
from _pistar.resources.loggers import server_logger
...
def wrap_generator(action_word, checker):
    """
    description: this function is used to wrap action word, which is generator.
    """

    @functools.wraps(action_word)
    def wrapper(*args, **kwargs):
        # get the testcase.
        testcase, caller = get_testcase_from_call_stack()

        logger = testcase.user_logger if testcase else server_logger
        ...
        action_word.__globals__['logger'] = logger

        # execute this action word.
        time_consuming = 0
        start_time = now()
        return_value = action_word(*args, **kwargs)
        time_consuming += now() - start_time

        if testcase is not None:
            testcase.append_action_word_execution_information(
                module_name=action_word.__module__,
                caller_name=caller,
...
def wrap_non_generator(action_word, checker):
    """
...
class Keyword:
    ...
    def __call__(self, action_word):
        # initialize an action word checker.
        checker = ActionWordChecker(action_word)
        ...
        if inspect.isgeneratorfunction(action_word):
            wrapper = wrap_generator(action_word=action_word, checker=checker)
        else:
            wrapper = wrap_non_generator(action_word=action_word,
                                         checker=checker)
        return wrapper
