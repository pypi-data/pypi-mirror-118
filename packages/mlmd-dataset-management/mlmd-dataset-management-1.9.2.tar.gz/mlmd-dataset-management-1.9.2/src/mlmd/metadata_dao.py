...
def get_image_executions_by_version(version_id, metadata_filter=None):
    ...
    data = execute_select_query("""SELECT im.image_id, im.image_name, ex.type_id, im.annotation FROM image_log il 
    ...
    return [{"image_id": item[0], "image_name": item[1], "execution_id": item[2], "annotation": item[3]} for item in data]
...
def get_all_tenants(tenant_name):
    r = execute_select_query("SELECT tenant_name, id FROM tenant")
    if len(r) > 0:
