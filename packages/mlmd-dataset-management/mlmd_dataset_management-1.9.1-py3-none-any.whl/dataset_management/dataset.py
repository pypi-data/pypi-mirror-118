...
class Dataset():
    ...
    def add_files_from_generator(self, generator, override=False):
        ...
        for filepath, filename, annotation, tenant in generator:
            metadata_list.append((filename, tenants[tenant], self.id, annotation))
            if override == False:
    ...
    def __collect_changelist(self, version, global_changelist, global_annotations, metadata_filter=None):
        ...
        for item in image_log_arr:
            ...
            if global_changelist.get(item.get("image_name")) is None:
                global_changelist[item.get("image_name")] = {"changes":[item.get("execution_id")], "annotation": item.get("annotation")}
            else:
    ...
    def get_filelist(self, metadata_filter=None, get_annotation=False, file_path=None):
        ...
        annotation_dict = dict(map(lambda t: (t[1], t[0]), enumerate(self.global_annotations)))
