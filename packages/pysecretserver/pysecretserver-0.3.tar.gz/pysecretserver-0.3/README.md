...
The working directory can be removed is desired.
...
- `get_item_metadata(item_id)`: Retrieve the metadata defined on an item.
...
- `get_fields()`: Retrieve a list of all metadata fields as a list() of dict()
...
Update a folder:
```
>>> folder = ss.Folders.get_item(1831)
>>> folder.update({'folderName':'Extra Double Secret 009'})
>>> ss.Folders.update_item(1831, folder)
