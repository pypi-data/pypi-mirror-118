...
class API(Generic[UserModel, Model, CreateForm, UpdateForm]):
    ...
    def bind(self, router: APIRouter):
        connection_name = self.model._meta.default_connection  # type: ignore
        ...
        if self.enable_create:
            ...
            @atomic(connection_name=connection_name)
            async def create(
            ...
            async def index(
                ...
                if condition:
                    for raw in condition:
                        ...
                        condition_q = Q(
                            **{
                                self.model.normalize_field(k): self.translate_condition(
                                    current_user, k, v
                                )
                                for k, v in kv.items()
                            },
