...
class Table:
    ...
    def _get_create_sql(cls):
        ...
        for name, field in inspect.getmembers(cls):
            ...
            elif isinstance(field, MTM):
                if not field.table._get_name() + '_id' in cls._get_column_names():
                    fields.append((name + "_id", "INTEGER"))
                mtm_sql = ALTER_MTM_SQL.format(table=field.table._get_name(),
                                               name=cls._get_name() + '_id')
    ...
    @classmethod
    def _get_table_scheme(cls):
        sql = f"PRAGMA table_info('{cls._get_name()}')"
