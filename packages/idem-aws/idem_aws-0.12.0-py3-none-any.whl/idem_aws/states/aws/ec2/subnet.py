...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    for subnet in ret["ret"]["Subnets"]:
        ...
        result[subnet["SubnetId"]] = new_subnet
        for i, data in enumerate(subnet["Ipv6CidrBlockAssociationSet"]):
            sub_subnet = new_subnet.copy()
            sub_subnet["ipv6_cidrBlock"] = data["Ipv6CidrBlock"]
            # The id needs to be in the name
            sub_subnet["name"] = subnet["SubnetId"]
            # Create a new state for this association set
            result[f"{subnet['SubnetId']}-{i}"] = new_subnet
    return result
