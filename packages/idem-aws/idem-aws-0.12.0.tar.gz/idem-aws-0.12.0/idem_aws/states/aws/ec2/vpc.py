...
async def present(
    ...
    **Autogenerated function**
    ...
    try:
        await hub.tool.boto3.resource.exec(resource, "reload")
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    for vpc in ret["ret"]["Vpcs"]:
        new_vpc = dict(
            ...
            amazon_provided_ipv6_cidr_block=bool(vpc["Ipv6CidrBlockAssociationSet"]),
            instance_tenancy=vpc["InstanceTenancy"],
            tag_specifications=vpc["Tags"],
        )
        result[vpc["VpcId"]] = new_vpc
        for i, data in enumerate(vpc["Ipv6CidrBlockAssociationSet"]):
            sub_vpc = new_vpc.copy()
            sub_vpc["ipv6_cidr_block"] = data["Ipv6CidrBlock"]
            sub_vpc["ipv6_cidr_block_network_border_group"] = data["NetworkBorderGroup"]
            sub_vpc["ipv6_pool"] = data["Ipv6Pool"]
            # The id needs to be in the name
            sub_vpc["name"] = vpc["VpcId"]
            # Create a new state for this association set
            result[f"{vpc['VpcId']}-{i}"] = new_vpc
    return result
