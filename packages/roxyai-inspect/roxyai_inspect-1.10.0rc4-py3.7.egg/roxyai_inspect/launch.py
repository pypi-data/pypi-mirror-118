...
if os.environ.get('VIRTUAL_ENV'):
    try:
        ...
        root = Path(roxyai_inspect.__file__).parent
    except Exception:
        root = Path(__file__).parent
    # 検査サーバが仮想環境にインストールされている場合
    SCRIPTS_FOLDER = Path(os.environ['VIRTUAL_ENV']) / 'Scripts'
    CHECKER_PATH = SCRIPTS_FOLDER / 'roxy_lic_checker.exe'
    if not CHECKER_PATH.exists():
        CHECKER_PATH = root.parent / 'Scripts' / 'roxy_lic_checker.exe'
else:
    # Python仮想環境外からの起動
...
def check_license(regfile: Path):
    ...
    if regfile:
        ...
        start_server = False
    if not SERVER_LICENSE.exists():
        err_exit('ライセンスファイルが登録されていません。', -1)
    else:
        command = f'{CHECKER_PATH} -i {SERVER_LICENSE}'
        print(f'run: {command}')
        print(f'SCRIPTS_FOLDER: {SCRIPTS_FOLDER}')
        result = run(command)
