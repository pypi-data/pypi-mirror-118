...
def calculate_statistics(Datos,statistics,temporal_resolution):
    ...
    if np.sum(['var' in i for i in statistics])>=1:
        ...
        for i, ii in enumerate(pos):
            ...
            aux=Datos.resample(str(h) + t).agg(pd.Series.sum, min_count=1); 
...
class evaluateInd_PSO(object):
    ...
    def __call__(self, ind):
        ...
        if np.sum(['M3' in i for i in self.s])>=1:
            ...
            for i, ii in enumerate(pos):
                h=int(self.s[ii].split("_",1)[1])
                a = NSRP_momentos_orden_3('Poisson',h,0, landa, mu_c, eta, xi, betha, alpha, alpha_p)
...
def NSRP_simulation1(Parametros_meses, year_ini, year_fin, temporal_resolution,process, Seasonality):

    landa=Parametros_meses[Parametros_meses.index=='landa'].values[0];Storm_origin=1/landa
    ipsilon=Parametros_meses[Parametros_meses.index=='ipsilon'].values[0];Number_cells_per_storm=ipsilon-1
    
    if process=='normal':
        eta=Parametros_meses[Parametros_meses.index=='eta'].values[0]; Duracion_cell=1/eta
        xi=Parametros_meses[Parametros_meses.index=='xi'].values[0]; Intensidad_cell=1/xi

    betha=Parametros_meses[Parametros_meses.index=='betha'].values[0]; Dist_cell_origin=1/betha
    ...
    Dataframe_simulacion_unido_day_estaciones=pd.DataFrame(index=time_d, columns = ['Rain'])
    Dataframe_simulacion_unido_hour_estaciones=pd.DataFrame(index=time_h, columns = ['Rain'])
    ...
    for perio, monthss in enumerate(Seasonality):
        n_days=np.sum(np.in1d(Dataframe_simulacion_unido_day_estaciones.index.month, monthss))
        ...
        Dataframe_simulacion_day_aux=pd.period_range('1800', periods=n_days, freq='D')
        Dataframe_simulacion_hour_aux=pd.period_range('1800', periods=n_days*24, freq='h')
        Dataframe_simulacion_min_aux=pd.period_range('1800', periods=n_days*24*60, freq='min')

        time_star=datetime.datetime.strptime(str(Dataframe_simulacion_day_aux[0]), '%Y-%m-%d')
        time_end=datetime.datetime.strptime(str(Dataframe_simulacion_day_aux[-1]), '%Y-%m-%d')
        ...
        for i in range(n_storms):
            ...
            for j in range(Number_cell_per_storm[i][0]):
                ...
                if temporal_resolution=='d':
                    time1_cell=time1 + datetime.timedelta(days=Distancia_horas_cell_sim[j])
                    time2_cell=time1_cell + datetime.timedelta(days=Duracion_horas_cell_sim[j])

                elif temporal_resolution=='h':
                    time1_cell=time1 + datetime.timedelta(hours=Distancia_horas_cell_sim[j])
                    time2_cell=time1_cell + datetime.timedelta(hours=Duracion_horas_cell_sim[j])
        ...
        #################################################################################
        ############################################################################
        tt=pd.period_range(start=Dataframe_simulacion_day_aux[0],end=Dataframe_simulacion_day_aux[-1], freq='min')
        ...
        if np.size(t_ordinal)==0:
            ...
            Dataframe_simulacion_unido_day_estaciones['Rain'].iloc[np.where(position_day)[0][0:len(df2)]]=df2.values.flatten()
            
            Dataframe_simulacion_unido_hour_estaciones['Rain'].iloc[np.where(position_hour)[0][0:len(df3)]]=df3.values.flatten()
