...
import sys
sys.path.append('/home/javi/source/lib_brach_javi/STNSRPM/NSRP/')
...
import yaml
import ast
from NSRP.parameters_calibration import parameters_calibration
...
class Calibration(object):
    ...
    def __call__(self, statistics, verbose=False):
        ...
        if self.hiperparams.process=='normal':
            ...
            Dataframe_parametros=pd.DataFrame(index=param_s,columns=self.hiperparams.Seasonality_str); n_p=5;
            ...
            # We create a dataframe to store the adjusted intensity function parameters.
            Dataframe_parametros=pd.DataFrame(index=param_s,columns=self.hiperparams.Seasonality_str); n_p=10
            limites=np.array([[lim[0,0],lim[0,1]],[lim[1,0],lim[1,1]],[lim[2,0],lim[2,1]],[lim[3,0],lim[3,1]],\
        ...
        for pri, prii in enumerate(self.hiperparams.Seasonality):
            ...
            if self.hiperparams.process=='normal':
                ...
                Dataframe_parametros.loc[:,str(prii)]=param_v
        ...
        resuls = parameters_calibration(Dataframe_parametros,statististics_fit_dataframe,self.statististics_dataframe, Error_dataframe)
        
        
        return resuls
        
    # def save_files(self,resuls,path_output_files): 
    #         resuls.Fitted_parameters.to_csv(path_output_files+'./Calibrated_parameters.csv')
    #         resuls.statististics_Real.to_csv(path_output_files+'./statististics_real.csv')
    #         resuls.statististics_Fit.to_csv(path_output_files+'./statististics_fit.csv')  
