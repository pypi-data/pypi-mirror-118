...
def data_download(target_number, outliers, sigma, dir):
    """
...
def evaluation(func, time, flux, flux_err):
    ...
    ---------------------------------------------------------------------
    Evaluates the fitted function with the independent variable.
    ---------------------------------------------------------------------
    INPUTS:     - func: defined harmonic series
                - time: julian date
                - flux: flux of star in e-/s
            -------------------------------------------------------------
    OUTPUTS:    - y: 1D evaluated values of the fitted function
                - y_err: 1D evaluated errors
    ----------------------------------------------------------------------
    PROCEDURE:
                1. Calculates least squares parameters for given func.
                2. Obtains fitted values.
                3. Obtains the uncertainty of the fitted values.
    ----------------------------------------------------------------------
    """

    param, param_err = min_squares_weighted(time, flux, flux_err, func)
    ...
    for j in range(0, l):
        for i in range(0, lf):
            y_err[0, j] = y_err[0, j] + (func(time[j])[i] * param_err[i]) ** 2
...
def chi2_reduced(func, time, flux, flux_err):
    ...
    ----------------------------------------------------------------------
    """
    deviations = flux - evaluation(func, time, flux, flux_err)[0]
...
def light_curve_fit(target_number, outliers, sigma, min_period, max_period, dir):
    """
    ...
    time, flux, flux_err = data_download(target_number, outliers, sigma, dir)
    T_0 = 0
    period, period_err = period_search(
        time, flux, flux_err, min_period, max_period)
    omega, omega_err = angular_frequency(
        time, flux, flux_err, min_period, max_period)
    minChi, mins = best_fit(
        time, flux, flux_err, omega, T_0)
    func = harm_fourier_series(mins, omega, T_0)
    fitted, fitted_err = evaluation(
        func, time, flux, flux_err)
...
def phase_plot(target_number, outliers, sigma, min_period, max_period, dir):
    """
    ...
    ---------------------------------------------------------------------
    INPUTS:
                - target_number: position of star on sector csv file.
                - outliers: boolean, trim outliers.
                - sigma: number of sigma (outliers).
                - min_period: min period to star the search.
                - max_period: max period to finish the search.
    ...
    time, phaseA, flux, flux_err, fitted, minChi, mins, period, period_err, amplitude, amplitude_err = phase_plot_fit(
        target_number, outliers, sigma, min_period, max_period)
...
def amplitude_test(min, max, amp, dir):
    """
...
def csv_amplitude_test(file, min, max, amp):
    """
                    csv_amplitude_test DOCUMENTATION
    ---------------------------------------------------------------------
    Detects variable stars with amplitude higher than threshold.
    ---------------------------------------------------------------------
    INPUTS:
    ...
    OUTPUTS:    - candidates: 1D numpy array with variable candidate
                ...
                - amplitude_errors: 1D numpy array with the uncertainty
                of the amplitude of each candidate.
                - csv file: csv file containing results of the
                amplitude test
    ...
    PROCEDURE:
                1. Calculates amplitude for an observed star.
                2. Calculates if amplitude is bigger than threshold.
                3. Stores the results in the CSV file
                4. Returns candidates and their characteristics.
    ----------------------------------------------------------------------
    """
