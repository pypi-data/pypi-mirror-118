...
from flywheel_gear_toolkit.context.constants import (
    BOTTOM_UP_PARENT_HIERARCHY,
    FILE_OBJECT_DICT,
)
...
class GearToolkitContext:
    ...
    def update_file_metadata(self, file_name, *args, **kwargs):
        ...
        Args:
            ...
            *args: Arbitrary list of dictionaries.
    ...
    def clean_metadata(self):
        ...
        self._update_metadata_zip_member_count()

    def write_metadata(self):
        """Write the metadata json file to the output folder."""
        # clean first to populate zip_member_count
        self.clean_metadata()
    ...
    def _update_metadata_zip_member_count(self):
        """Update metadata with zip_member_count for zip files in output_dir."""
        zip_files = [
            x
        ...
        for zf in zip_files:
            try:
                with closing(ZipFile(zf)) as archive:
                    count = len(archive.infolist())
                    self.update_file_metadata(zf.name, zip_member_count=count)
            except BadZipFile:
                log.warning(
                    f"Invalid zip file {str(zf)}. " f"Skipping zip_member_count update"
                )
