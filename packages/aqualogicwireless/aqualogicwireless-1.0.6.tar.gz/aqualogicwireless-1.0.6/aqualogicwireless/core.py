...
class AquaLogic():
    ...
    def _write_burst(self, data):
        for i in range(1, 6):
            Timer(0.008 * i, self._write,
                  [data['frame']]).start()
    ...
    def _send_frame(self):
        if not self._send_queue.empty():
            ...
            Timer(0.048, self._write_burst, [data]).start()
            ...
            try:
                if data['desired_states'] is not None:
                    ...
                    # the state to change.
                    Timer(1.98, self._check_state, [data]).start()
            except KeyError:
                pass
        ...
        try:
            while True:
                ...
                if frame_type == self.FRAME_TYPE_KEEP_ALIVE:
                    ...
                    text = frame.replace(b'\xdf', b'\xc2\xb0').replace(
                        b'\xba', b'\x3a').decode('utf-8')
                    parts = text.split()
