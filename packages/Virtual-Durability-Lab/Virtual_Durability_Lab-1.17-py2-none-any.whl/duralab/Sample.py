...
class Sample:
    """
        ...
        representation : string
            ...
            Choices : 'Random_Variates', 'Cumulative_distribution', 'Probability_density' 
        ...
        ----------
        ValueError
            Law parameter incorrect. You must choose one of the Exponential, Poisson, Weibull, Lognormal, Normal laws.
    ...
    def __init__(self, **kwargs):
        ...
        #Define if the graph is show automtically or not
        self.GRAPH = True
        ...
        self.law_text = kwargs.get("law", None)
        self.name = kwargs.get("name", self.law_text)
        ...
        if self.law_text not in ["Uniform", "Exponential", "Weibull", "Lognormal", "Normal"]:
        ...
        self.law = {"Exponential" : s.expon, "Uniform" : s.uniform, "Weibull" : s.weibull_min, "Lognormal" : s.lognorm, "Normal" : s.norm}[self.law_text]
         
        self.exec_sample()

    def exec_sample(self):
        if(self.law_text == "Weibull"):
        
            if(self.param1>0 and self.scale>0):
                
                if (self.loc == 0):
                    self.Allan_Plait_representation = True
    
                self.x = np.linspace(self.law.ppf(0.01, self.param1, loc=self.loc, scale=self.scale), self.law.ppf(0.99, self.param1, loc=self.loc, scale=self.scale), 1000)
                
                if self.representation == "Random_variates":
                    self.sample_definition(c=self.param1, loc=self.loc, scale=self.scale, size=self.size)
                else:
                    self.sample_definition(x=self.x, c=self.param1, loc=self.loc, scale=self.scale)  
        ...
        elif(self.law_text == "Lognormal"):
            
            if (self.param1>0):
                self.x = np.linspace(self.law.ppf(10e-8, self.param1, loc=self.loc, scale=self.scale), self.law.ppf(1-10e-8, self.param1, loc=self.loc, scale=self.scale), 1000)

                if self.representation == "Random_variates":
                    self.sample_definition(s=self.param1, loc=self.loc, scale=self.scale, size=self.size) 
                else:
                    self.sample_definition(x=self.x, s=self.param1, loc=self.loc, scale=self.scale) 
            elif (self.param1<=0):
        ...
        elif(self.law_text == "Uniform"):
               
            self.x = np.linspace(self.law.ppf(10e-8, loc=self.loc, scale=self.scale), self.law.ppf(1-10e-8,loc=self.loc, scale=self.scale), 1000)
            if self.representation == "Random_variates":
        ...
        elif(self.law_text == "Exponential"):
    ...
    def sample_visualization(self, **kwargs):
        ...
        self.fig, self.ax = plt.subplots(2, 1) if self.Allan_Plait_representation and self.representation == "Random_variates" else plt.subplots(1,1)
...
class TestSample:
    ...
    def functional_test_2(self):
        self.sample = Sample(name="TestPlanEfficiency",size=10,representation="Cumulative_distribution", law="Weibull", scale=19000, param1=1.3)
        print(self.sample)
