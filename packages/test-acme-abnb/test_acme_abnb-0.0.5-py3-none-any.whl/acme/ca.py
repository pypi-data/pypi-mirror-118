...
class LetsEncrypt:
    ...
    def _register_lets_encrypt_account(self) -> None:
        ...
        try:
            ...
            account_json = get_secret(f'{prefix}/otter/account.json', element=None, region=self.region)
            with open('account.json', 'w') as outfile:
                outfile.write(account_json)
            account_key = get_secret(f'{prefix}/otter/account.key', region=self.region)
