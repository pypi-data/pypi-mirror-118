...
class Site(Model):
    ...
    soil: Mapped[list["Soil"]] = relationship("Soil", back_populates="site")
    soil_hydrology: Mapped[list["SoilHydrology"]] = relationship(
        "SoilHydrology", back_populates="site"
    )
    param01 = Column(Float)
    param02 = Column(Float)
    param03 = Column(Float)
...
class Soil(Model):
    ...
    id = Column(Integer, primary_key=True)
    site_id = Column(Integer, ForeignKey("site.id"))
    site: "Site" = relationship("Site", order_by="Soil.depth", back_populates="soil")
    depth = Column(Float)
...
class SoilHydrology(Model):
    ...
    site: "Site" = relationship(
        "Site", order_by="SoilHydrology.depth", back_populates="soil_hydrology"
    )
    depth = Column(Float)
