...
def processAlarmStatistikData(    
    ...
    try:             
        ...
        SEGResDct=fGetResTimes(l,TCsLDSRes1)
        ...
        DruckResDct=fGetResTimes(l,TCsLDSRes2)
...
def addNrToAlarmStatistikData(    
    ...
    Returns: SEGResDct,DruckResDct added with:
    value: dct
...
def processAlarmStatistikData2(    
    ...
    try:             
        ...
        for idx,(ID,tPairsDct) in enumerate(DruckResDct.items()):
            ...
            for idxTriple,(DIVPipelineName,SEGName,SEGResIDBase) in enumerate(tripleLst):      

                if idxTriple>0:
                    logger.debug("{:s}ID {:35s} wird bei SEGName {:s} nicht gezaehlt sondern nur bei SEGName {:s}.".format(logStr,ID,SEGName,tripleLst[0][1]))
                    continue

                if len(tPairsDct['Alarm'])>0:                   
                    logger.debug("{:s}SEGName {:20s}: durch ID {:40s} mit Alarm. Nr des Verweises von ID auf ein Segment: {:d}".format(logStr,SEGName,ID, idxTriple+1))
        ...
        for idx,(ID,tPairsDct) in enumerate(SEGDruckResDct.items()):
            for idx2,ext in enumerate(tPairsDct.keys()):        
                if ext in ['AL_S_SB_S','AL_S_ZHKNR_S','AL_S_NR']:                   
                    pass
                else:
                    tPairs=tPairsDct[ResChannelResultNames[idx2]]
        ...
        for idx,(ID,tPairsDct) in enumerate(SEGDruckResDct.items()):            
            for idx2,ext in enumerate(tPairsDct.keys()):        
                v=tPairsDct[ext]
                if ext in ['AL_S_SB_S','AL_S_ZHKNR_S']:
            ...
            #    baseIDs=dfSegsNodesNDataDpkt[dfSegsNodesNDataDpkt['SEGResIDBase']==ID]['DruckResIDBase'].unique()
...
def plotDfAlarmStatistik(    
    ...
    df['FörderZeitAl']=dfAlarmStatistik.apply(lambda row: "{!s:s} (Nrn.: {!s:s})".format(row['FörderZeitAl'],row['FörderZeitenAlNrn']) if row['FörderZeitenAlAnz'] > 0 else row['FörderZeitAl'] ,axis=1)
    df['RuheZeitAl']  =dfAlarmStatistik.apply(lambda row: "{!s:s} (Nrn.: {!s:s})".format(row['RuheZeitAl']  ,row['RuheZeitenAlNrn']  ) if row['RuheZeitenAlAnz']   > 0 else row['RuheZeitAl']   ,axis=1)
...
def fCVDTime(row,dfSEG,dfDruck,replaceTup=('2021-','')):
    ...
    return Time.replace(replaceTup[0],replaceTup[1])
...
def buildDfAlarmEreignisse( 
   ...
   ,NrAsc=[False]+4*[True]
    ...
    Returns dfAlarmEreignisse:
        ...
        tD_ZHKNR:           Lebenszeit der ZHKNR; x am Ende, wenn ZHK andauert bei Res12-Ende; '-1', wenn Lebenszeit nicht ermittelt werden konnte
        ...
        NrSEGDruck:         lfd. Nr Alarm BaseType
    ...
    try:       
            ...
            for ResIDBase,dct in ResDct.items():
                ...
                if len(AL_S) > 0:       
                    ...
                    for idx,AL_S_Timepair in enumerate(AL_S):            
                        ...
                        ZHKNR_S_Lst=ZHKNRnListen[idx]
                        #ZHKNR_S_Lst=ResSrc.loc[t1:t2,ID].unique()
                        if len(ZHKNR_S_Lst) != 1:                
                            logger.warning(("{:s}ID: {:s}: Alarm {:d} von {!s:s} bis {!s:s}: Anzahl verschiedener ZHKNRn !=1: {:d} {:s}: ZHKNR eines Alarms wechselt waehrend eines Alarms. Alarm wird verarbeitet mit 1. ZHKNR.".format(logStr,ID
                           ,idx
                           ,t1
                           ,t2
                           ,len(ZHKNR_S_Lst)
        ...
        dfAlarmEreignisse['SEGName']=dfAlarmEreignisse.apply(lambda row: 
                                                             ...
                                                             else  dfSegsNodesNDataDpkt[dfSegsNodesNDataDpkt['DruckResIDBase']==row['OrteIDs'][0]]['SEGName'].iloc[0] 
                                                             ,axis=1)
        ...
        # lfd. Nummern
        dfAlarmEreignisse['NrSEGDruck']=dfAlarmEreignisse.groupby(['LDSResBaseType']).cumcount() + 1        
        ...
        # AlarmEvent = namedtuple('alarmEvent','tA,tE,ZHKNR,LDSResBaseType')
        dfAlarmEreignisse=dfAlarmEreignisse[['Nr','tA', 'tE','tD','ZHKNR','tD_ZHKNR','LDSResBaseType'
                    ,'OrteIDs', 'Orte', 'Ort', 'SEGName'
                    ,'Voralarm', 'Type', 'Name'
                    ,'NrSEGDruck', 'NrName', 'NrSEGName'
                    ]]
...
def plotDfAlarmEreignisse(    
    ...
    try:                     
        ...
        df=dfAlarmEreignisse[['Nr','LDSResBaseType','Voralarm','Type','NrSEGDruck','tA','tE','tD','ZHKNR','Name','Orte','tD_ZHKNR','NrName','NrSEGName','SEGName']].copy()
        ...
        df=df[['Nr','LDSResBaseType','NrSEGDruck','tA','tE','tD','ZHKNR','Name','NrName','NrSEGName','SEGName','tD_ZHKNR']]#,'Orte']]
        ...
        df['NrSEGName (SEGName)']=df.apply(lambda row: "{!s:s} ({!s:s})".format(row['NrSEGName'],row['SEGName']),axis=1)

        df=df[['Nr','ResTyp - Voralarm','NrSEGDruck','tA','tD','ZHKNR','ZHKName','NrName','NrSEGName (SEGName)','ZHKZeit']]
        ...
        t=plt.table(cellText=df.values, colLabels=df.columns
                    ...
                    ,colWidths=[.03,.1,.065,.08,.08,.05,.225
                                ,.05,.14 #,.16
                                ,.18]
                    , cellLoc='left'
                    , loc='center')    
