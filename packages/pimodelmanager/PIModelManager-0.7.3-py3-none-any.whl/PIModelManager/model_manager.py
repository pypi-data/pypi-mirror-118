...
class ModelManager:
    ...
    def _remove_unused_models(cls) -> None:
        ...
        for model in models_to_delete:
            model_name, language, version_model = model
            del cls._models[model_name][language][version_model]
            for pandora_model in cls._downloaded_models:
                if (pandora_model.name == model_name) and (pandora_model.language == language) and (pandora_model.version == version_model):
                    indices_to_be_deleted.append(
                        cls._downloaded_models.index(pandora_model))
            logger.info(
                f'Deleting unused model {model_name} {language} {version_model} from RAM')
    ...
    def _initialize_model_from_config(cls, config: Dict[str, str]) -> None:
        ...
        if not cls._models[model_name][language][version]:
            cls._models[model_name][language][version] = cls._model_class_mapping[model_name](
                model_path, config)
            logger.info(
    ...
    @ classmethod
    def get_model(cls, model_name: str, language: str, token_info: Any = {}, path: bool = False) -> Any:
        version = ''
        tenant_name = None
        client = ''
        if token_info:
        ...
        if path:
            try:
                return cls.get_model_path(language=language, name=model_name, client=client, version=version)
            except:
                pass
        else:
            try:
    ...
    def download_models(cls) -> None:
        ...
        for tenant_name in cls._tenant_configs:
            for model_name in cls._tenant_configs[tenant_name]:
                for language in cls._tenant_configs[tenant_name][model_name]:
                    model = cls._tenant_configs[tenant_name][model_name][language]
