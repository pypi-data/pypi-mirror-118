...
from ..integration._utils import _adx_support, _timezone_offsets
...
def upsert_device_sensors(df: pandas.DataFrame, api_inputs: ApiInputs, tag_columns: list = None,
    ...
    tag_columns : list, default = None
        Columns of dataframe that contain tags.
    metadata_columns : list, default = None
        Column(s) of dataframe that contain device-level metadata.
    ...
    tuple[str, pandas.DataFrame]
        (response, df) - Returns the response status and the dataframe containing the parsed response text.
    ...
    url = api_prefix + api_inputs.datacentre + "/" + api_inputs.api_project_id + "/Integration/DeviceSensors/Upsert"
    ...
    _adx_support(api_inputs=api_inputs, build_sensors=True)
...
def upsert_sites(df: pandas.DataFrame, api_inputs: ApiInputs, tag_columns: list = None,
    ...
    url = api_prefix + api_inputs.datacentre + "/" + api_inputs.api_project_id + "/Integration/Sites/Upsert"
    ...
    _adx_support(api_inputs=api_inputs, build_sites=True)
    logger.info("Ingestion complete. ")
...
def upsert_data(data_frame, api_inputs: ApiInputs, table_name: str, key_columns: list,
    ...
    -------
    str
        Returns the text of the response from API.
