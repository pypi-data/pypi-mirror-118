...
def thd(signal, fundamental, max_harmonic, during=None, mode="multiple_harmonics_only", max_tol_harmonics="auto"):
    ...
    max_tol_harmonics : "auto" or float - default is "auto"
        Defines a tolerance between the harmonic values necessary for THD computation dependent on the mode of
        calculation. If the tolerance is too small the function will return an exception informing that the expects
        harmonics weren't found considering the value set up in ``max_tol_harmonics``.
        When "auto" is selected the frequency values is defined by the equation below, where the number of cycles is
        forced to an integer value considering that the user entered with the correct amount of capture time.
        The tolerance exception can also be raised based on the principle of frequency interval calculation.

        The equation who predicts the frequency interval is:

        .. math::
            \Delta f = \frac{fundamental}{\left \lfloor fundamental \times T  \right \rfloor}
        where :math:`\Delta f` is the frequency interval calculated. The denominator
        :math:`\left \lfloor fundamental \times T  \right \rfloor` of the fraction is the number of cycles
        rounded as integer.
    ...
    """
    return _impl.thd(signal, fundamental, max_harmonic, during, mode, max_tol_harmonics)
