...
from skdecide.builders.domain import MultiAgent, Sequential, Simulation, Actions, Goals, Markovian, \
...
try:
    ...
    class MARTDP(ParallelSolver, Solver, DeterministicPolicies, Utilities):
        ...
        def __init__(self,
                     ...
                     heuristic: Optional[Callable[[Domain, D.T_state], Tuple[D.T_agent[Value[D.T_value]], D.T_agent[D.T_concurrency[D.T_event]]]]] = None,
            ...
            if heuristic is None:
                self._heuristic = lambda d, s: ({a: Value(cost=0) for a in s}, {a: None for a in s})
            else:
