...
from typing import Optional, Dict, Set, List, Iterable, Tuple
from skdecide.builders.domain.scheduling.graph_toolbox import Graph
from skdecide import Domain, Space, Value, Distribution, TransitionOutcome, ImplicitSpace, DiscreteDistribution
...
from skdecide.builders.domain.scheduling.modes import SingleMode, MultiMode
from skdecide.builders.domain.scheduling.resource_consumption import VariableResourceConsumption, ConstantResourceConsumption
from skdecide.builders.domain.scheduling.precedence import WithPrecedence
from skdecide.builders.domain.scheduling.preemptivity import WithPreemptivity, WithoutPreemptivity
from skdecide.builders.domain.scheduling.resource_type import WithResourceTypes, WithResourceUnits, WithoutResourceUnit
from skdecide.builders.domain.scheduling.resource_renewability import RenewableOnly, MixedRenewable
from skdecide.builders.domain.scheduling.scheduling_domains_modelling import SchedulingActionEnum, State, \
    SchedulingAction
from skdecide.builders.domain.scheduling.task_duration import SimulatedTaskDuration, DeterministicTaskDuration, \
    UncertainUnivariateTaskDuration
from skdecide.builders.domain.scheduling.task_progress import CustomTaskProgress, DeterministicTaskProgress
from skdecide.builders.domain.scheduling.skills import WithResourceSkills, WithoutResourceSkills
from skdecide.builders.domain.scheduling.time_lag import WithTimeLag, WithoutTimeLag
from skdecide.builders.domain.scheduling.time_windows import WithTimeWindow, WithoutTimeWindow
from skdecide.builders.domain.scheduling.preallocations import WithPreallocations, WithoutPreallocations
from skdecide.builders.domain.scheduling.conditional_tasks import WithConditionalTasks, WithoutConditionalTasks
from skdecide.builders.domain.scheduling.resource_availability import UncertainResourceAvailabilityChanges, \
    WithoutResourceAvailabilityChange, DeterministicResourceAvailabilityChanges
from skdecide.builders.domain.scheduling.resource_costs import WithResourceCosts, WithoutResourceCosts, WithModeCosts, WithoutModeCosts
from skdecide import T, EnumerableSpace, SamplableSpace
import random
...
class SchedulingDomain(WithPrecedence,
                       MultiMode,
    ...
    def _state_sample(self, memory: D.T_memory[D.T_state], action: D.T_agent[D.T_concurrency[D.T_event]]) -> \
        ...
        next_state = self.update_resource_availability_simulation(next_state,
                                                                  action)
        is_terminal = self._is_terminal(next_state)
        is_goal = self._is_goal(next_state)
        return TransitionOutcome(state=next_state,
    ...
    def _get_next_state_distribution(self, memory: D.T_memory[D.T_state],
        ...
        next_state_distrib = self.update_complete_dummy_tasks_uncertain(next_state_distrib, action)
        next_state_distrib = self.update_conditional_tasks_uncertain(next_state_distrib, action)
        next_state_distrib = self.update_time_uncertain(next_state_distrib, action)
        next_state_distrib = self.update_resource_availability_uncertain(next_state_distrib,
                                                                         action)  # maybe this should be done after self.update_time too ??
    ...
    def _get_next_state(self,
        ...
        next_state = self.update_resource_availability(next_state, action)  # maybe this should be done after self.update_time too ??
    ...
    def _get_applicable_actions_from(self, memory: D.T_memory[D.T_state]) -> D.T_agent[Space[D.T_event]]:
        ...
        if isinstance(self, WithoutResourceSkills) and isinstance(self, WithoutResourceUnit):
            return SchedulingActionSpace(domain=self,
                                         state=memory)
        else:
            # return SchedulingActionSpaceWithResourceUnit(domain=self, state=memory)
            return SchedulingActionSpaceWithResourceUnitSamplable(domain=self,
                                                                  state=memory)
    ...
    def _get_goals_(self) -> D.T_agent[Space[D.T_observation]]:
        return ImplicitSpace(lambda state:
    ...
    def initialize_domain(self):
        ...
        self.graph = self.compute_graph() # TODO : this is very specific to the precendence caracteristic,
    ...
    def update_time_uncertain(self, states: DiscreteDistribution[State], action: SchedulingAction):
        """Update the time of the state if the time_progress attribute of the given EnumerableAction is True."""
    ...
    def update_res_consumption(self, state: State):
        ...
        for task_id in state.tasks_ongoing:
            resource_to_use = self.get_resource_used(task=task_id,
                                                     mode=state.tasks_mode[task_id],
                                                     resource_unit_names=set(),
                                                     time_since_start=
                                                     state.tasks_details[task_id].get_task_active_time(state.t+1))
            ...
            for completed_task in completed_tmp:
                ...
                for res in next_state.resource_used_for_task[completed_task]:
                    ...
                    if self.is_renewable(res):
                        next_state.resource_used[res] -= res_consumption
                next_state.resource_used_for_task.pop(completed_task)
                next_state.tasks_details[completed_task].end = next_state.t+1  # WARNING : considering how it's coded, we should put +1 here.
                if completed_task in self.get_task_on_completion_added_conditions():
                    all_models[completed_task] = []
                    for i in range(len(self.get_task_on_completion_added_conditions()[completed_task])):
                        for j in range(len(self.get_task_on_completion_added_conditions()[completed_task][i].get_values())):
                            all_values.append({'task': completed_task,
                                               'cond': self.get_task_on_completion_added_conditions()[completed_task][i]
                                              .get_values()[j][0],
                                               'prob': self.get_task_on_completion_added_conditions()[completed_task][i]
                                              .get_values()[j][1]})
    ...
    def update_complete_tasks_simulation(self, state: State):
        ...
        next_state = self.update_complete_tasks_uncertain(
            states=DiscreteDistribution([(next_state, 1.)])).sample()
        return next_state
    ...
    def update_complete_dummy_tasks_uncertain(self, states: DiscreteDistribution[State], action: SchedulingAction):
    ...
    def update_pause_tasks(self, state: State, action: SchedulingAction):
        ...
        if action.action == SchedulingActionEnum.PAUSE:
            ...
            for res in next_state.resource_used_for_task[paused_task]:
                ...
                if self.is_renewable(res) or (not self.is_renewable(res)
    ...
    def update_resume_tasks(self, state: State, action: SchedulingAction):
        ...
        if action.action == SchedulingActionEnum.RESUME:
            ...
            next_state.tasks_details[resumed_task].resumed.append(next_state.t) # Need to call this before get_task_active_time()
            time_since_start = next_state.tasks_details[resumed_task].get_task_active_time(next_state.t)
            b, resource_to_use = self.check_if_action_can_be_started(next_state, action=action)
            if not b:
                return next_state
    ...
    def check_if_action_can_be_started(self, state: State, action: SchedulingAction)->Tuple[bool, Dict[str, int]]:
        """Check if a start or resume action can be applied. It returns a boolean and a dictionary of resources to use.
        """
        if not action.action == SchedulingActionEnum.START and not action.action == SchedulingActionEnum.RESUME:
            return True, {}
        started_task = action.task
        resource_to_use = self.get_resource_used(task=started_task,
                                                 mode=action.mode,
                                                 resource_unit_names=action.resource_unit_names,
                                                 time_since_start=
                                                 state.tasks_details[started_task].get_task_active_time(state.t))
        if any(resource_to_use[r] >
               state.resource_availability[r]-state.resource_used[r]
               for r in resource_to_use):
            return False, resource_to_use
        b = self.check_if_skills_are_fulfilled(task=started_task,
                                               mode=action.mode,
                                               resource_used=resource_to_use)
        return b, resource_to_use

    def get_resource_used(self, task: int, mode: int,
                          resource_unit_names: Set[str],
                          time_since_start: int):
        r_used = {}
        for res in self.get_resource_types_names():
            res_consumption = self.get_tasks_modes()[task][mode] \
                .get_resource_need_at_time(resource_name=res, time=time_since_start)
            # next_state.resource_availability[res] -= res_consumption
    ...
    def update_start_tasks(self, state: State, action: SchedulingAction):
        ...
        if action.action == SchedulingActionEnum.START:
            can_be_started, resource_to_use = self.check_if_action_can_be_started(state, action)
    ...
    def update_start_tasks_uncertain(self, state: State, action: SchedulingAction):
        ...
        if action.action == SchedulingActionEnum.START:
            ...
            for value_duration in duration_distrib.get_values():
                ...
                next_state.tasks_remaining.remove(started_task)
                next_state.tasks_details[started_task].start = next_state.t
                next_state.tasks_details[started_task].sampled_duration = value_duration[0]
                for res in resource_to_use:
                    next_state.resource_used[res] += resource_to_use[res]
                next_state.resource_used_for_task[started_task] = resource_to_use
                next_state.tasks_progress[started_task] = 0.
    ...
    def get_possible_starting_tasks(self, state: State):
        ...
        possible_task_with_ressource = [(n, mode)
                                        ...
                                        if all(state.resource_availability[key] -
                                               ...
                                               self.get_task_modes(n)[mode]
                                               .get_resource_need_at_time(resource_name=key,
                                                                          time=state.t) >= 0
                                               for key in self.get_resource_types_names())]
        # print("Possible task with ressource : ", possible_task_with_ressource)
        return {n: {mode: self.get_task_modes(n)[mode].get_non_zero_ressource_need_names(0)}
    ...
    def state_is_overconsuming(self, state: State):
        return any(state.resource_used[k] > state.resource_availability[k] for k in state.resource_used)
    ...
    def update_resource_availability_uncertain(self, states: DiscreteDistribution[State], action: SchedulingAction):
        """Update resource availability for next time step. This should be called after update_time()."""
        next_states = DiscreteDistribution([(state, prob) for (state, prob) in states.get_values()])
    ...
    def update_conditional_tasks_uncertain(self, states: DiscreteDistribution[State], action: SchedulingAction):
        ...
        if action.time_progress:
            for next_state, _ in next_states.get_values():
                ...
                all_considered_tasks = next_state.tasks_remaining.union(next_state.tasks_ongoing).union(
                    next_state.tasks_progress).union(next_state.tasks_complete)
                new_tasks = all_available_tasks.symmetric_difference(all_considered_tasks)
                for task in new_tasks:
                    next_state.tasks_remaining.add(task)
            return next_states
        return next_states

    def update_conditional_tasks_simulation(self, state: State, action: SchedulingAction):
        ...
        next_state: State = state
        next_state = self.update_conditional_tasks_uncertain(states = DiscreteDistribution([(next_state, 1.)]), action=action).sample()
        return next_state
    ...
    def _get_transition_value(self, memory: D.T_memory[D.T_state], action: D.T_agent[D.T_concurrency[D.T_event]],
        ...
        if SchedulingObjectiveEnum.COST in self.get_objectives():
            ...
            renewable_type_cost_val = 0.
            nonrenewable_type_cost_val = 0.
            for res in self.get_resource_types_names():
                if self.is_renewable(res):
        ...
        weighed_transition_cost = 1. * transition_makespan + 1. * transition_cost
        return Value(cost=weighed_transition_cost)

    def _is_terminal(self, state: D.T_state) -> bool:
        all_task_possible = self.all_tasks_possible(state)
        return state.t > self.get_max_horizon() or (not all_task_possible) \
               or (len(state.tasks_remaining) == 0
                   and (len(state.tasks_ongoing) == 0)
                   and (len(state.tasks_paused) == 0))
...
class UncertainSchedulingDomain(SchedulingDomain,
                                D_uncertain):
    """This is the highest level scheduling domain class (inheriting top-level class for each mandatory
    domain characteristic).
    """
...
class DeterministicSchedulingDomain(SchedulingDomain,
                                    D_det):
    """This is the highest level scheduling domain class (inheriting top-level class for each mandatory
    domain characteristic).
    """
...
class SchedulingActionSpace(EnumerableSpace[SchedulingAction], SamplableSpace[SchedulingAction]):
    ...
    def _get_elements(self) -> Iterable[T]:
        choices = [SchedulingActionEnum.START,
        ...
        for choice in choices:
            if choice == SchedulingActionEnum.START:
                ...
                task_possible_to_start: Dict[int,
                                             Dict[int, List[str]]] = self.domain.get_possible_starting_tasks(self.state)
                list_action += [SchedulingAction(task=t,
            ...
            if choice == SchedulingActionEnum.RESUME:
                ...
                list_action += [SchedulingAction(task=t, action=SchedulingActionEnum.RESUME,
                                                 mode=self.state.tasks_mode[t],
                                                 time_progress=False)
                                for t in task_possible_to_resume]
            if choice == SchedulingActionEnum.TIME_PR:
                list_action.append(SchedulingAction(task=None,
...
class SchedulingActionSpaceWithResourceUnit(EnumerableSpace[SchedulingAction], SamplableSpace[SchedulingAction]):
    def __init__(self, domain: SchedulingDomain, state: State):
        self.domain = domain
        self.state = state
        self.elements = self._get_elements()
        ...
        for choice in choices:
            if choice == SchedulingActionEnum.START:
                ...
                for possible_to_start in task_possible_to_start:
                    for mode in task_possible_to_start[possible_to_start]:
                        possible = self.domain.find_one_ressource_to_do_one_task(task=possible_to_start,
                                                                                 mode=mode)
                        list_action += [SchedulingAction(task=possible_to_start,
                                                         action=SchedulingActionEnum.START,
                                                         mode=mode,
                                                         time_progress=False,
                                                         resource_unit_names={r} if r is not None else None)
                                        for r in possible
                                        if r is None or self.state.resource_used[r] + 1
            ...
            if choice == SchedulingActionEnum.RESUME:
                ...
                for possible_to_resume in task_possible_to_resume:
                    possible = self.domain.find_one_ressource_to_do_one_task(task=possible_to_resume,
                                                                             mode=
                                                                             self.state.tasks_mode[possible_to_resume])

                    list_action += [SchedulingAction(task=possible_to_resume,
                                                     action=SchedulingActionEnum.START,
...
class SchedulingActionSpaceWithResourceUnitSamplable(SamplableSpace[SchedulingAction]):
    ...
    def sample(self) -> T:
        ...
        if random_choice in {SchedulingActionEnum.START,
                             SchedulingActionEnum.RESUME}:
            if random_choice == SchedulingActionEnum.START:
                ...
                task_possible_to_start: Dict[int,
                                             Dict[int, List[str]]] = self.domain.get_possible_resume_tasks(self.state)
            task_modes = [(t, m)
                          for t in task_possible_to_start
                          for m in task_possible_to_start[t]]
            if len(task_modes) > 0:
                ...
                if len(skill_of_task) == 0:
                    return SchedulingAction(task=possible_to_start,
                ...
                for resource in resources_skills:
                    if self.state.resource_used[resource]+1 > self.state.resource_availability[resource]:
                        continue
                    if any(self.domain.get_skills_of_resource(resource=resource).get(s, 0) > 0
                           for s in skill_of_task):
                        resources += [resource]
                        for s in cur_usage:
                            cur_usage[s] += self.domain.get_skills_of_resource(resource=resource).get(s, 0)
        ...
        if random_choice == SchedulingActionEnum.PAUSE:
            task_possible_to_pause = [t for t in self.state.tasks_ongoing
                                      if self.domain.get_task_preemptivity()[t]]
            if len(task_possible_to_pause) > 0:
                return SchedulingAction(task=random.choice(task_possible_to_pause),
                                        action=SchedulingActionEnum.PAUSE,
                                        mode=None,
                                        time_progress=False)
            else:
...
class SingleModeRCPSP(DeterministicSchedulingDomain,
                      SingleMode,
...
class SingleModeRCPSPCalendar(DeterministicSchedulingDomain,
                              SingleMode,
                              DeterministicTaskDuration,
                              DeterministicTaskProgress,
                              WithoutResourceUnit,
...
class MultiModeRCPSP(DeterministicSchedulingDomain,
...
class MultiModeRCPSPWithCost(DeterministicSchedulingDomain,
                             MultiMode,
                             DeterministicTaskDuration,
                             DeterministicTaskProgress,
                             WithoutResourceUnit,
...
class MultiModeRCPSPCalendar(DeterministicSchedulingDomain,
                             MultiMode,  # this changed from Single mode RCPSP
                             DeterministicTaskDuration,
                             DeterministicTaskProgress,
                             WithoutResourceUnit,
...
class MultiModeRCPSP_Stochastic_Durations(UncertainSchedulingDomain,
...
class SingleModeRCPSP_Stochastic_Durations(UncertainSchedulingDomain,
                                           SingleMode,
                                           UncertainUnivariateTaskDuration,  # this changed from Single mode RCPSP
                                           DeterministicTaskProgress,
                                           WithoutResourceUnit,
...
class SingleModeRCPSP_Stochastic_Durations_WithConditionalTasks(UncertainSchedulingDomain,
                                                                SingleMode,
                                                                UncertainUnivariateTaskDuration,  # this changed from Single mode RCPSP
                                                                DeterministicTaskProgress,
                                                                WithoutResourceUnit,
...
class SingleModeRCPSP_Simulated_Stochastic_Durations_WithConditionalTasks(SchedulingDomain,
                                                                          SingleMode,
                                                                          # SimulatedTaskDuration,
                                                                          DeterministicTaskProgress,
                                                                          WithoutResourceUnit,
