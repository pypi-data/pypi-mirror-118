...
class SimpleProcessbar:
    ...
    def step(self, current_index: int, current_str: str = '>', remain_str: str = '.') -> None:
        ...
        :rtype: None
        '''
        if current_index >= self.array_length:
            raise IOError('当前序列索引大于序列总长度')
        ...
        print(f'\r|{current_str * current_length}{remain_str * remain_length}|',  # 进度条
              f'{current_index + 1:{self.array_length_width}}/{self.array_length}',  # 数字进度，current_index/totle_index格式
