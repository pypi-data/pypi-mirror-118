...
scrapeurl="http://52.91.40.112:5000/start_scraper"
...
class scraper():
    ...
    def train(self,*argss):
        ...
        start_time.start()
        res=requests.post(url = trainurl, data = json.dumps({"user":global_user,"pages":argss,"links":links,"is_pypi":True,"bot":"autoscraper"}), headers=headers, timeout=60*60)
        try:
            res=json.loads(res.text)
        except:
            res=res.text
    ...
    def scrape(self,*argss,**kwargs):
        ...
        res=requests.post(url = scrapeurl, data = json.dumps(data_send), headers=headers, timeout=60*60)
        try:
            res=json.loads(res.text)
        except:
            res=res.text
    ...
    def run(self,*argss,**kwargs):
        ...
        res=requests.post(url = runurl, data = json.dumps(args), headers=headers,timeout=60*60)
        try:
            res=json.loads(res.text)
        except:
            res=res.text
