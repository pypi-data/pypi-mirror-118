...
class DatalakeFileSource(iobase.BoundedSource):  # noqa
    ...
    def __get_file_paths_from_folder(folder_path: str, file_system_client: FileSystemClient) -> List[PathProperties]:
        try:
            ...
            for path in paths:
                processed_file = file_system_client.get_file_client(path.name).get_file_properties().metadata
                ...
                if not('status' in processed_file and (processed_file['status'] == 'processed'
                       or processed_file['status'] == 'processing')):
