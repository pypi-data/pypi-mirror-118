...
class RedisRoot:
    ...
    def __init__(self, connection_pool=None, prefix='redis_test', ignore_deserialization_errors=True,
    ...
    def redis_instance(self):
        connection = self.connection_pool.get_conection()
        return connection

    def _get_redis_cache_existing_data(self):
        redis_models_with_redis_dicts = {}
        for redis_model, to_cache_in in self.registered_models.items():
            now = datetime.datetime.now()
            if to_cache_in >= now:
                meta = redis_model.__model_data__['meta']
                cache_conf = {}
                enabled = False
                if 'cache' in meta.keys():
                    cache_conf = meta['cache']
                    if cache_conf['enabled']:
                        enabled = True
                if enabled:
                    ttl = cache_conf['ttl']
                    instances = self._get_all_model_instances(redis_model)
                    redis_models_with_redis_dicts[redis_model] = instances
                    now = datetime.datetime.now()
                    self.registered_models[redis_model] = now + datetime.timedelta(seconds=ttl)
        return redis_models_with_redis_dicts

    def _run_cache(self):
        while True:
            redis_models_with_redis_dicts = self._get_redis_cache_existing_data()
            for redis_model, redis_dicts in redis_models_with_redis_dicts.items():
                meta = redis_model.__model_data__['meta']
                cache_conf = meta['cache']
                django_model = meta['django_model']
                cache_func = cache_conf['cache_func']
                cache_func(redis_model, redis_dicts, django_model)

    def _get_connection_pool(self, connection_pool):
        if isinstance(connection_pool, redis.ConnectionPool):
            connection_pool.connection_kwargs.set('decode_responses', True)
...
class RedisModel:
    ...
    def _check_meta_cache(self, user_cache_conf):
        name = self.__model_data__['name']
        redis_root = self.__model_data__['redis_root']

        def default_cache_func(*args, **kwargs):
            return None
        ...
        cache_conf = {
            ...
            'prefix': f'{redis_root.prefix}-{name}-cache',
        }
        ...
        if type(user_cache_conf) == dict:
            if 'enabled' in user_cache_conf.keys():
                if type(user_cache_conf['enabled']) == bool:
                    ...
                    raise Exception(f'{name} -> Meta -> cache -> enabled must be bool')
            if 'ttl' in user_cache_conf.keys():
                if type(user_cache_conf['ttl']) == int:
                    cache_conf['ttl'] = user_cache_conf['ttl']
                else:
                    raise Exception(f'{name} -> Meta -> cache -> ttl must be int')
            if 'django_model' in user_cache_conf.keys():
                if issubclass(user_cache_conf['django_model'], DjangoModel):
                    ...
                    raise Exception(f'{name} -> Meta -> cache -> django_model must be subclass of django.db.models Model')
            if 'cache_func' in user_cache_conf.keys():
                if type(user_cache_conf['cache_func']) == Callable:
                    ...
                    raise Exception(
                        f'{name} -> Meta -> cache -> cache_func must be callable function')
            if 'prefix' in user_cache_conf.keys():
                if type(user_cache_conf['prefix']) == str:
                    cache_conf['prefix'] = user_cache_conf['prefix']
                else:
                    raise Exception(f'{name} -> Meta -> cache -> prefix must be str')
        else:
            raise Exception(f'{name} -> Meta -> cache must be dict')
    ...
    def __getattr__(self, attr):
        ...
        attr_str = str(attr)
        print(attr_str)
        if attr_str in fields.keys():
            field = fields[attr_str]
            return field.value
        elif attr_str in meta.keys():
            return meta[attr_str]
        else:
            raise Exception(f'{name} has no field {attr_str}')
