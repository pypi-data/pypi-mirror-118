...
# def redis_django_foreign_key_test(connection_pool, prefix):
#     redis_root = RedisRoot(
#         prefix=prefix,
#         connection_pool=connection_pool,
#         ignore_deserialization_errors=True
...
def ttl_test(connection_pool, prefix):
    redis_root = RedisRoot(
        prefix=prefix,
        connection_pool=connection_pool,
        ignore_deserialization_errors=True
...
def async_test(connection_pool, prefix):
    ...
    try:
        ...
        async def async_task(data_count, use_async=True):
            ...
            async def create_list(list_length):
            ...
            async def get_object_existence(lists_with_ids):
                exists = False
            ...
            async def update_list(lists_with_ids):
                result = False
                objects = redis_root.get(ListCheckModel, redis_list=lists_with_ids['list'])
                if objects:
                    if len(objects) == 1:
            ...
            async def get_objects_existence(list_of_lists_with_ids, use_async=True):
                if use_async:
                    async_get_object_by_list_tasks = [
                        get_object_existence(lists_with_ids)
                    ...
                    list_of_results = [
                        await get_object_existence(lists_with_ids)
                        for i, lists_with_ids in enumerate(list_of_lists_with_ids)
                    ]
            ...
            async def delete_lists(list_of_lists_with_ids, use_async=True):
                if use_async:
                    async_delete_list_tasks = [
                        delete_list(lists_with_ids['id'])
                        for i, lists_with_ids in enumerate(list_of_lists_with_ids)
                    ]
                    list_of_results = await asyncio.gather(*async_delete_list_tasks)
                else:
                    list_of_results = [
                        await delete_list(lists_with_ids)
                        for i, lists_with_ids in enumerate(list_of_lists_with_ids)
                    ]
                return list_of_results
