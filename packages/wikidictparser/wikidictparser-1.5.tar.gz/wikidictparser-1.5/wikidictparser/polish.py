...
class PlWiktionaryParser(WiktionaryParserBase):
    ...
    def get_word_data(self):
        ...
        try:
            ...
            logging.warning('Could not parse - returning None')
            return None
    ...
    def parse_numbering(self, number_string, meaning_dict):
        ...
        for unit_number_string in number_string_list:
            ...
            expression_minors2 = r'^(\d+)\.(\d+)\,(\d+)$'  # matches 1.1,3
            # Try out every match and add appriopriate (major, minor) tuple/s
            if re.match(expression_minors2, unit_number_string):
                logging.debug('Expression minors')
                match = re.match(expression_minors2, unit_number_string)
                major = int(match[1])
                minor_1 = int(match[2])
                minor_2 = int(match[3])
                for minor_key in [minor_1, minor_2]:
    ...
    def clean_declination_df(self, df, part_of_speech):
        ...
        if 'rzeczownik' in part_of_speech:
        ...
        if 'przymiotnik' in parts_of_speech or 'zaimek' in parts_of_speech:
            ...
            new_header_mappings = {
                'przypadek': 'przypadek',
                'liczba pojedyncza': 'liczba pojedyncza mos/mzw',
                'liczba pojedyncza.1': 'liczba pojedyncza mrz',
                'liczba pojedyncza.2': 'liczba pojedyncza ż',
                'liczba pojedyncza.3': 'liczba pojedyncza n',
                'liczba mnoga': 'liczba mnoga mos',
                'liczba mnoga.1': 'liczba mnoga nmos'
            }
            new_header = list(
                map(lambda x: new_header_mappings[x], old_columns))
            if df.shape[0] > 11:
                for i in range(13, 20):
                    df.iloc[i, 0] = df.iloc[i, 0]+" stopień wyższy"
                for i in range(24, 31):
                    df.iloc[i, 0] = df.iloc[i, 0]+" stopień najwyższy"
                df = df.iloc[list(range(2, 9)) +
                             list(range(13, 20))+list(range(24, 31))]
            else:
                df = df.iloc[2:-2]
            df.columns = new_header
    ...
    def parse_declination(self):
        ...
        for dd in dds:
            ...
            # if a table found
                ...
                part_of_speech = []
                part_of_speech_major = {-1}
                for number in number_list:
                    if number[0] not in part_of_speech_major:
                        part_of_speech.append(
                            self.language_section_dict['pl']['znaczenia'][number[0]][1]['część_mowy/part_of_speech'])
                        part_of_speech_major.add(number[0])
                ...
                for key1, value1 in declination.items():
                    for key2, value2 in value1.items():
                        # print(value1)
                        value1[key2] = value2.replace(' / ', ',').replace(' /', ',').replace(
                            '/ ', ',').replace('/', ',').replace(', ', ',').split(',')
            elif dd.find('span', {'title': 'stopień wyższy'}):
                logging.debug('przysłówek')
                logging.debug(dd.findAll(text=True, recursive=True))
                declinations_list = dd.findAll(text=True, recursive=True)
                declination = {
                    "st. podstawowy": self.current_word,
                    "st. wyższy": declinations_list[2].strip(' ;'),
                    "st. najwyższy": declinations_list[4].strip(' ;')
                }
            else:
                raise ValueError(
        ...
        # filling with nieodm if empty for a meaning
        for major_key in self.language_section_dict['pl']['znaczenia']:
            if major_key not in declination_dict:
                declination_dict[major_key] = {}
            for minor_key in self.language_section_dict['pl']['znaczenia'][major_key]:
                if minor_key not in declination_dict[major_key].keys():
                    if (minor_key > 1):
                        declination_dict[major_key][minor_key] = declination_dict[major_key][minor_key-1]
                    else:
                        declination_dict[major_key][minor_key] = declination_dict[major_key-1][1]
        # logging.debug(declination_dict)
