...
@report.step('send terminal command: {command}')
...
@report.step("get environment items")
def get_environment_items(key):
    return os.environ.items()


@report.step("get environment variable: {key}")
def get_environment_variable(key):
    return os.environ.get(key)


@report.step("set environment variable: {key}, {value}")
...
@report.step('{step_description}')
def step_data(step_description):
    pass


@report.step("assert validation - {step_description}")
