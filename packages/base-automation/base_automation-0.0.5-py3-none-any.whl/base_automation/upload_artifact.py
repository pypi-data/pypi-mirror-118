...
def upload_pypi_artifact():
    command = f"twine upload -u {shared_utilities.get_environment_variable('pypi-user')} -p {shared_utilities.get_environment_variable('pypi-password')} --repository-url https://upload.pypi.org/legacy/ dist/*"
    shared_utilities.terminal_command(command)
