...
def author_compare(record_1,record_2):
    ...
    print(authors_1["author"])
    print(authors_2["author"])
    ...
    for key in bib_database.entries:
        if key['ENTRYTYPE'] in requiredkeys:
            for entry in RequiredFields[key['ENTRYTYPE']]:
                ...
                if flag == 0:
                    error = error + 1
                    print("{}：{} 条目缺少 {} 域\n".format(key['ID'], key['ENTRYTYPE'], entry))
        else:
            error = error + 1
                ...
                for entry in bib_database.entries[j].keys():
                    if not entry in bib_database.entries[i].keys():
                        ...
                        if not operator.eq(bib_database.entries[i][entry],bib_database.entries[j][entry]):
                            print("{}--{}：题目均为“{}”，“{}”内容存在冲突".format(bib_database.entries[i]['ID'],bib_database.entries[j]['ID'],bib_database.entries[i]['title'],entry))
        if not i in dellist:
...
def Merge(bib_filename_1,bib_filename_2):
    ...
    print("文件“{}.bib”和文件“{}.bib”已合并为文件“{}.bib”".format(bib_filename_1, bib_filename_2, bib_filename_1 + '_' + bib_filename_2))

#必须基本功能5：
def Format(bib_filename):
    """
    ...
    tip = "已将文件{}.bib格式化，生成文件{}_format.bib。".format(bib_filename)
    return tip
    ...
    for key in bib_database.entries:
        try:
            for word in key['title'].split():
                if not operator.eq(word.lower(),'a') and not operator.eq(word.lower(),'the'):
                    key['ID'] = (name(key) + key['year'] + word).lower()
                    break
        except:
            continue
...
#可选高阶功能2：
def bib_database(bib_filename, user_name, passward, database):
    """
    存储各类条目的 'entrytype''ID''title''author''year'
    创建数据库表，并插入.bib文件中相应的内容
    ...
    """
    # 打开数据库连接
    db = MySQLdb.connect("localhost", user_name, passward, database, charset='utf8' )
    # 使用cursor()方法获取操作游标 
    cursor = db.cursor()
    try:
        # 执行sql语句
        cursor.execute("CREATE TABLE bib_database(entrytype varchar(25),ID varchar(32),title varchar(255),author varchar(1024),year varchar(6));")
        # 提交到数据库执行
        db.commit()
    except:
        # 错误回滚
    ...
    with open(bib_filename + '.bib',encoding='utf-8') as bib_file:
            ...
            parser.customization = pretreatment  
            bib_database = bp.load(bib_file,parser = parser)
    sql = "insert into bib_database (`entrytype`, `ID`, `title`, `author`, `year`)values(%s, %s, %s, %s, %s)"
    for key in bib_database.entries:
        if not 'title' in key:
            key['titlt']='None'
        if not 'author' in key:
            key['author']='None'
        if not 'year' in key:
            key['year']= 'None'
        values = (key['ENTRYTYPE'],key['ID'],key['title'],key['author'],key['year'])
        try:
            # 执行sql语句
...
fields_id = ["ID", "ENTRYTYPE", "author", "title", "year", "journal", "volume", "number", "pages", "month", "note"]
fields_len = {"ID":"64", "ENTRYTYPE":"32", "author":"4096", "title":"1024", "year":"32", "journal":"1024", "volume":"256", "number":"128", "pages":"64", "month":"64", "note":"256"}

def database_storage(bib_filename, user_name, passward, database):
    """
    ...
    for i in range(len(fields_id)):
        if i == len(fields_id)-1:
            sql_create = sql_create +  fields_id[i] +  " varchar(%s)"%(fields_len[fields_id[i]])
        else:
            sql_create = sql_create +  fields_id[i] +  " varchar(%s), "%(fields_len[fields_id[i]])
    ...
    for key in bib_database.entries:
        ...
        for i in range(len(fields_id)):
            if i == len(fields_id)-1:
                try:
                    sql_insert = sql_insert + "\"" + key[fields_id[i]] + "\"" + ");"
                except:
...
def database_query(bib_filename, entry, fields, user_name, passward, database):
    """
    ...
    for row in results:
        ...
        if operator.eq(row[fields_id.index("ENTRYTYPE")],entry):
            for field in fields_list:
                if not operator.eq(row[fields_id.index(field)],""):
                    key[field] = row[fields_id.index(field)]
...
if __name__ == "__main__":
    ...
    if function == 1:
        PythonList(str(sys.argv[1]))
    
    elif function == 2:
        CheckIntegrity(str(sys.argv[1]))

    elif function == 3:
        CheckConflicts(str(sys.argv[1]))

    elif function == 4:
        print("请输入需要合并的文件名：")
        bib_filename = str(input())
        Merge(str(sys.argv[1]),bib_filename)
        ...
        print("请输入数据库名称：")
        database = str(input())
        database_storage(str(sys.argv[1]), user_name, passward, database)
    elif function == 8:
        print("请输入需要的条目：")
        entry = str(input())
        print("请输入需要的域（用逗号隔开）：")
        ...
        print("请输入数据库名称：")
        database = str(input())
        database_query(str(sys.argv[1]), entry, fields, user_name, passward, database)
    else:
        print("无此功能！")
...
# 记录不同条目的域
StorageFields = {
    "article" : ["author", "title", "journal", "year", "volume", "number", "pages", "month", "note", "ID"],
    "book" : ["author", "title", "publisher", "year", "volume", "number", "series", "address", "edition", "month", "note", "ID"],
    "inproceedings" : ["author", "title", "booktitle", "year", "editor", "volume", "number", "series", "pages", "address", "month", "organization", "publisher", "note", "ID"],
    "misc" : ["author", "title", "howpublished", "month", "year", "note", "ID"],
    "techreport" : ["author", "title", "institution", "year", "type", "number", "address", "month", "note", "ID"]}
...
def database_storage(bib_filename, user_name, passward, database):
    ...
    cursor = db.cursor()
    # SQL创建和插入语句
    sql_create=["CREATE TABLE database_article(author varchar(1024), title varchar(1024), journal varchar(1024), year varchar(1024), volume varchar(1024), number varchar(1024), pages varchar(1024), month varchar(1024), note varchar(1024),  ID varchar(1024));",
    "CREATE TABLE database_book(author varchar(1024), title varchar(1024), publisher varchar(1024), year varchar(1024), volume varchar(1024), number varchar(1024), series varchar(1024), address varchar(1024), edition varchar(1024), month varchar(1024), note varchar(1024), ID varchar(1024));",
    "CREATE TABLE database_inproceedings(author varchar(1024), title varchar(1024), booktitle varchar(1024), year varchar(1024), editor varchar(1024), volume varchar(1024), number varchar(1024), series varchar(1024), pages varchar(1024), address varchar(1024), month varchar(1024), organization varchar(1024), publisher varchar(1024), note varchar(1024), ID varchar(1024));",
    "CREATE TABLE database_misc(author varchar(1024), title varchar(1024), howpublished varchar(1024), month varchar(1024), year varchar(1024), note varchar(1024), ID varchar(1024));",
    "CREATE TABLE database_techreport(author varchar(1024), title varchar(1024), institution varchar(1024), year varchar(1024), type varchar(1024), number varchar(1024), address varchar(1024), month varchar(1024), note varchar(1024), ID varchar(1024));"]
    sql_insert={"article":"insert into database_article (`author`, `title`, `journal`, `year`, `volume`, `number`, `pages`, `month`, `note`, `ID`)values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                "book":"insert into database_book (`author`, `title`, `publisher`, `year`, `volume`, `number`, `series`, `address`, `edition`, `month`, `note`, `ID`)values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                "inproceedings":"insert into database_inproceedings (`author`, `title`, `booktitle`, `year`, `editor`, `volume`, `number`, `series`, `pages`, `address`, `month`, `organization`, `publisher`, `note`, `ID`)values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                "misc":"insert into database_misc (`author`, `title`, `howpublished`, `month`, `year`, `note`, `ID`)values(%s, %s, %s, %s, %s, %s, %s)",
                "techreport":"insert into database_techreport (`author`, `title`, `institution`, `year`, `type`, `number`, `address`, `month`, `note`, `ID`)values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"}
    # 数据库创建（暂时收录5种）
    cursor.execute("DROP TABLE IF EXISTS database_article")
    cursor.execute("DROP TABLE IF EXISTS database_book")
    cursor.execute("DROP TABLE IF EXISTS database_inproceedings")
    cursor.execute("DROP TABLE IF EXISTS database_misc")
    cursor.execute("DROP TABLE IF EXISTS database_techreport")
    ...
    # 数据库插入
    for key in bib_database.entries:
        if key['ENTRYTYPE'] in StorageFields:
            for data in StorageFields[key['ENTRYTYPE']]:
                if not data in key:
                    key[data]='None'
            if operator.eq(key['ENTRYTYPE'],"article"):
                value_insert = (key["author"], key["title"], key["journal"], key["year"], key["volume"], key["number"], key["pages"], key["month"], key["note"], key["ID"])
            elif operator.eq(key['ENTRYTYPE'],"book") :
                value_insert = (key["author"], key["title"], key["publisher"], key["year"], key["volume"], key["number"], key["series"], key["address"], key["edition"], key["month"], key["note"], key["ID"])
            elif operator.eq(key['ENTRYTYPE'],"inproceedings"):
                value_insert = (key["author"], key["title"], key["booktitle"], key["year"], key["editor"], key["volume"], key["number"], key["series"], key["pages"], key["address"], key["month"], key["organization"], key["publisher"], key["note"], key["ID"])
            elif operator.eq(key['ENTRYTYPE'],"misc") :
                value_insert = (key["author"], key["title"], key["howpublished"], key["month"], key["year"], key["note"], key["ID"])
            else:
                value_insert = (key["author"], key["title"], key["institution"], key["year"], key["type"], key["number"], key["address"], key["month"], key["note"], key["ID"])
            try:
                #执行sql语句
                cursor.execute(sql_insert[key['ENTRYTYPE']],value_insert)
                #提交到数据库执行
                db.commit()
            except:
                db.rollback()
    print("已成功录入文件{}.bib到数据库{}。".format(bib_filename,database))
"""
...
"""def database_query(bib_filename, entry, fields, user_name, passward, database):
    ...
    cursor = db.cursor()
    # SQL查询语句
    sql = "SELECT * FROM database_" + entry +';'
    bib_mysql=[]
    fields_list=fields.split(',')
    try:
        # 执行SQL语句
        ...
        for row in results:
            ...
            key['ENTRYTYPE'] = entry
            for field in fields_list:
                if not operator.eq(row[StorageFields[entry].index(field)],'None'):
                    key[field] = row[StorageFields[entry].index(field)]
            bib_mysql.append(key)
        bib_mysql_database=BibDatabase()
        bib_mysql_database.entries=bib_mysql 
        # 将 BibDatabase 对象转化为.bib文件 
    ...
    # 关闭数据库连接
    db.close()
    tip = "已生成{}.bib文件。".format(bib_filename)
    return tip
"""
