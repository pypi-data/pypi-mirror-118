...
class LaharFlowData(object):
    ...
    def raster(self, filename, var=None, full_domain=False, nodata=np.nan, masked=True, crs=None, store_local=True):
        ...
        if store_local:
            self.rasters[os.path.splitext(filename)] = ds
    ...
    def max_bed_evolution(self, vmin=None, vmax=None):
        last_outfile = self.get_snapshot_files()[-1]
        if os.path.splitext(last_outfile) not in self.rasters.keys():
            data = self.raster(last_outfile)
        else:
            data = self.rasters[last_outfile]
    ...
    def max_depth(self, vmin=None):
        ...
        if 'MaxHeights' not in self.rasters.keys():
            data = self.raster('MaxHeights.txt')
        else:
            data = self.rasters['MaxHeights']
    ...
    def POI_value(self, filename, x, y, var=None, full_domain=False, nodata=np.nan, fill_value=0, interp_method='linear'):
    ...
    def transect_value(self, filename, x, y, var=None, full_domain=False, nodata=np.nan, fill_value=0, interp_method='linear'):
