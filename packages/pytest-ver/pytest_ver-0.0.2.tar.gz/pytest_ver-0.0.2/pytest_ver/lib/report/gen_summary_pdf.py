...
class GenSummaryPdf(GenBasePdf):
    ...
    # -------------------
    def gen(self):
        # TODO add headers; get text from ini.json
        #    'left': [xx]
        #    'center': [xx]
        #    'right': [page #]
        # TODO add footers; get text from ini.json
        #    'left': [xx]
        #    'center': [copyright]
        #    'right': [xx]
    ...
    def _gen_req_summary(self):
        ...
        num_failing = 0
        num_passing = 0
        for reqid, item in self._summary['reqid'].items():
            if item['result'] == 'FAIL':
                num_failing += 1
            else:
                num_passing += 1
    ...
    def _gen_proto_summary(self):
        ...
        num_failing = 0
        num_passing = 0
        for reqid, item in self._summary['protoid'].items():
            if item['result'] == 'FAIL':
                num_failing += 1
            else:
                num_passing += 1
