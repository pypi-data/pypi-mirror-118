...
class RowObject:
    ...
    def row_to_var(self):
        for i in range(len(self.columns)):
            exec(f"self.{self.columns[i]} = value", {'self': self, 'value': self.row_data[i]})
...
class Filter:
    ...
    def filter(self, **kwargs):
        ...
        return Sort_by(self.model_in, self.table_name, self.db_path, self.columns, self.query, self.params)
