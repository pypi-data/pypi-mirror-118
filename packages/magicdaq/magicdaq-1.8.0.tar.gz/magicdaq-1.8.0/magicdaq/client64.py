...
class Client64(object):
    ...
    def __init__(self, module32, host='127.0.0.1', port=None, timeout=10.0, quiet=True,
        ...
        **kwargs
        ...
        #*** Select Pickle Protocol ***
        # We need to select a pickle protocol that is compatible with the user's version of python
        # as well as with the 32bit server .exe (frozen with Python 32 Bit 3.7.4 (supports up to Pickle Protocol 4))
        # We select the highest possible version that will work with the user's instillation of python
        # Allowed versions:
        # https://stackoverflow.com/questions/23582489/python-pickle-protocol-choice

        # Obtain the Version #s for the User's version of Python
        major, minor = sys.version_info.major, sys.version_info.minor

        # If we ever introduce support for Python 2.x someday:
        ...
        if major <= 2:
            py_version_exception_text = 'We detected that you are using Python version: '+str(major)+'.'+str(minor)+'\n'
            py_version_exception_text += 'MagicDAQ may only be used with Python3. Python 2 is not supported.' +'\n'
            py_version_exception_text += 'Please install Python 3 for Windows: ' +'\n'
            py_version_exception_text += 'https://www.python.org/downloads/windows/' +'\n'
            py_version_exception_text += 'You will then need to install the MagicDAQ module using your Python3 instillation' +'\n'
            py_version_exception_text += 'https://magicdaq.github.io/magicdaq_docs/#/Install_MagicDAQ'+'\n'
            py_version_exception_text += 'Feel free to email support at: support@magicdaq.com'
            raise Exception (py_version_exception_text)

        # Determine pickle protocol version
        if (major == 3) and (minor < 4):
            self._pickle_protocol = 3
        else:
            # Protocol 4 is the highest pickle protocol supported by 3.7.4 Python (which is in the frozen 32bit server)
            self._pickle_protocol = 4

            # Someday this might be useful if we start constantly upgrading the server freezing version of Python
    ...
    def request32(self, method32, *args, **kwargs):
        ...
        **kwargs
        ...
        if response.status == ERROR:
            print('WARNING: the MagicDAQ API sent back an ERROR after attempting to process request: '+str(method32))

            # Including an exception here makes it more difficult for the user to determine the true cause of the problem
            # because the 32bit server Python API code will also raise an exception, which will have text that is more specific to the
            # issue that caused the problem

            #raise Server32Error('64bit client has received response.status ERROR from the 32bit server. method32 issued:'+str(method32))
