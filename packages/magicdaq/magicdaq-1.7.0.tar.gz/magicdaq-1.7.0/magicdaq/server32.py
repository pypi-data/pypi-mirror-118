...
class _RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        print('Server32: do_GET called.')

        try:

            print('Server32: self.path: ',self.path)

            if self.path == METADATA:

                print('Server32: in METADATA')

                response = {'path': self.server.path, 'pid': os.getpid()}

                print('Server32: response: ',response)
            ...
            print('Server32: before pickle file open')

            with open(self.server.pickle_path, 'wb') as f:

                print('Server32: inside WITH')

                # Apparently this line fails if the MagicDAQ driver is not installed.
                try:

                    print('Server32: before first pickle dump')
                    print('Server32: pickle protocol: ', self.server.pickle_protocol)

                    pickle.dump(response, f, protocol=self.server.pickle_protocol)

                    print('Server32: after first pickle dump')

                except Exception as exception_text:

                    print('Server32: YIKES! exception!')
