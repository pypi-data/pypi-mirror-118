    ...
    def run(self, incoming, methods, info, bot_info):
        try:
            ...
            if info["command"] == "PRIVMSG" and info['args'][1] == ".bootinfo":
                ...
                methods["send"](info["address"],f"Boot Time: {bt.year}/{bt.month}/{bt.day} {bt.hour}:{bt.minute}:{bt.second}")
            ...
            if info["command"] == "PRIVMSG" and info['args'][1] == ".cpuinfo":
                ...
                # Number of cores.
                methods["send"](info["address"],"Physical cores: {}".format(psutil.cpu_count(logical=False)))
                methods["send"](info["address"],"Total cores: {}".format(psutil.cpu_count(logical=True)))
                # CPU frequencies.
            ...
            if info["command"] == "PRIVMSG" and info['args'][1] == ".networkinfo":
                ...
                for interface_name, interface_addresses in if_addrs.items():
                    for address in interface_addresses:
                       #methods["send"](info["address"],f"=== Interface: {interface_name} ===")
                        if str(address.family) == 'AddressFamily.AF_INET':
                            methods["send"](info["address"],f"IP Address: {address.address}  ||  Netmask: {address.netmask}".format())
                        elif str(address.family) == 'AddressFamily.AF_PACKET':
                            methods["send"](info["address"],f"MAC Address: {address.address}  ||  Netmask: {address.netmask}")
                # Get IO statistics since boot.
                net_io = psutil.net_io_counters()
                methods["send"](info["address"],f"Total Bytes Sent: {self.get_size(net_io.bytes_sent)} ||  Total Bytes Received: {self.get_size(net_io.bytes_recv)}")

            if info["command"] == "PRIVMSG" and info['args'][1] == ".diskinfo":
                # Disk Information
                methods["send"](info["address"],"="*40 + "Disk Information"+ "="*40 + "\nPartitions and Usage:")
                # Get all disk partitions.
                partitions = psutil.disk_partitions()
                for partition in partitions:
                    methods["send"](info["address"],f"=== Device: {partition.device} ===\n  Mountpoint: {partition.mountpoint}\n  File system type: {partition.fstype}")
                    try:
                        ...
                        # isn't ready.
                        continue
                    methods["send"](info["address"],f"  Total Size: {self.get_size(partition_usage.total)}\n  Used: {self.get_size(partition_usage.used)}\n  Free: {self.get_size(partition_usage.free)}\n  Percentage: {partition_usage.percent}%")
                # Get IO statistics since boot.
                disk_io = psutil.disk_io_counters()
                methods["send"](info["address"],f"Total read: {self.get_size(disk_io.read_bytes)}\nTotal write: {self.get_size(disk_io.write_bytes)}")
