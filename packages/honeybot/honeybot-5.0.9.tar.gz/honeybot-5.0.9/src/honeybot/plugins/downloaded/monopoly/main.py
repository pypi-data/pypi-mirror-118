...
class Plugin:
    ...
    def checkWon(methods, info):
        ...
        if len(Plugin.players) == 1 and Plugin.stage > 0:
            ...
            methods["send"](
                info["address"], "The winner of this monopoly game is " + Plugin.winner
            )
            return True
        else:
            return False
    ...
    def count_color_property(portfolio, col):
        ...
        properties = [
            asset for asset in portfolio if isinstance(asset, Property)
        ]  # util and railroad do not have color
        return sum(
            property.color == col for property in properties
        )  # True is equal to 1 in python
    ...
    def count_railroads(portfolio):
        ...
        return len(
            [railroad for railroad in portfolio if isinstance(railroad, Railroad)]
        )
    ...
    def show_rolls(methods, info, player):
        ...
        methods["send"](
            info["address"],
            "Upcoming spaces that " + player.getName() + " could roll to:",
        )
        ...
        for i in range(2, 13):
            ...
            if (
                isinstance(space, Property)
                or isinstance(space, Railroad)
                or isinstance(space, Utility)
            ):
                ...
                if owner_info[0]:
                    if owner_info[1] == player:
                        ...
                        if isinstance(space, Property):
                            message = (
                                message
                                + " and can buy a house for if you"
                        ...
                        message = (
                            ...
                            + " you will end up at "
                            + space.get_name()
                            + " which you is owned by "
                            + owner_info[1].getName()
                    ...
                    message = (
                        ...
                        + " you will end up at "
                        + space.get_name()
                        + " which is unowned and can be bought for"
                        + " "
                        + str(space.price)
                    )
            else:
    ...
    def get_player_info(methods, info, player):
        ...
        if player.getPosition() == 10 and player.imprisoned:
            ...
            methods["send"](
                info["address"], name + " is at the jail but not inside... yet"
            )
        else:
    ...
    def get_info(methods, info):
        ...
        if Plugin.winner != "" and Plugin.game_over:
            methods["send"](
                info["address"], "The game is over and the winner was " + Plugin.winner
            )
        elif len(Plugin.players) > 0:
            for player in Plugin.players:
                Plugin.get_player_info(methods, info, player)
            methods["send"](
                info["address"],
                "It is " + Plugin.players[Plugin.turn].getName() + "'s turn!",
            )
        else:
    ...
    def get_help(methods, info):
        """print help messages"""
        methods["send"](
            info["address"], "'.monopoly create' creates a new game people can join."
        )
        methods["send"](
            info["address"],
            "'.monopoly start' starts the game. People can no longer join.",
        )
        methods["send"](
            info["address"], "'.monopoly roll' rolls two dice and move on the board."
        )
        methods["send"](
            info["address"],
            "'.monopoly buy' buys the property where you are on the board or buy a house for the property.",
        )
        methods["send"](
            info["address"],
            "'.monopoly pass' refuses to buy the property or house for the property.",
        )
        methods["send"](info["address"], "'.monopoly leave' leaves the game")
        methods["send"](
            info["address"], "'.monopoly gameinfo' gets information about each player."
        )
    ...
    def get_rent(asset, asset_owner, move_amount):
        ...
        if isinstance(asset, Property):
            ...
            num_util = len(
                [
                    property
                    for property in asset_owner.getPortfolio()
    ...
    def move_to_space(methods, info, player, space):
        ...
        if (
            player.getPosition() > board_spaces.index(space)
            and player.getPosition() <= 39
        ):
            methods["send"](
                info["address"],
                player.getName() + " passed go moving to " + space.get_name(),
            )
            ...
            methods["send"](
                info["address"],
                player.getName() + " was already at " + space.get_name(),
            )
        else:
            methods["send"](
                info["address"], player.getName() + " moved to " + space.get_name()
            )
            player.setPosition(board_spaces.index(space))

    def get_location_info(
        methods, info, player, location
    ):  # location is a space object
        ...
        if isinstance(location, Property):
            ...
            methods["send"](
                info["address"],
                "Remember that you need a full set to buy houses! Of the other properties in the set:",
            )
            set_others = []
            ...
            for space in set_others:
                ...
                if owner_info[0]:
                    if owner_info[1].getName() == player.getName():
                        methods["send"](
                            info["address"], "You already own " + space.get_name()
                        )
                    else:
                        methods["send"](
                            info["address"],
            ...
            for rail in other_rails:
                ...
                if owner_info[0]:
                    if owner_info[1].getName() == player.getName():
                        methods["send"](
                            info["address"], "You already own " + rail.get_name()
                        )
                    else:
                        methods["send"](
                            info["address"],
                            rail.get_name() + " is owned by " + owner_info[1].getName(),
                        )
                else:
            ...
            if owner_info[0]:
                if owner_info[1].getName() == player.getName():
                    methods["send"](
                        info["address"], "You already own " + other_util.get_name()
                    )
                else:
                    methods["send"](
                        info["address"],
            ...
            methods["send"](
                info["address"], "No special information can be given for this space!"
            )
    ...
    def land_unowned_property(methods, info, player, space):
        ...
        methods["send"](
            info["address"],
            space.get_name()
            + " is unowned and may be"
            + " bought for "
            + str(space.price)
            + ". Enter '.monopoly buy' if you "
    ...
    def community_card(methods, info, player):
        ...
        methods["send"](
            info["address"],
            player.getName() + " is picking up a community card, it says:",
        )
        card = community_deck.pop(
            0
        )  # take first card, returns a dict {"message":[int,(tuple)]}
    ...
    def chance_card(methods, info, player):
        ...
        methods["send"](
            info["address"], player.getName() + " is picking up a chance card, it says:"
        )
        card = chance_deck.pop(
            0
        )  # take first card, returns a dict {"message":[int,(tuple)]}
        card_string = next(iter(card))
        methods["send"](info["address"], card_string)
    ...
    def earn(methods, info, player, amount):
        ...
        methods["send"](
            info["address"], player.getName() + " now has a pot of " + player.getPot()
        )
        Plugin.next_turn()
    ...
    def jail(methods, info, player):
        ...
        if player.get_outta_jail_card:
            methods["send"](
                info["address"],
                player.getName()
                + " is at jail but used his get out of jail free card!",
            )
            player.setPosition(10)
            Plugin.next_turn()
        else:
            methods["send"](
                info["address"], player.getName() + " has been sent to jail!"
            )
            player.setPosition(10)
    ...
    def fine(methods, info, player, amount):
        ...
        if player_alive:
            ...
            methods["send"](
                ...
                + " was forced to pay "
                + str(amount)
                + " on and now has no more money so has left the game",
            )
            Plugin.leave(player.getName())
            Plugin.checkWon(methods, info)
        Plugin.next_turn()
    ...
    def repairs(methods, info, player, house_fee, hotel_fee):
        ...
        property_list = [
            asset for asset in player.getPortfolio() if isinstance(asset, Property)
        ]
        hotel_total = 0
        house_total = 0
    ...
    def pay_all(methods, info, player, amount):
        ...
        if player_alive:
            ...
            methods["send"](
                ...
                + " was forced to pay "
                + str(amount)
                + " on repairs and now has no more money so has left the game",
            )
            Plugin.leave(player.getName())
            Plugin.checkWon(methods, info)
    ...
    def reading_rail(methods, info, player):
        ...
        if owner_info[0]:
            ...
            if owner_info[1] == player:
                methods["send"](
                    info["address"],
                    "You own Reading Railroad, but "
                    + "can not buy a house as railroads do not have houses!",
                )
                Plugin.next_turn()
            else:
                owner_railroads = Plugin.count_player_properties(owner_info[1])[
                    "Railroad"
                ]
                rent = reading_rail.rents[owner_railroads]
                methods["send"](
                    info["address"],
                    "Reading Railroad is owned by"
                    + owner_info[1].getName()
                    + " and so "
                    + player.getName()
                    + " must pay them "
    ...
    def util(methods, info, player):
        ...
        if owner_info[0]:
            if owner_info[1] == player:
                methods["send"](
                    info["address"],
                    player.getName() + " has landed on his own utility.",
                )
                Plugin.next_turn()
            else:
                methods["send"](
                    ...
                    + " has landed on "
                    + owner_info[1].getName()
                    + "'s utility and must pay ten times whatever he rolls now with two dice!",
                )
                total = random.randint(1, 6) + random.randint(1, 6)
                methods[send](
                    info["address"],
                    player.getName() + " rolled a total of " + str(total),
                )
    ...
    def move_back_three(methods, info, player):
        ...
        if player.getPosition == 22:
            ...
            methods["send"](
                info["address"], "You moved back to the New York Ave. space..."
            )
            ...
            methods["send"](
                info["address"], "You moved back to the community card space..."
            )
    ...
    def create(methods, info, name):
        ...
        if Plugin.create_req:
            ...
            methods["send"](
                info["address"], Plugin.join(name)
            )  # automatically adds the creator
            methods["send"](
                info["address"],
                "to join this monopoly game " + "enter '.monopoly join'",
            )
        else:
    ...
    def join(nickname):
        ...
        if Plugin.stage == 0 and Plugin.start_join_req:
            ...
            return (
                "a game has not yet started, use '.monopoly create'"
    ...
    def roll(methods, info):
        ...
        if Plugin.roll_req and not Plugin.game_over:
            ...
            if (
                not player.imprisoned or player.prison_time == 2
            ):  # if user will move for sure
                roll1 = random.randint(1, 6)
                roll2 = random.randint(1, 6)
                if (
                    player.prison_time == 2 and player.imprisoned
                ):  # if this is the users last time in prison
                    methods["send"](
                        info["address"],
                        "You are in prison, this is your last "
                    ...
                    if roll1 == roll2:
                        methods["send"](
                            info["address"],
                            "You rolled a " + str(roll1) + " twice and so are free!",
                        )
                    else:
                        methods["send"](
                            info["address"],
                            "You did not roll a double so have to pay a fee to leave!",
                        )
                ...
                if (
                    isinstance(new_location, Property)
                    or isinstance(new_location, Railroad)
                ) or isinstance(new_location, Utility):
                    ...
                    if new_location_owned[0]:  ##if property is owned by somebody
                        ...
                        # buy
                        Plugin.land_unowned_property(
                            methods, info, player, new_location
                        )

                elif (
                    player.getPosition() == 2
                    or player.getPosition() == 17
                    or player.getPosition() == 33
                ):
                    Plugin.community_card(methods, info, player)

                elif (
                    player.getPosition() == 7
                    or player.getPosition() == 22
                    or player.getPosition() == 36
                ):
                    ...
                    if player.get_outta_jail_card:
                        ...
                        methods["send"](
                            info["address"],
                            name
                            + " landed on go to jail but used his get out of jail card...",
                        )
                        Plugin.next_turn()
                    else:
                        player.setPosition(10)
                ...
                methods["send"](
                    info["address"],
                    "you are in jail and will not move unless you roll a double!",
                )
                roll1 = random.randint(1, 6)
                roll2 = random.randint(1, 6)
                if roll1 == roll2:
            ...
            if Plugin.stage == None:
                ...
                for player in Plugin.players:
                    if player.getName() == player_name:  # we should remove the player
                        if Plugin.turn == (
                            len(Plugin.players) - 1
                        ):  # check if there will be an index error when user removed
                            Plugin.turn -= 1
            ...
            if owner_info[0]:
                if owner_info[1].getName() == player.getName():  # if owner is user
                    ...
                    if int(player.getPot()) > property.house_cost:
                        if (
                            Plugin.count_player_properties(player)[property.color]
                            == Property.set_houses[property.color]
                        ):  # if full set
                            ...
                            methods["send"](
                                ...
                                + " for "
                                + property.get_name()
                                + " so if you land there now you have to pay "
                                + str(property.rents[property.house_count]),
                            )
                            Plugin.next_turn()
                        else:
                            methods["send"](
                                info["address"], "you need a full set to buy a house"
                            )
                            Plugin.next_turn()
                    else:
                        methods["send"](
                            info["address"],
                            "you do not have enough money to buy a house",
                        )
                        Plugin.next_turn()
            else:
                if int(player.getPot()) > property.price:
                    ...
                    if isinstance(property, Property) or isinstance(property, Railroad):
                        methods["send"](
                            ...
                            + " for "
                            + str(property.price)
                            + " and if you land there you will have to pay "
                            + str(Plugin.get_rent(property, player, 0)),
                        )
                        Plugin.next_turn()
                    elif isinstance(property, Utility):
                        if Plugin.count_player_properties(player)["utility"] == 1:
                            methods["send"](
                                info["address"],
                                player.getName()
                                + "bought "
                                ...
                                + " for "
                                + str(property.price)
                                + " and if you land there you will have to pay four times whatever you roll!",
                            )
                        else:
                            methods["send"](
                                info["address"],
                                ...
                                + " for "
                                + str(property.price)
                                + " and if you land there you will have to pay ten times whatever you roll!",
                            )
                else:
                    methods["send"](
                        info["address"],
                        "You do not have enough money to buy" + property.get_name(),
                    )
                    Plugin.next_turn()
        else:
        ...
        try:
            if info["command"] == "PRIVMSG":
                ...
                if msgs[0] == ".monopoly":
                    ...
                    if not (
                        Plugin.checkWon(methods, info)
                    ):  # if Plugin.checkWon evaluates to true it will print winner already
                        if len(msgs) == 1:
                            methods["send"](
                                info["address"],
                                ".monopoly requires an argument:"
                            ...
                            if msgs[1] == "info":
                                if msgs[2] == "rolls":
                                    ...
                                    if isinstance(player, Player):
                                        ...
                                        methods["send"](
                                            info["address"], "You are not playing!"
                                        )
                                    ...
                                    for space in board_spaces:
                                        if space.get_name() == " ".join(msgs[2:]):
                                            ...
                                            if isinstance(
                                                ...
                                                Plugin.get_location_info(
                                                    methods, info, pl, space
                                                )
                                                break
                                            else:
                                                methods["send"](
                                                    info["address"],
                                                    "You are not playing and so can not request this information!",
                                                )
                                    else:
                                    ...
                                    if prop == False:
                                        ...
                                        if not found:
                                            methods["send"](
                                                info["address"], "Invalid command"
                                            )

                            else:
                                methods["send"](
                                    info["address"],
                                    "Only the info command allows"
                                    + " for a third argument which can be rolls, a player name or"
                                    + " a property name. Try '.monopoly info rolls'",
                                )
                        ...
                        # following commands can be executed by anyone
                            ...
                            Plugin.stage == 1
                        ):  # following commands only executable if the game has started
                            if len(Plugin.players) > 0:
                                if (
                                    name == Plugin.players[Plugin.turn].getName()
                                ):  # check the correct player issued the command
                                    ...
                                    if msgs[1].lower() == "roll":
                                        ...
                                        methods["send"](
                                            info["address"],
                                            ".monopoly requires one argument:"
                                            + " create, join, start, roll, buy, pass, info, help or leave",
                                        )
                                else:
                                    methods["send"](
                                        info["address"], "It is not your turn!"
                                    )

                        elif (
                            msgs[1].lower() == "roll"
                            or msgs[1].lower() == "buy"
                            or msgs[1].lower() == "pass"
                        ):
                            methods["send"](
                                info["address"],
                                "Invalid command at this time, wait for game to start!",
                            )
