    ...
    def run(self, incoming, methods, info, bot_info):
        try:
            ...
            if info["command"] == "PRIVMSG" and msgs[0] == ".horoscope":
                ...
                if (
                    len(msgs) > 3
                ):  # the message should only contain .horoscope and the user's star sign
                    ...
                    if month.lower() == "december" or month == "12":
                        if is_date_valid(12, day):
                            ...
                            methods["send"](
                                info["address"], Plugin.horoscope(astro_sign)
                            )
                        else:
                        ...
                        methods["send"](
                            info["address"], ".horoscope requires a valid month"
                        )
                elif len(msgs) == 2:
                    starsign = msgs[1]
                    ...
                    methods["send"](
                        info["address"],
                        ".horoscope requires a star sign or day and month",
                    )
