...
class Queues():
    ...
    def __messagequeue():
        # time.sleep(0.01)
        kcwsqlite.sqlite().connect(queuesdbpath).table("model_kcweb_queues").where("code=2").delete()
        kcwsqlite.sqlite().connect(queuesdbpath).table("model_kcweb_queues").where("code = 3 and updtime<"+str(int(time.time())-600)).delete()
        while True:
