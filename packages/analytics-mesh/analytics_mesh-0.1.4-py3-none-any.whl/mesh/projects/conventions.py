...
class SimplePath(PathConventions):
    ...
    def __init__(self,
        ...
        margs = {k: str(v) for k, v in local_vars.items() if v is not None}
        log.debug(f'filtered args: {set(local_vars.keys()).difference(set(margs.keys()))}')
        self.values = [margs[x] for x in self.keys if x in margs.keys()]
