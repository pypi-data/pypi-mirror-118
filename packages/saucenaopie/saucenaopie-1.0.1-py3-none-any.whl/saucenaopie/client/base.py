...
from ..types import SauceResponse, AccountInfo, Header, SauceResult, ResultIndex
from ..types.sauce import MangaSauce, ArtSauce, VideoSauce, TwitterSauce, BooruSauce
from ..helper import Helper, SauceIndex, AccountType
from ..exceptions import (
    BadAPIKey, FileIsTooLarge, ShortLimitReached, LongLimitReached, ImageInvalid,
    UnknownServerError, UnknownClientError, AccountBanned, TooManyFailedRequests,
)
...
class BaseSauceClient(ABC):
    def __init__(self, api_key: str, test_mode: bool = False) -> None:
        ...
        :param test_mode: Makes the API return at least 1 result to (almost) every search query for testing purposes
        """
    ...
    def search(
        ...
        """
        Perform a search with SauceNao. You can provide a file path, BytesIO or URL (along with the from_url argument).

        :param file: File path / BytesIO / URL
        :param index: SauceNao database index to search in, look at :class:`saucenaopie.helper.DBIndex`
        ...
        :param result_limit: Limit the number of results, 8 is API default. WARNING: They aren't sorted by similarity!
    ...
    def _process_response(self, response: httpx.Response) -> SauceResponse:
        try:
            ...
            if error.response.status_code == 403:
                ...
                if 'Daily' in header["message"]:
                    raise LongLimitReached(
                        "Daily limit reached.",
                        long_remaining=header["long_remaining"], short_remaining=header["short_remaining"],
                        long_limit=header["long_limit"], short_limit=header["short_limit"],
                    )
            ...
            raise UnknownServerError("Server returned unknown error.", status_code=error.response.status_code)
    ...
    def _parse_response_data(self, data: dict) -> SauceResponse:
        ...
        return SauceResponse(
            account_info=AccountInfo(
                user_id=header["user_id"], short_limit=header["short_limit"], long_limit=header["long_limit"],
                long_remaining=header["long_remaining"], short_remaining=header["short_remaining"],
                account_type=AccountType.get_value_name(int(header["account_type"])).lower()
            ),
            header=Header(
                results_requested=header["results_requested"], search_depth=header["search_depth"],
                min_similarity=header["minimum_similarity"], results_returned=header["results_returned"],
            ),
    ...
    def _result_to_object(self, result: dict) -> SauceResult:
        ...
        if header["index_id"] in SauceIndex.get_video_indexes():
            sauce = VideoSauce(
                urls=urls, title=title, episode=data.get("part"), year=data.get("year"), timestamp=data.get("est_time")
            )
        elif header["index_id"] in SauceIndex.get_manga_indexes():
            sauce = MangaSauce(urls=urls, title=title, chapter=data.get("part"), author=self._get_author(data))
        elif header["index_id"] in SauceIndex.get_booru_indexes():
            sauce = BooruSauce(
                urls=urls, title=title, gelbooru_id=data.get("gelbooru_id"), danbooru_id=data.get("danbooru_id"),
                characters=data.get("character"), material=data.get("material"), source_url=data.get("source")
            )
        elif header["index_id"] == SauceIndex.TWITTER:
            sauce = TwitterSauce(
                urls=urls, title=title, tweet_id=data['tweet_id'], user_id=data['twitter_user_id'],
            ...
            if header["index_id"] in {SauceIndex.PIXIV, SauceIndex.PIXIV_HISTORICAL}:
                ...
                author_url = (
                    f"https://web.archive.org/web/http://www.portalgraphics.net/pg/profile/?user_id={author_id}"
                )
            ...
            sauce = ArtSauce(
                urls=urls, title=title, author=self._get_author(data), author_url=author_url
            )

        return SauceResult[type(sauce)](
            similarity=header["similarity"], thumbnail=header["thumbnail"], data=sauce,
            index=ResultIndex(
                id=header["index_id"], name=SauceIndex.get_value_name(header["index_id"], human_readable=True)
            )
        )
    ...
    def _get_author(result_data: dict) -> Optional[str]:
        keys = (
            "author", "author_name", "member_name", "creator", "pawoo_user_display_name", "company"
        )
        for key in keys:
            if value := result_data.get(key):
                return value
