...
@LastEditTime: 2021-08-31 09:08:45
...
class QueueUpHelper:
    """
    :description: 排队系统帮助类 提供加入排队、退出排队、查询单个排队情况、批量查询排队情况、更新可操作时间、签到、清空队列等功能
    """
    ...
    @classmethod
    def _delete_user(self, queue_name):
        """
        ...
        if SevenHelper.is_continue_request(f"queue_delete_user:{queue_name}", 500) == True:
    ...
    @classmethod
    def _delete_and_notice_user(self, queue_name):
        """
        ...
        :last_editors: HuangJianYi
        """
        invoke_result_data = self._delete_user(queue_name)
        if invoke_result_data.error_code == "del":
            self._delete_user(queue_name)
    ...
    def clear(self, queue_name):
        ...
        try:
            ...
            if zset_result > 0 and hash_result > 0:
                return invoke_result_data
            else:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
    ...
    @classmethod
    def queue(self, queue_name, user_id, user_nick="", queue_length=10):
        """
        :description: 加入排队
        :param act_id：活动标识
        :param queue_name：队列名称
        ...
        invoke_result_data = InvokeResultData()
        try:
            acquire_lock_name = f"queueup_queue:{queue_name}"
            ...
            if not redis_init.zscore(zset_name, data):
                if self._get_queue_num(queue_name) >= queue_length:
                    ...
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "limit"
                    invoke_result_data.error_message = "当前排队人数已达上限"
                    return invoke_result_data
                ...
                result_data["queue_num"] = self._get_queue_num(queue_name)  #当前排队人数
    ...
    @classmethod
    def query(self, queue_name, user_id):
        """
        ...
        try:
            ...
            self._delete_and_notice_user(queue_name)
            ...
            if score:
                ...
                query_user["before_num"] = self._get_queue_num(queue_name)  #排在前面的人数
                invoke_result_data.data = query_user
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
    ...
    @classmethod
    def muti_user_query(self, user_id):
        """
        :description: 批量查询某用户排队情况
        :param user_id：用户标识
        :return: 
        :last_editors: HuangJianYi
        """
        ...
        try:
            ...
            for item in match_result:
                ...
                query_invoke_result_data = self.query(hash_value["queue_name"], user_id)
    ...
    @classmethod
    def muti_queue_query(self, queue_name_list, user_id):
        """
        ...
        try:
            for queue_name in queue_name_list:
                query_invoke_result_data = self.query(queue_name, user_id)
                invoke_result_data.data.append(query_invoke_result_data.data)
            return invoke_result_data
        except Exception as ex:
    ...
    @classmethod
    def sign(self, queue_name, user_id, quit_other_queue=True):
        """
