...
@LastEditTime: 2021-08-26 10:30:10
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_client_task_list(self,app_id,act_id,module_id,user_id,task_types="",app_key="",app_secret="",is_log=False,task_sub_table=None,):
        """
        :description: 获取任务列表
        :param app_id:应用标识
        :param act_id:活动标识
        ...
        for task_info_dict in task_info_list:
            ...
            if task_info_dict["task_type"] == TaskType.free_gift.value:
                result["content"] = ""
                status = 1 if task_count.complete_count > 0 else 0
                result["text"] = ["领取", "已领取"]
            ...
            #邀请新用户
                ...
                limit_num = int(config_json["limit_num"]) if config_json.__contains__("limit_num") else 0
            ...
            #加入会员
                ...
                if not user_info_dict:
                    ...
                    if top_base_model.check_is_member(access_token,app_key,app_secret,is_log) == True:
                        ...
                        result["status"] = 0
                    invoke_result_data = top_base_model.get_join_member_url(access_token,app_key,app_secret,is_log)
                result["content"] = ""
                result["join_member_url"] = invoke_result_data.data if invoke_result_data.success == True else ""
    ...
    def process_receive_reward(self,app_id,act_id,module_id,user_id,login_token,task_id,task_sub_type,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None,asset_sub_table=None,invite_sub_table=None,collect_sub_table=None,browse_sub_table=None):
        """
        ...
        :param collect_sub_table:收藏分表名称
        :param browse_sub_table:浏览分表名称
        :return 
        :last_editors: HuangJianYi
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    task_count_id = self._get_task_count_id(act_id,module_id,task_type,task_sub_type,user_id)
                    task_count = task_count_model.get_entity_by_id(task_count_id)
    ...
    def process_free_gift(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None,asset_sub_table=None):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"",only_id,handler_name,request_code,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
    ...
    def process_one_sign(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None,asset_sub_table=None,stat_sub_table=None):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"签到1天",only_id,handler_name,request_code,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        task_count.id = task_count_id
                        task_count.app_id = app_id
                        ...
                        if reward_value > 0:
                            ...
                            stat_base_model.add_stat_list(app_id, act_id, module_id, user_info_dict["user_id"], user_info_dict["open_id"], key_list_dict,stat_sub_table)
                else:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = task_invoke_result_data.error_code
                    invoke_result_data.error_message = task_invoke_result_data.error_message
    ...
    def process_weekly_sign(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_new_user=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None,asset_sub_table=None,stat_sub_table=None):
        """
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],f"签到{task_count.complete_count}天",only_id,handler_name,request_code,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        invoke_result_data.data = reward_value
                        if reward_value > 0:
            ...
            if invoke_result_data.success == True:
                ...
                if is_invite > 0:
                    ...
                    if task_invoke_result_data.success == True:
                        task_info_dict = task_invoke_result_data.data
                        config_json = task_info_dict["config_json"]
                        limit_num = config_json["limit_num"] if config_json.__contains__("limit_num") else 0
                        satisfy_num = int(config_json["satisfy_num"]) if config_json.__contains__("satisfy_num") else 1
                        task_count_model = TaskCountModel(context=self.context,sub_table=task_sub_table)
    ...
    def process_collect_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5,task_sub_table=None,collect_sub_table=None):
        """
    ...
    def process_browse_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5,task_sub_table=None,browse_sub_table=None):
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                is_browse = browse_log_model.get_cache_total("act_id=%s and user_id=%s and goods_id=%s", params=[act_id, user_id, goods_id])
                if is_browse > 0:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
    ...
    def process_favor_store(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_user_nick=True,continue_request_expire=5,asset_sub_table=None,stat_sub_table=None):
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if user_info_dict["is_favor"] ==1:
                    ...
                    UserBaseModel(context=self.context)._delete_user_info_cache(user_info_dict["act_id"],user_info_dict["id"])
                    task_invoke_result_data = self.check_task_info(act_id,module_id,task_type)
                    if task_invoke_result_data.success == True:
                        task_info_dict = task_invoke_result_data.data
                        config_json = task_info_dict["config_json"]
    ...
    def process_join_member(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_code,check_user_nick=True,continue_request_expire=5,asset_sub_table=None,stat_sub_table=None):
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if user_info_dict["is_member_before"] == 0:
                    #新增入会用户数
                    stat_base_model.add_stat(app_id, act_id, module_id, user_info_dict["user_id"], user_info_dict["open_id"], "AddMemberUserCount",1,stat_sub_table)
