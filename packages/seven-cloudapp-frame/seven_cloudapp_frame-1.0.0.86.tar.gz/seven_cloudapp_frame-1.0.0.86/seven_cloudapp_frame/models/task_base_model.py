...
@LastEditTime: 2021-09-02 15:30:41
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_client_task_list(self,app_id,act_id,module_id,user_id,task_types="",app_key="",app_secret="",is_log=False):
        ...
        for task_info_dict in task_info_list:
            ...
            if task_info_dict["task_type"] == TaskType.free_gift.value:
                ...
                status = 1 if task_count.complete_count >= limit_num else 0
                result["text"] = ["领取", "已领取"]
    ...
    # def process_browse_site(self,app_id,act_id,module_id,user_id,login_token,task_sub_type,handler_name,check_user_nick=True,continue_request_expire=5):
    #     """
    ...
    #     :last_editors: HuangJianYi
    #     """
    #     acquire_lock_name = f"process_browse_site:{act_id}_{module_id}_{user_id}"
    #     identifier = ""
    #     task_type = TaskType.browse_goods.value
    #     now_day = SevenHelper.get_now_day_int()
    #     now_datetime =  SevenHelper.get_now_datetime()
    ...
    #             if task_invoke_result_data.success == True:
    #                 task_info_dict = task_invoke_result_data.data
    #                 config_json = [sub_config for sub_config in task_info_dict["config_json"] if task_sub_type == sub_config["id"]]
    #                 if config_json:
    #                     limit_num = 1
    #                     satisfy_num = 1
    #                     task_sub_table = SevenHelper.get_sub_table(act_id,config.get_value("task_sub_table_count",0))
    #                     task_count_model = TaskCountModel(context=self.context,sub_table=task_sub_table)
    #                     task_count_id = self._get_task_count_id(act_id,module_id,task_info_dict["task_type"],task_sub_type,user_id)
    #                     task_count = task_count_model.get_entity_by_id(task_count_id)
    #                     task_count = TaskCount() if not task_count else task_count
    #                     if task_info_dict["complete_type"] == 1:
    #                         limit_meaasge = "今日浏览上限"
