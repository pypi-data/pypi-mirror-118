...
@LastEditTime: 2021-08-18 18:10:06
...
class AddActInfoHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = act_base_model.add_act_info(app_id, act_name,act_type,theme_id,"","",close_word)
...
class UpdateActInfoHandler(TaoBaseHandler):
    ...
    def post_async(self):
        ...
        share_desc_json = self.get_param("share_desc_json", "-1")
        is_rule = int(self.get_param("is_rule", -1))
        rule_desc_json = self.get_param("rule_desc_json", "-1")
...
class NextProgressHandler(TaoBaseHandler):
    ...
    :description: 下一步配置
    """
    @filter_check_params("app_id,act_id,finish_key")
    def get_async(self):
        """
        :description: 下一步配置
        :param app_id：应用标识
