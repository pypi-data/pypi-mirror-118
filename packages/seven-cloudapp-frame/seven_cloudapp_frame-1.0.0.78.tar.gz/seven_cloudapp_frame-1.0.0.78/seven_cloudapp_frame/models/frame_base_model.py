...
@LastEditTime: 2021-09-01 09:08:34
...
class FrameBaseModel():
    ...
    def business_process_executing(self,app_id,act_id,module_id,user_id,login_token,handler_name,check_new_user=False,check_user_nick=True,continue_request_expire=5,acquire_lock_name=""):
        ...
        if module_id > 0:
            module_base_model = ModuleBaseModel(context=self.context)
            act_module_dict = module_base_model.get_act_module_dict(module_id)
            if not act_module_dict:
                invoke_result_data.success = False
    ...
    def get_sub_table(self,act_id,sub_count=0):
        """
        :description: 获取分表名称
        :param act_id:活动标识
        ...
        if not sub_count:
            return sub_table
        sub_table = str(act_id % sub_count)
        return sub_table
