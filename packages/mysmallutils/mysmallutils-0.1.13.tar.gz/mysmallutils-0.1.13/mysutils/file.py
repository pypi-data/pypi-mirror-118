...
class Removable(PathLike):
    ...
    def __fspath__(self):
        if len(self.__files) != 1:
            raise ValueError('To use this class as a PathLike instance it is necessary to have just exactly one file.')
...
def removable_tmp(folder: bool = False, suffix: str = None, prefix: str=None) -> PathLike:
    """ This function is used with "with" python command. As following:
    ...
    :return: A object with enter and exit methods.
    """
    tmp = mkdtemp(suffix=suffix, prefix=prefix) if folder else mktemp(suffix=suffix, prefix=prefix)
