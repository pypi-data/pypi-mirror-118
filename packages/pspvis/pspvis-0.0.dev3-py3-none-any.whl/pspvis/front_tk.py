...
class ToolBar(ttk.Frame):
    ...
    """
    def __init__(self, master: 'TkUI', comm: Callable, *args, **kwargs):
        ...
        self.search_field.grid(sticky=tk.EW, row=1, column=1)
        re_check.grid(sticky=tk.EW, row=1, column=2)
        search_btn.grid(sticky=tk.EW, row=1, column=3)
...
class TkUI(ThemedTk):
    ...
    def set_plot(self, **settings: Any):
        ...
        progress.done()
        progress.destroy()
        self.search_bar = ToolBar(self, self.plot.ui_comm)
    ...
    def refresh_central(self, inherit: bool = True, **kwargs):
        """
    ...
    def _spot_mod(self, increment: int = None):
        if increment is None:
            ...
            if not (0 <= new_size <= 300):
                return
        self.search_bar.user_vars['spot_size'].set(new_size)
    ...
    def _set_shortcuts(self):
        ...
        self.bind('<Control-q>',
                  lambda *_: self.refresh_central(inherit=False))
