...
class CTool():
    ...
    # 水体提取
    @classmethod
    def extract(cls, tifDir, sampleType, tempDir, outDir, type):
        print(f"tif:{tifDir},sampleType:{sampleType},tempDir:{tempDir},outDir:{outDir},type:{type}")
        if type == "sentinel2" or type == "GF1_wfv":
            ...
            for idx, folder in enumerate(classs):
                ...
                path1 = WaterExtract.band_merge(ori_landsat, outDir)
                WaterExtract.landset8(path1, sampleType,  tempDir, outDir)
            ...
            if not os.path.exists(tempDir):
                ...
                shutil.rmtree(tempDir)
                os.mkdir(tempDir)
            WaterExtract.GF1(tifDir, sampleType, tempDir, outDir)
