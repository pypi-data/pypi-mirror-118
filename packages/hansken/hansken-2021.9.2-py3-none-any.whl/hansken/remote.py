...
class Connection:
    ...
    def unique_values(self, project_id, select, query=None, after=None, count=None, incomplete_results=None,
        ...
        # TODO HANSKENPY-10209: Below code is a duplicate from search, extract function and apply to other places where
        #  response.raw is returned.
        if _expect_ok(response).headers.get('transfer-encoding') == 'chunked':
            # avoid handing over the raw file descriptor when the response claims chunked transfer encoding
            # dealing with the actual chunked content is handled by iter_content
            return ChunkedIO(response.iter_content(64 << 10))
            ...
            # TODO HANSKENPY-10209: Decode gzip instead of throwing an error.
            if _expect_ok(response).headers.get('content-encoding') == 'gzip':
                raise ValueError("Could not decode gzipped response.")
            # assume raw content
            return response.raw

    def create_search_request(self, query=None, *,
                              start=0, count=None, sort=None, facets=None, snippets=None, tracelet_type=None,
                              project_ids=None, select=select_all, incomplete_results=None, timeout=None):
        """
