...
from tests.fixtures.models import tabular_batch_model, INT_INPUT, FLOAT_INPUT, PRED, GROUND_TRUTH, TABULAR_MODEL_ID
...
class TestBulkInferences:
    ...
    @responses.activate
    def test_send_set_reference_data(self, tabular_batch_model):
        self._mock_sending_reference_set(TABULAR_MODEL_ID)
        resp, resp_close = tabular_batch_model.set_reference_data(data=pd.DataFrame(
            {**self.INFERENCE_DATA, GROUND_TRUTH: self.GROUND_TRUTH_VALUES}))
        assert resp["counts"]["failure"] == 1
        assert resp_close is None

    @responses.activate
    def test_send_bulk_inferences(self, tabular_batch_model):
        self._mock_batch_upload(TABULAR_MODEL_ID, 'batch_1')
        resp, resp_close = tabular_batch_model.send_bulk_inferences(
            data=pd.DataFrame({
    ...
    @responses.activate
    def test_send_bulk_inferences_auto_retry(self, tabular_batch_model):
    ...
    @responses.activate
    def test_send_bulk_close_auto_retry(self, tabular_batch_model):
    ...
    @responses.activate
    def test_send_bulk_ground_truth(self, tabular_batch_model):
        self._mock_batch_upload(TABULAR_MODEL_ID, 'batch_1')
        resp = tabular_batch_model.send_batch_ground_truths(
    ...
    @responses.activate
    def test_send_parquet_batch(self, tabular_batch_model):
        self._mock_batch_upload(TABULAR_MODEL_ID, 'batch_1')
        file_loc = os.path.join(os.path.dirname(__file__), 'data', 'inference_batch_data')
        resp, close_resp = tabular_batch_model.send_bulk_inferences(batch_id="batch_1", directory_path=file_loc)
        self._assert_result(resp)
    ...
    def test_send_bulk_inferences_missing_metadata(self, tabular_batch_model, case: BulkInferencesCase):
        self._mock_batch_upload(TABULAR_MODEL_ID, 'batch_1')
        ...
        with patch("arthurai.core.inferences.shortuuid.uuid") as uuid_mock:
            with patch("arthurai.core.inferences.datetime") as datetime_mock:
                ...
                if isinstance(case.expected_result, pd.DataFrame):
                    resp, resp_close = tabular_batch_model.send_bulk_inferences(**kwargs)
                else:
                    with pytest.raises(case.expected_result):
                        tabular_batch_model.send_bulk_inferences(**kwargs)
    ...
    @pytest.mark.parametrize("case", bulk_inferences_metadata_cases)
    def test_add_inference_metadata_to_dataframe(self, tabular_batch_model, case: BulkInferencesCase):
        uuid_generator = MockShortUUID(case.partner_inf_ids)
        datetime_generator = MockDatetime([self.TIMESTAMP_VALUE], return_raw_strings=True)
        ...
        with patch("arthurai.core.inferences.shortuuid.uuid") as uuid_mock:
            with patch("arthurai.core.inferences.datetime") as datetime_mock:
                uuid_mock.side_effect = uuid_generator.next
                datetime_mock.now.side_effect = datetime_generator.next
                kwargs = {'df': case.input_dataframe, 'model_attributes': tabular_batch_model.attributes}
                if case.ignore_join_errors is not None:
                    kwargs['ignore_join_errors'] = case.ignore_join_errors
                if isinstance(case.expected_result, pd.DataFrame):
