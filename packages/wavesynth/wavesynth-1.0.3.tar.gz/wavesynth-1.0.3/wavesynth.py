...
some basic sounds. This library does not by default play any sounds using
your system's speakers. It just generates lists of numbers between 0 and
1, which it can save as .wav files to be played by another program. If
you have the `simpleaudio` package installed, however, you will be able
...
# Import simpleaudio if it's available, but don't make a fuss if it isn't.
try:
    import simpleaudio
except Exception:
    simpleaudio = None
...
def stepUpFrom(pitch, nSteps=1):
    """
    Returns the tone that's nSteps (default 1) whole-step(s) above the
    given pitch, on an 8-note octave. Detects the nearest piano-key pitch
    and gives the tone in steps above (or below if nSteps is negative)
    that.

    If the pitch is out of range of the piano keys, simply returns a
    pitch two semitones higher per step.
    """
    if not isinstance(pitch, (int, float)):
        raise TypeError("Pitch must be a number.")
    ...
    index = pianoIndex(pitch)
    if (
        index is None
     or index + 2 > len(PIANO_KEYS) - 1
     or index - 2 < 0
    ):
        index = None

    if index is None:
        return pitch * SEMITONE ** (2 * nSteps)

    elif nSteps >= 0:
        nextName, nextFreq = PIANO_KEYS[index + 1]
        if 'b' in nextName: # it's a black key, so we skip it
            nextFreq = PIANO_KEYS[index + 2][1]

        if nSteps == 1:
            return nextFreq
        else:
            return stepUpFrom(nextFreq, nSteps - 1)
...
def stepDownFrom(pitch, nSteps=1):
    """
    Returns the tone that's `nSteps` (default 1) whole-step(s) below the
    given pitch, on an 8-note octave. Detects the nearest piano-key pitch
    and gives the tone one step below that.

    If the pitch is out of range of the piano keys, simply returns a
    pitch two semitones lower.
    """
    return stepUpFrom(pitch, -nSteps)


def leapUpFrom(pitch, nSteps=1):
    """
    Works like `stepUpFrom`, but for the pentatonic scale. Note that the
    pentatonic scale has much more limited range than the piano keys.
    When given a pitch that falls between notes on the pentatonic scale,
    this function finds the nearest pentatonic pitch and uses that as the
    basis for the pitch.

    When outside of the range of the pentatonic scale, this function
    returns a note that's four semitones above (or below, if `nSteps` is
    negative) the given pitch.
    """
    if not isinstance(pitch, (int, float)):
        raise TypeError("Pitch must be a number.")
    ...
    index = pentatonicIndex(pitch)
    default = pitch * SEMITONE**(4 * nSteps)
    if (
        index is None
     or index + 1 > len(PENTATONIC_NOTES) - 1
     or index - 1 < 0
     or default < PENTATONIC_NOTES[0][1]
     or default > PENTATONIC_NOTES[-1][1]
    ):
        return default

    if nSteps == 1:
        return PENTATONIC_NOTES[index + 1][1]
    elif nSteps == -1:
        return PENTATONIC_NOTES[index - 1][1]
    elif nSteps > 1:
        return leapUpFrom(PENTATONIC_NOTES[index + 1][1], nSteps - 1)
    else: # less than -1
        return leapUpFrom(PENTATONIC_NOTES[index - 1][1], nSteps + 1)


def leapDownFrom(pitch, nSteps=1):
    """
    The opposite of `leapUp`.

    When outside of the range of the pentatonic scale, this function
    returns a note that's four semitones above the given pitch.
    """
    return leapUpFrom(pitch, -nSteps)
...
def setPitch(pitch):
    ...
    some other pitch-modifying function is used (e.g., `stepUp`,
    `halfStepDown`, etc.).
...
def stepUp(nSteps=1):
    """
    Modifies the current pitch value to be `nSteps` full step(s) higher
    than its old value. This uses `stepUpFrom` to compute the new pitch.
    `nSteps` defaults to 1 if no argument is provided.
    """
    global CURRENT_PITCH
    CURRENT_PITCH = stepUpFrom(CURRENT_PITCH, nSteps)


def stepDown(nSteps=1):
    """
    The opposite of `stepUp`: reduces the current pitch value. `nSteps`
    defaults to 1.
    """
    stepUp(-nSteps)
...
def leapUp(nSteps=1):
    """
    Increases the current pitch value by `nSteps` pentatonic scale steps
    (which are larger than the scientific notation steps used by `stepUp`
    and `stepDown`). `leapUpFrom` is used to compute the new pitch value.
    `nSteps` defaults to 1.
    """
    global CURRENT_PITCH
    CURRENT_PITCH = leapUpFrom(CURRENT_PITCH, nSteps)


def leapDown(nSteps=1):
    """
    The opposite of `leapUp`: recudes the current pitch value. `nSteps`
    defaults to 1.
    """
    leapUp(-nSteps)
...
The default volume level is 0.6, not 1.0.
"""
...
def louder(steps=1):
    ...
    (up to the maximum volume). Does nothing if the volume is already
    maxed out. A number of steps may be provided to take multiple steps
    at once.
    """
    ...
    CURRENT_VOLUME = min(1, CURRENT_VOLUME * 1.5 ** steps)
...
def quieter(steps=1):
    ...
    CURRENT_VOLUME = min(1, CURRENT_VOLUME * (1 / 1.5) ** steps)
...
def addNote(duration):
    ...
    `setPitch`, and/or `setVolume` or related functions like `stepUp` or
...
def renderTrack(track):
    ...
    the simpleaudio module is available.
    """
    ...
    # Pre-construct a WaveObject if simpleaudio is available.
    if simpleaudio is not None:
        _debug("Creating wave object...")
        _debug(
            (
...
def playTrack(wait=True):
    ...
    This function only works if the `simpleaudio` package is available,
    and raises a NotImplementedError if it isn't.

    When playing asynchronously, it returns a `WaveObject` instance,
    which has a `wait_done` method that can be used to wait until the
    playback is finished. When playing synchronously, it returns None.
    ...
    if simpleaudio is None:
        raise NotImplementedError(
            "You must have the `simpleaudio` package installed in order"
          + " to play sounds.\n"
          + "You should be able to use your package manager (or maybe"
          + " `pip`) to install the `simpleaudio` package."
        )
