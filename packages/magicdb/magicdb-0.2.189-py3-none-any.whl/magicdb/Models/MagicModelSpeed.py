...
class MetaBase:
    ...
    fields_for_array_actions: T.Set[str] = Field(default_factory=set)
...
class MagicModelSpeed(BaseModelName, metaclass=QueryAndBaseMetaClasses):
    ...
    def make_update_d(self):
        ...
        if not data_from_db:
            ...
            update_d = make_update_obj(
                original=data_from_db,
                new=new_d,
                fields_for_array_actions=self.__class__.Meta.fields_for_array_actions,
            )
    ...
    def update(
        ...
        try:
            ...
            if hasattr(e, "message") and "no document to update" in e.message.lower():
                if create:
                    return self.save(batch=batch, transaction=transaction, merge=True)
                else:
    ...
    @staticmethod
    def get_class_with_func_from_collection_name(
        collection_name: str, f_name: str
    ...
    @staticmethod
    def get_all_collection_names_with_func(f_name: str) -> T.Set[str]:
        subs = MagicModelSpeed.get_all_subclasses_of_model()
        collection_names = set()
        for sub in subs:
            if getattr(sub, f_name)():
                collection_names.add(sub.get_collection_name())
        return collection_names

    @staticmethod
    def get_collections_with_func(f_name: str) -> T.List[T.Type[MagicModelSpeed]]:
        names = MagicModelSpeed.get_all_collection_names_with_func(f_name=f_name)
        return [
            MagicModelSpeed.get_class_with_func_from_collection_name(
                collection_name=name, f_name=f_name
            )
            for name in names
        ]

    @staticmethod
    def all_models_to_trigger() -> T.Set[T.Type[MagicModelSpeed]]:
        models_with_trigger = MagicModelSpeed.get_collections_with_func(
            f_name="get_triggers"
        )
        models_with_redis = MagicModelSpeed.get_collections_with_func(
            f_name="get_redis"
        )
        models_to_trigger = {*models_with_trigger, *models_with_redis}
        return models_to_trigger
    ...
    def refresh_all_triggers() -> None:
        start = time.time()
        classes = MagicModelSpeed.get_collections_with_func(f_name="get_triggers")
    ...
    @classmethod
    def get_or_create(cls, id: str) -> MagicModelSpeed:
        model = cls.collection.get(id=id)
        if not model:
            model = cls(id=id)
        return model

    @classmethod
    def strawberry_fields(cls) -> T.Iterable[str]:
        sorted_fields = sorted(
            cls.__fields__.values(), key=lambda x: (0 if x.required else 1)
        )
        cls.__fields__ = {field.name: field for field in sorted_fields}
        return cls.__fields__.keys()

    """SET OR REMOVE FROM REDIS"""

    @classmethod
    def refresh_redis(cls, id: str) -> bool:
        r = cls.get_redis()
        ttl = cls.get_redis_ttl_secs()
        if not r:
            return False
        key = f"{cls.get_collection_name()}/{id}"
        raw = magicdb.db.conn.document(key).get().to_dict()
        if not raw:
            r.delete(key)
            print(f"deleted {key} from redis")
        else:
            r.set(key, json.dumps(raw, default=pydantic_encoder), ex=ttl)
            print(f"added {key} to redis with ttl {ttl}")
        return True
