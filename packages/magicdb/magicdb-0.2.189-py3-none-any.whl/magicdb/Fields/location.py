...
class Location(FrontendParserModel):
    ...
    def display_short_address(self):
        return f"{self.street_address}, {self.city}, {self.state_abbr}"

    def display_city_and_state(self):
        return f"{self.city}, {self.state_abbr}"

    def prettify_date(self, date: datetime, short: bool = False) -> Optional[str]:
        ...
        try:
            ...
            date_str = "%b %d"
            if not short:
                date_str = f"{date_str}, %Y at %I:%M %p"
            return tz_date.strftime(date_str)
