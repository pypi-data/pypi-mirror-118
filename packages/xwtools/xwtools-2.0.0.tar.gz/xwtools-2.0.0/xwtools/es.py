...
def _get_ner_key_type_str(kw, key, query_list, case_sensitive):
    name_list = []
    for x in query_list:
...
def _get_ner_key_type_list(kw, key, query_list, case_sensitive):
    name_list = []
...
def getNerByEs(kw, es_model, limit=40, key='name', key_type="str", reverse=True, case_sensitive=False):
    """
    :param kw: 输入的文本内容
    ...
    if key_type == 'list':
        name_list = _get_ner_key_type_list(kw, key, query_list, case_sensitive)
    else:
        name_list = _get_ner_key_type_str(kw, key, query_list, case_sensitive)
