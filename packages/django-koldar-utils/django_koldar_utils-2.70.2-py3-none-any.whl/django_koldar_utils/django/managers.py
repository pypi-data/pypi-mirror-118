...
#todo readd Generic[TMODEL],
class ExtendedManager(IManager, models.Manager, OnlyActiveManagerMixIn):
    """
    ...
    #     try:
    #         self.model._meta.get_field(self.active_field_name())
...
#todo readd Generic[TMODEL],
class ExtendedUserManager(IManager, UserManager, OnlyActiveManagerMixIn):
    """
...
class OnlyActiveManagerMixIn:
    ...
    If the model does not contains an "active" flag, we will do nothing
    ...
    def get_queryset(self):
        if hasattr(self.model, self.active_field_name()):
            return super().get_queryset().filter(**{self.active_field_name(): True})
        else:
            return super().get_queryset()
...
class StandardOnlyActiveManager(models.Manager, OnlyActiveManagerMixIn):
    """
...
class StandardOnlyIsActiveManager(models.Manager, OnlyActiveManagerMixIn):
    """
    A manager that consider only rows whose actve field is True.
