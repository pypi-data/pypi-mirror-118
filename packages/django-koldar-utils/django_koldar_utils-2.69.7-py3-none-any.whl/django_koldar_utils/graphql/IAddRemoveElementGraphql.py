...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    @abc.abstractmethod
    def _is_element_in_relationship(self, mutation_class: type, context: Union[AddMutationContext, RemoveMutationContext], data: Dict[str, any], info: any,
                                *args, **kwargs) -> bool:
        """
    ...
    @abc.abstractmethod
    def _get_remove_mutation_primary_input_list(self, context: RemoveMutationContext) -> graphene.Argument:
        """
    ...
    def _generate_add_mutation(self, context: AddMutationContext) -> type:
        ...
        def body(mutation_class, info, *args, **kwargs) -> any:
            ...
            for f in fields_to_check:
                d[f] = getattr(input, f.name)
            d[context.active_flag_name] = True
            # check if the element we need to add exists in the database
            if not self._is_element_in_relationship(mutation_class, context, d, info, *args, **kwargs):
                if context.add_if_not_present:
        ...
        arguments = dict()
        arguments[input_name] = GraphQLHelper.argument_required_input(context.graphene_secondary_input_type,
    ...
    def _generate_remove_mutation(self, context: RemoveMutationContext) -> type:
        ...
        def body(mutation_class, info, *args, **kwargs) -> any:
            ...
            for x in input_parameter:
                d = dict()
                for f in fields_to_check:
                    d[f] = getattr(x, f.name)
                d[context.active_flag_name] = True
                exists_in_db = self._is_element_in_relationship(mutation_class, context, d, info, *args, **kwargs)
                if exists_in_db:
...
class SimpleAddRemoveElementGraphQL(RemoveByIds, IAddRemoveElementGraphql):
    ...
    def _is_element_in_relationship(self,
                                    mutation_class: type,
                                    context: Union[AddMutationContext, RemoveMutationContext],
                                    data: Dict[str, any],
                                    info: any,
                                    *args, **kwargs
                                    ) -> bool:
        return context.django_secondary_owner_type._default_manager.filter(**data) is not None

    def _add_object_to_db(self,
                          mutation_class: type,
                          context: Union[AddMutationContext, RemoveMutationContext],
                          data: Dict[str, any],
                          info,
    ...
    def _get_number_of_elements_in_association(self,
                                               context: Union[AddMutationContext, RemoveMutationContext]
                                               ) -> int:
        return getattr(context.django_main_owner_type, self.relationship_manager).all().count()

    def _add_association_between_models_in_db(self,
                                              context: Union[AddMutationContext, RemoveMutationContext],
                                              object_to_add: any
                                              ) -> int:
    ...
    def _remove_association_between_models_from_db(self,
                                                   mutation_class: type,
                                                   context: Union[AddMutationContext, RemoveMutationContext],
                                                   data: Dict[str, any],
                                                   info,
