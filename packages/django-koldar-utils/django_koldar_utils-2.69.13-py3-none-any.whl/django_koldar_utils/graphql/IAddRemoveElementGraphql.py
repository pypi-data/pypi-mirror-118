...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    def _get_add_mutation_input_parameter_names(self, context: AddMutationContext) -> Dict[type, str]:
        """
        :return: name of the graphql mutation parameter representing the object to add.
        The object return as many values as the size of the relationship. keys are django types and values are the
        input parameter types
        """
        return {k: stringcase.pascalcase(f"{k.__name__}Item") for k in context.django_types}
    ...
    def generate(self, graphene_main_owner_type: type, graphene_secondary_types: List[type],
                 graphene_secondary_input_types: List[type], relation_name: str,
        ...
        django_secondary_owner_types = [x._meta.model for x in graphene_secondary_types]
        ...
        if generate_add_mutation:
            add_mutation = self._generate_add_mutation(AddMutationContext(
                ...
                django_secondary_owner_types=django_secondary_owner_types,
                graphene_secondary_types=graphene_secondary_types,
                graphene_secondary_input_types=graphene_secondary_input_types,
    ...
    def _generate_add_mutation(self, context: AddMutationContext) -> type:
        ...
        arguments = dict()
        arguments[input_name] = GraphQLHelper.argument_required_input_list(
