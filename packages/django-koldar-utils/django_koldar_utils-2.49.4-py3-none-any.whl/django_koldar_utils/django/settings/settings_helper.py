...
def get_paths(file: str) -> Tuple[str, str]:
    """
    get the interesting paths w.r.t a django project

    :return: tuple.
        - first is SETTING_DIR (absolute path of the settings.py of the project)
        - second is BASE_DIR: directory where the django project is located (both in apps and in project directory)
    """
    SETTING_DIR = Path(file).resolve()
    # Build paths inside the project like this: BASE_DIR / 'subdir'.
    BASE_DIR = Path(file).resolve().parent.parent
    return SETTING_DIR, BASE_DIR
