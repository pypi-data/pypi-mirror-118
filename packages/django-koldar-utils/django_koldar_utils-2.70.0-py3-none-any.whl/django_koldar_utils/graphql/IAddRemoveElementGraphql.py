...
class SimpleAddRemoveElementGraphQL(RemoveByIds, IAddRemoveElementGraphql):
    ...
    def _add_association_between_models_in_db(self, context: AddMutationContext, model_definitions: List[ModelDefinition], relationship_endpoints: List[any], relationship_specific_data: any) -> Tuple[any, int]:
        ...
        if hasattr(manager, 'through'):
            ...
            through_model._default_manager.create({**d, **dict(relationship_specific_data)})
        else:
            getattr(context.model_types[0].django_type, self.relationship_manager).add(relationship_endpoints)
        return True, 1
