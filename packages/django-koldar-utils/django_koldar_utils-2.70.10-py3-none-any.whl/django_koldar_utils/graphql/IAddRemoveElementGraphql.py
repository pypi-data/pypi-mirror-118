...
class RemoveMutationContext(AbstractContext):
    ...
    def __init__(self, model_types: List[ModelDeclarationContext], relation_name: str,
        ...
        self.description: str = None
        self.primary_input_name: str = None
        self.output_name: str = None
...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    def _remove_association_between_models_from_db(self, mutation_class: type, context: RemoveMutationContext,
                          relationship_to_remove: any, info, *args, **kwargs) -> Tuple[any, int]:
        """
    ...
    @abc.abstractmethod
    def _get_remove_mutation_primary_input(self, context: RemoveMutationContext) -> graphene.Field:
        """

        :param context: context of the mutation generation
        :return: an element that will belong to the mutation remove graphql input parameters. This
            element contains something that allows us to remove mutliple relationships from the database. relationship
            endpoint models won't be touched at all!
        """
        pass
    ...
    def _generate_remove_mutation(self, context: RemoveMutationContext) -> type:
        ...
        context.primary_input_name = self._get_remove_mutation_input_parameter_name(context)
        ...
        # ARGUMENTS
        arguments = dict()
        arguments[context.primary_input_name] = self._get_remove_mutation_primary_input(
            context=context
        )
    ...
    def _remove_mutation_actual_body(self, mutation_class, info, *args, **kwargs) -> any:
        ...
        # input parameter may be a sngle value, or a list. Who knows? If it is not a list, we make a list of it
        primary_input_values = kwargs[context.primary_input_name]
        if not isinstance(primary_input_values, list):
            primary_input_values = [primary_input_values]

        # for every element in the list "primary_input_value", we try to remove it
        elements_removed = []
        rows_removed = 0
        for data in primary_input_values:
            # d = self._convert_graphql_input_to_dict(x, fields_to_check)
            # d[active_flag_name] = True
            # d = self._prepare_data_before_checking_presence_in_db(mutation_class, context, )

            result, removed = self._remove_association_between_models_from_db(
                mutation_class=mutation_class,
                context=context,
                relationship_to_remove=data,
                info=info,
                *args,
                **kwargs
            )
            elements_removed.append(result)
        ...
        # yield result
        return mutation_class(**{
            context.output_name: elements_removed,
...
class SimpleAddRemoveElementGraphQL(RemoveByIds, IAddRemoveElementGraphql):
    ...
    def _remove_association_between_models_from_db(self, mutation_class: type, context: AbstractContext, relationship_to_remove: any, info, *args,
                                                   **kwargs) -> List[any]:
        getattr(context.model_types[1].django_type, self.relationship_manager).filter(**relationship_to_remove).update(**{context.active_flag_name: False})
        return [1]
