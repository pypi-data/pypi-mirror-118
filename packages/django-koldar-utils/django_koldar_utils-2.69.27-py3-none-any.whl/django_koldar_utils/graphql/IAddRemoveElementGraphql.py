...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    def _add_mutation_actual_body(self, mutation_class, info, *args, **kwargs) -> any:
        result_create = 0
        if self.get_context_key() not in kwargs:
            raise ValueError(f"Cannot find '{self.get_context_key()}' context!")

        context: AddMutationContext = kwargs[self.get_context_key()]
    ...
    def _generate_add_mutation(self, context: AddMutationContext) -> type:
        ...
        return GraphQLHelper.create_mutation(
            ...
            body=functools.partial(self._add_mutation_actual_body, **{self.get_context_key(): context})
        )
