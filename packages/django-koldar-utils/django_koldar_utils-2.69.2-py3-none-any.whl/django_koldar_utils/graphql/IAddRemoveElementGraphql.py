...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    @abc.abstractmethod
    def _is_element_in_relationship(self, mutation_class: type, django_element_type: type, data: Dict[str, any], info: any,
                                *args, **kwargs) -> bool:
        """
    ...
    @abc.abstractmethod
    def _add_object_to_db(self, mutation_class: type, django_element_type: type,
    ...
    @abc.abstractmethod
    def _get_object_from_db(self, mutation_class: type, django_element_type: type, active_flag_name: str,
                          data: Dict[str, any], info, *args, **kwargs) -> Optional[any]:
        """
        Add an object passed in input to the database.
        By default we will call create with data
    ...
    def generate(self, django_list_owner_type: type, django_element_type: type,
        ...
        if generate_remove_mutation:
            remove_mutation = self._generate_remove_mutation(
                django_list_owner_type=django_list_owner_type,
                django_element_type=django_element_type,
                graphene_element_input_type=graphene_element_input_type,
                relation_name=relation_name,
    ...
    def _generate_add_mutation(self,
        ...
        def body(mutation_class, info, *args, **kwargs) -> any:
            ...
            if not self._is_element_in_relationship(mutation_class, django_element_type, d, info, *args, **kwargs):
                if add_if_not_present:
                    ...
                    object_to_add = self._add_object_to_db(mutation_class, django_element_type, create_args,
                                                           info, *args, **kwargs)
    ...
    def configure_add_mutation_inputs(self, original_arguments: Dict[str, graphene.Field], django_element_type: type) -> Dict[
        str, graphene.Field]:
        return original_arguments

    def configure_remove_mutation_inputs(self, original_arguments: Dict[str, graphene.Field], django_element_type: type) -> Dict[
        str, graphene.Field]:
        return original_arguments
    ...
    def _generate_remove_mutation(self,
        ...
        def body(mutation_class, info, *args, **kwargs) -> any:
            ...
            for x in input_parameter:
                ...
                d[active_flag_name] = True
                exists_in_db = self._is_element_in_relationship(mutation_class, django_element_type, d, info, *args, **kwargs)
                if exists_in_db:
