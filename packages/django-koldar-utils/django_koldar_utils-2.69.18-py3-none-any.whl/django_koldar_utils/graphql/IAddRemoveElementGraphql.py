...
class IAddRemoveElementGraphql(abc.ABC):
    ...
    def _get_add_mutation_input_parameter_names(self, context: AddMutationContext) -> Iterable[Tuple[int, str]]:
        ...
        for i, x in enumerate(context.model_types):
            if x.__name__ not in index_types:
                index_types[x.__name__] = 0
            else:
                index_types[x.__name__] += 1

            s = index_types[x.__name__] if index_types[x.__name__] > 0 else ""
            yield i, stringcase.pascalcase(f"{x.__name__}Item{s}")
    ...
    def _generate_add_mutation(self, context: AddMutationContext) -> type:
        ...
        for index, input_param in enumerate(input_names):
    ...
    def _add_mutation_output_graphene_field(self, context: AddMutationContext) -> graphene.Field:
        ...
        return GraphQLHelper.returns_nonnull(
            return_type=context.model_types[0].django_type,
            description=f"the {context.model_types[0].django_type.__name__} we just added in the relation"
        )
    ...
    def _add_mutation_name(self, context: AddMutationContext) -> str:
        ...
        """
        return f"Add{stringcase.pascalcase(context.relation_name)}Of{''.join(map(lambda x: stringcase.pascalcase(x.__name__), context.model_types))}"
    ...
    def _remove_mutation_name(self, context: RemoveMutationContext) -> str:
        ...
        :return: name of the mutation that adds elements to the relationship
        """
        return f"Remove{stringcase.pascalcase(context.relation_name)}Of{''.join(map(lambda x: stringcase.pascalcase(x.__name__), context.model_types))}"
