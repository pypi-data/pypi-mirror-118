...
class Client(Fist):
    ...
    def insert_credit_cash_repay(self, tg_name: str, amount: float, debt_id: str = '', repay_type: message_pb.CreditRepayType = message_pb.CreditRepayType.CREDIT_REPAY_TYPE_UNSPECIFIED, sub_account: str = ''):
        '''
        提交现金还款请求
    ...
    def insert_credit_extend_debt_date(self, tg_name: str, debt_id: str, defer_days: int, sub_account: str = ''):
        '''
        提交合约负债展期请求
    ...
    def req_credit_debt_info(self, tg_name, sub_account=''):
        '''
        请求查询负债合约信息
        ...
        -1 if failed, or req_id(int) if succeed
        '''
        req = message_pb.ReqCreditDebtInfo()
        req.account_id = tg_name
        req.sub_account = sub_account
        req.sender = self.fist_name
        ret = self.req(self.trade_router, req, message_pb.MsgType.MSG_TYPE_REQ_CREDIT_DEBT_INFO, 0)
        err_id = ret.get_err_id()
        if err_id == 0:
            return ret.get_req_id()
        else:
            self.logger.error('[req_credit_debt_info] error occurred (eid){} (tg){}'.format(err_id, tg_name))
            return -1
    ...
    def on_rsp_credit_debt_info(self, obj, frame_nano):
        '''
        处理请求查询信用账户负债合约信息

        obj: RspCreditDebtInfo
        '''
        pass
    ...
    def on_pub_frame(self, f):
        ...
        if msg_callback is not None:
            ...
            elif msg_type == message_pb.MsgType.MSG_TYPE_RSP_CREDIT_DEBT_INFO:
                rsp_credit_debt_info_obj = f.get_obj(message_pb.RspCreditDebtInfo)
                self.on_rsp_credit_debt_info(rsp_credit_debt_info_obj, frame_nano)
