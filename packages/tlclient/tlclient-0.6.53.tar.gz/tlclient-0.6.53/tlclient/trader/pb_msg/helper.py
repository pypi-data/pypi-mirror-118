...
def _reqCreditCashRepayStr(self):
    return '<rcred_cr oid={} ref={} amt={} did={}>'.format(
        self.order_id, self.order_ref, self.amount, self.debt_id
    )
...
def _reqCreditExtendDebtDateStr(self):
    return '<rcred_edd oid={} ref={} did={} ddays={}>'.format(
        self.order_id, self.order_ref, self.debt_id, self.defer_days
    )
...
def _rtnCreditCashRepayStr(self):
    return '<cred_cr oid={} ref={} ramt={} cramt={} did={}'.format(
        self.order_id, self.order_ref, self.request_amount, self.cash_repay_amount, self.debt_id
    )


def _rtnCreditExtendDebtDateStr(self):
    return '<cred_edd oid={} ref={} did={} os={} ot={}'.format(
        self.order_id, self.order_ref, self.debt_id, message_pb.CreditDebtExtendOperStatus.Name(self.oper_status), self.oper_time.ToDatetime()
    )
...
def _rspCreditDebtInfoOneStr(self):
    return '<dio dt={} did={} pid={} ref={} ds={} exch={} tkr={} od={} ed={} oed={} ra={} rq={} rp={}>'.format(
        message_pb.CreditDebtType.Name(self.debt_type), self.debt_id, self.position_id, self.order_xtp_id, message_pb.CreditDebtStatus.Name(self.debt_status), message_pb.Exchange.Name(self.exchange), self.symbol,
        self.order_date.ToDatetime(), self.end_date.ToDatetime(), self.orig_end_date.ToDatetime(), self.remain_amt, self.remain_qty, self.remain_principal
    )


def _rspCreditDebtInfoStr(self):
    s = '<c_di tg={} count={} eid={} err={} rt={}\n'.format(self.account_id, len(self.infos), self.err_id, self.err_msg, self.recv_time.ToDatetime())
    for one in self.infos:
        s += '\t' + str(one) + '\n'
    s += '>'
    return s
...
def _rspCreditTickerAssignInfoOneStr(self):
    return '<taio exch={} tkr={} lm_qty={} yd_qty={} lf_qty={} fz_qty={}>'.format(
        message_pb.Exchange.Name(self.exchange), self.symbol, self.limit_qty, self.yesterday_qty, self.left_qty, self.frozen_qty
    )
...
message_pb.CreditDebtInfoOne.__str__ = _rspCreditDebtInfoOneStr
message_pb.RspCreditDebtInfo.__str__ = _rspCreditDebtInfoStr
