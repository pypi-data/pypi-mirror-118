...
class HelpPaper(ArgparseAction):
    ...
    def __call__(self, *args, **kwargs):
        ...
        print(
            ...
            .format(
                units=", ".join(sorted(papersize.HUMAN_UNITS)),
                sizes=", ".join(papersize.HUMAN_SIZES),
            )
            .strip()
        )
        sys.exit(0)
...
class ListUnits(ArgparseAction):
    ...
    def __call__(self, *args, **kwargs):
        # pylint: disable=signature-differs
        for key, value in sorted(papersize.HUMAN_UNITS.items()):
            print(f"{key}: {value}")
        sys.exit(0)
...
class ListSizes(ArgparseAction):
    ...
    def __call__(self, *args, **kwargs):
        # pylint: disable=signature-differs
        for key, value in sorted(papersize.HUMAN_SIZES.items()):
            print(f"{key}: {value}")
        sys.exit(0)
