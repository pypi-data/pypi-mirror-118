...
from dora_lakehouse.sdk import sanitize
...
class Table(Catalog, ABC):
    ...
    def __init__(self, database:Database, name:str, columns:list, hash_def:str, location:str=None, source:list=None, delete:str="1=2", description:str=None,table_type:str="EXTERNAL_TABLE",serialization:str="parquet",compression:str="snappy",serde_library:str="org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe",serde_input:str="org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat",serde_output:str="org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat",check:str=None, update:str='upsert'):
        """ Default column constructor
        ...
        if location is None:
            location = f"{environ['BKT_RFN']}/{database.name}/{name}"
        self.location = location.strip()
        # Order columns by data and partition types
        _data_cols = list()
        _part_cols = list()
        for _col in Column.validate(columns):
            if _col.ispartition():
                _part_cols.append(_col)
            else:
                _data_cols.append(_col)
        self.columns = list(_data_cols + _part_cols)
        self.table_type = Table.validate('type',table_type)
        self.serialization = Table.validate('serialization',serialization)
        self.compression = Table.validate('compression',compression)
        self.update = Table.validate('update',update)
        self.description = description
        if description is not None:
            self.description = description.strip()
        # SerDe info
        self.serde_library = serde_library
        self.serde_input = serde_input
        self.serde_output = serde_output
        self.check = check
        self.delete = delete
