...
from ice_moviepy.compat import DEVNULL
from ice_moviepy.config import get_setting
...
class FFMPEG_AudioWriter:
    ...
    def write_frames(self, frames_array):
        try:
            ...
            error = (str(err) + ("\n\nice_moviepy error: FFMPEG encountered "
...
def ffmpeg_audiowrite(clip, filename, fps, nbytes, buffersize,
    ...
    logger(message="ice_moviepy - Writing audio in %s" % filename)
    ...
    if write_logfile:
        logfile.close()
    logger(message="ice_moviepy - Done.")
