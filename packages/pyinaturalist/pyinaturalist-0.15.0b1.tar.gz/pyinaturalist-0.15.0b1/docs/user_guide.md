...
{py:class}`.iNatSession`, but any {py:class}`requests.Session` or compatible object will work.
...
>>> from pyinaturalist import iNatSession, get_taxa
>>> session = iNatSession(expire_after=timedelta(days=5))
>>> get_taxa(q='warbler', locale=1, session=session)
```

To store the cache somewhere other than the default cache directory:
```python
>>> session = iNatSession(cache_name='~/data/api_requests.db')
```
...
requests. The easiest way to do this is with {py:class}`.iNatSession`. For example, to reduce the
rate to 50 requests per minute:

```python
>>> from pyinaturalist import iNatSession, get_taxa
>>> session = iNatSession(per_minute=50)
>>> get_taxa(q='warbler', locale=1, session=session)
```
