...
def multiclass_rank(C_mod, key_list, target_line ,score_type = "abs", comvert="default", show_range="top50",target_label=None):
    ...
    for tl1 in target_label:
        ...
        for count in range(len_key_l):
            ...
            out2 = out_l.values.tolist()
            ans2 = ans_l.values.tolist()

            print(len(out2),len(ans2))

            sa2 = []

            if score_type == "abs":
                for jjj in range(len(out2)) : sa2.append(out2[jjj] - ans2[jjj])
            elif score_type == "rel":
                for jjj in range(len(out2)) : sa2.append((out2[jjj] - ans2[jjj]) / ans2[jjj])
