...
async def do_login(email: Text, password: Text):
    ...
    async with get_async_http_session().post(
        ...
        if is_success_status(response.status):
            ...
            console.print(
                "> [red]ERROR[/red] ❌ Login failed! Please check your username and password"
            )
...
async def app_install(name: Text):
    ...
    async with get_async_http_session().post(
        ...
        if is_success_status(response.status):
            ...
            console.print(
                f"⏩ To use the {name} app: [dark_orange3]neuralspace {name}[dark_orange3] --help"
            )
        else:
            if json_response["message"] == APP_IS_INSTALLED:
                console.print(
                    f"> [deep_sky_blue2]INFO[/deep_sky_blue2] ✅  {APP_IS_INSTALLED}"
                )
                console.print(
                    f"⏩ To use the {name} app: [dark_orange3]neuralspace {name}[dark_orange3] --help"
                )
