...
class RotateCBOXCoords(BMTool):
    ...
    def run(self, args):
        ...
        for cbox_path, rotation_meta in tqdm.tqdm(matched_pairs, desc="Rotate"):
            ...
            if is_filament:
                # filament path
                rotated_dat= self.rotate_filaments(filaments=cbox_dat, meta=rotation_meta, propagation=propagation)
            else:
                # bounding box path
            ...
            coords_dat = rotated_dat
            if is_filament:
                all_boxes = []
                for fil in rotated_dat:
                    all_boxes.extend(fil.boxes)
                coords_dat = all_boxes
    ...
    @staticmethod
    def zsplit_filament(boxes : List[BoundBox]) -> List[Filament]:
        '''
        Splits filament into sub-filaments with constant z value
        '''
        splitted_filaments: List[Filament] = []
        fil_boxes: List[BoundBox] = []

        for b in boxes:

            b = copy.deepcopy(b)
            b.z = int(np.round(b.z))
            if len(fil_boxes)==0 or fil_boxes[-1].z == b.z:
                fil_boxes.append(b)
            else:
                f = Filament(boxes=fil_boxes)
                splitted_filaments.append(f)
                fil_boxes = []
        splitted_filaments.append(f)

        return splitted_filaments

    @staticmethod
    def zsplit_filament_2(boxes: List[BoundBox], propagation: int) -> List[Filament]:
        '''
        Splits filament into sub-filaments with constant z value
        '''
    ...
    def rotate_filaments(self, filaments : List[Filament],
                         meta : RotationMeta,
                         propagation: int = 2) -> List[Filament]:
        '''
        Rotates the filament positions. Splits them into sub-filaments to have a constant z-value.
        '''
        ...
        for filament in filaments:
            ...
            splitted_filaments = self.zsplit_filament_2(rotated_boxes, propagation)
            rotated_filaments.extend(splitted_filaments)
