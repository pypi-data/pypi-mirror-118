...
class h5dict:
    def __init__(self, hdf5, name, mode, conv, parent=None, key=""):
        ...
        self.parent = parent
        self.key = key
        self.new_h5py = version_parse(h5py.__version__) >= version_parse('3.0')
        self.closed = False
        ...
        if self.in_write_mode():
            ...
            # Else stores data
                ...
                try:
                    ...
                    self.write_ds(key, data=format(value), **filters)
                    path = self.get_path() + "/" + key
                    print(
                        "Warning: Tried to store incompatible type on {} in {}, storing representation.".format(
                            path, self.name
                        ),
                        file=sys.stderr,
                    )
    ...
    def instance(self, hdf5, key):
        ...
        self.check_closed()
        return h5dict(
            hdf5, self.name, self.mode, self.conversions, parent=self, key=key
        )
