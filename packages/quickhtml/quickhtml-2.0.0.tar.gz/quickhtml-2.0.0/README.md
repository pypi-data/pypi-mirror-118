...
This is a project that I have made purely for educational purposes. The goal was to better understand text manipulation/pre-processing and regular expressions. You can use it if you want to, but there are many far better solutions available, which I would highly recommend over this. My project implements most of the base Markdown syntax, with one notable exception being mixing nested tags _(see [Mixing nested tags](#mixing-nested-tags))_, so albeit having everything I needed and serving me well for my purposes, it is not as complete and reliable as other preprocessors are, since, again, it is just a small, educational project.

With that said, please note that while I may commit changes such as refactoring code from time to time, I am not actively maintaining this project anymore, thus open issues may not be resolved unless by pull requests and collaboration from other people. At this time, The only issue known to me is that lists and blockquotes can not be mixed together, that is, blockquotes can not have a list inside them, lists can not have blockquotes inside them, and lists can not have another type of list inside them. Still, if you'd like to contribute to this project, I'd be more than wiling to review and accept pull requests.
...
_At the moment, multiline items are not supported inside blockquotes. This means each line is a new item._
...
_At the moment, multiline items are not supported inside lists. This means each line is a new item._
