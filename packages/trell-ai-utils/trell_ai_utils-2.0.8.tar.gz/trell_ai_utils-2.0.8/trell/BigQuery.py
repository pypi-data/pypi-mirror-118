...
class BigQuery:
    ...
    def __init__(self,service_account_file_path=config.BIG_QUERY_SERVICE_ACCOUNT_FILE_PATH):
    ...
    def get_data(self, query=None, query_config=None, max_retries=0, time_interval=5):
        ...
        while (flag):
            try:
                if query:
                    df = gbq.read_gbq(query=query, project_id=config.READ_BIG_QUERY_PROJECT,
    ...
    def dump_data(self, database=None, table=None, dataframe=None, mode="append"):
        ...
        try:
            gbq.to_gbq(dataframe, '{}.{}'.format(database, table), config.WRITE_BIG_QUERY_PROJECT, if_exists=mode,
    ...
    def insert_rows_array(self, dataset=None, table=None, rows_to_insert=None):
        ...
        :param string table: target table name
        :param list rows_to_insert: list of dictionaries in which each dictionary is a row with keys as column names
        """
    ...
    def insert_rows_in_bigquery(self, dataset=None, table=None, rows_to_insert=None):
        ...
        try:
            table_id = '{0}.{1}.{2}'.format(config.WRITE_BIG_QUERY_PROJECT, dataset, table)
