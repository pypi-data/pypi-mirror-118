...
# 初始化日志
from ktestpy.common.customize_error import RunTimeTooLong
from ktestpy.common.html_reporter import format_time, generate_html_report
from ktestpy.common.my_logger import MyLogger
from ktestpy.common.test_case_finder import DiscoverTestCases
from ktestpy.common.test_filter import TestFilter
from ktestpy.common.user_options import parse_options
from ktestpy.configs.global_config import get_config, init, set_config
...
def main(args=None):
    ...
    raw_test_suites, excluded_test_suites = TestFilter(original_test_cases).tag_filter_run(options.include_tags_any_match)
...
if __name__ == "__main__":
    ...
    if cases_run_success:
        log.info('\033[1;32m')
        log.info('%s cases Passed, details info are:\n %s' % (len(cases_run_success), list(map(lambda x: x["testcase"],  cases_run_success))))
        log.info('\033[0m')

    if cases_run_fail:
        log.info('%s cases Failed, details info are:\n %s' % (len(cases_run_fail), list(map(lambda x: x["testcase"],  cases_run_fail))))
        log.error('\033[1;31m')
    if cases_encounter_error:
        log.info('%s cases Run error, details info are:\n %s' % (len(cases_encounter_error), list(map(lambda x: x["testcase"],  cases_encounter_error))))
        log.error('\033[0m')

    if skipped_cases:
        log.info('%s cases Skipped, details info are::\n %s' % (len(skipped_cases), list(map(lambda x: x["testcase"],skipped_cases))))

    log.info("\n{pre_fix} Tests done {timer}, total used {c_time:.2f} seconds {pre_fix} " .format(pre_fix='---' * 10, timer=time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end)), c_time=end-start))

    # Generate automation report
    log.info("\n{pre_fix} Starting to generate automation report... {pre_fix} " .format(pre_fix='---' * 10))
    generate_html_report(out_put_folder, cases_run_success, cases_run_fail, cases_encounter_error, skipped_cases, time.time(), platform.system(), False)
    log.info("\n{pre_fix} Generate report done, please check report.html from '{report}'. {pre_fix} " .format(pre_fix='---' *10, report=out_put_folder))
