...
def get_qlcallablebond(bond_info: FixedRateBond,
    ...
    callbond = ql.CallableFixedRateBond(
                ...
                bond_schedule, 
                coupons, 
                ql_day_count[bond_setting.day_count],
                ql_business_day[bond_setting.business_day], 
                bond_info.face_value, 
                datestr_to_qldate(bond_info.issue_date),
                call_schedule)
    return callbond
...
def callbond_stats(value_date: str,
                    call_class: CallableBondParam, 
                    ql_calendar,
                    clean_price: float,
    ...
    oas = ql_callbond.OAS(clean_price, 
                        curve_handle,
                        ql_day_count[call_class.setting.day_count],
                        ql.Compounded,
                        ql_frequency[call_class.setting.frequency],
                        vdate) * 100
    call_risks["OAS"] =  oas
    bond_yield  = ql_callbond.bondYield(clean_price,
                        ql_day_count[call_class.setting.day_count],
                        ql.Compounded,
                        ql_frequency[call_class.setting.frequency],
                        vdate) * 100
    ...
    e_duration = ql_callbond.effectiveConvexity(oas/100, 
                                curve_handle,
                                ql_day_count[call_class.setting.day_count],
                                ql.Compounded,
                                ql_frequency[call_class.setting.frequency])
...
def qlCallableBond_from_Class(call_class: CallableBondParam,
                            qlcalendar):
    qlschedule = qlSchedule_from_Model(call_class.info,
                                        call_class.setting, qlcalendar) 
