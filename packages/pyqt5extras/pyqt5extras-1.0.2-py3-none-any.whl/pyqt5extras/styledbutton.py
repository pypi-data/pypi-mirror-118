...
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLabel, QAbstractButton, QGraphicsDropShadowEffect, \
...
class StyledButton(QAbstractButton):
    ...
    def __repr__(self):
        return f"<pyqt5extras.StyledButton()>"
    ...
    def paintEvent(self, event):
        ...
        if self.isChecked():
            ...
            if self.anim_press.current() > 0.001 and self._hover:
                fc = self.anim_press.lerp(fc2, fc3)
            elif self.anim_press.current() > 0.001:
                fc = self.anim_press.lerp(fc1, fc3)
            else:
                fc = self.anim_hover.lerp(fc1, fc2)
        ...
        if self.anim_press.current() > 0.001 and self._hover:
            fs = self.anim_press.lerp(fs2, fs3)
        elif self.anim_press.current() > 0.001:
            fs = self.anim_press.lerp(fs1, fs3)
        else:
            fs = self.anim_hover.lerp(fs1, fs2)
        ...
        if self.anim_press.current() > 0.001 and self._hover:
            pc = self.anim_press.lerp(pc2, pc3)
        elif self.anim_press.current() > 0.001:
            pc = self.anim_press.lerp(pc1, pc3)
        else:
            pc = self.anim_hover.lerp(pc1, pc2)

        pw1 = self.styleDict["default"]["border-width"]
        pw2 = self.styleDict["hover"]["border-width"]
        pw3 = self.styleDict["press"]["border-width"]
        if self.anim_press.current() > 0.001 and self._hover:
            pw = self.anim_press.lerp(pw2, pw3)
        elif self.anim_press.current() > 0.001:
            pw = self.anim_press.lerp(pw1, pw3)
        else:
            pw = self.anim_hover.lerp(pw1, pw2)
        ...
        if self.isChecked():
            ...
            if self._was_checked:
                b = self.anim_press.lerp(b2, b4)
            elif self.anim_press.current() > 0.001 and self._hover:
                b = self.anim_press.lerp(b2, b3)
            elif self.anim_press.current() > 0.001:
                b = self.anim_press.lerp(b1, b3)
            else:
                b = self.anim_hover.lerp(b1, b2)
        ...
        if self.anim_press.current() > 0.001 and self._hover:
            r = self.anim_press.lerp(r2, r3)
        elif self.anim_press.current() > 0.001:
            r = self.anim_press.lerp(r1, r3)
        else:
            r = self.anim_hover.lerp(r1, r2)
        ...
        if all(crn) and self.styleDict["default"]["render-fast"]:
            ...
            if crn[0]:
                pt.drawEllipse(QPointF(r, r), r, r)
            else:
                pt.drawRect(0, 0, r, r)
            if crn[3]:
                pt.drawEllipse(QPointF(self.width() - r, self.height() - r), r, r)
            else:
                pt.drawRect(self.width() - r, self.height() - r, r, r)
            if crn[2]:
                pt.drawEllipse(QPointF(r, self.height() - r), r, r)
            else:
                pt.drawRect(0, self.height() - r, r, r)
            if crn[1]:
                pt.drawEllipse(QPointF(self.width() - r, r), r, r)
            else:
                pt.drawRect(self.width() - r, 0, r, r)
            ...
            if crn[0]:
                pt.drawEllipse(QPointF(r, r), r - pw, r - pw)
            else:
                pt.drawRect(pw, pw, r + 1, r + 1)
            if crn[3]:
                pt.drawEllipse(QPointF(self.width() - r, self.height() - r), r - pw, r - pw)
            else:
                pt.drawRect(self.width() - r - pw, self.height() - r - pw, r, r)
            if crn[2]:
                pt.drawEllipse(QPointF(r, self.height() - r), r - pw, r - pw)
            else:
                pt.drawRect(pw, self.height() - r - pw, r, r)
            if crn[1]:
                pt.drawEllipse(QPointF(self.width() - r, r), r - pw, r - pw)
            else:
                pt.drawRect(self.width() - r - pw, pw, r, r)

        pt.end()

        if not self.anim_hover.done():
            self.update()
        if not self.anim_press.done():
            self.update()
