...
class CodeTextEdit(QWidget):
    ...
    def __init__(self):
        ...
        def slot():
            self.cursor_lbl.setText(
                f"{self.editor.textCursor().blockNumber()}:{self.editor.textCursor().positionInBlock()}")

    def __repr__(self):
        return f"<pyqt5extras.CodeTextEdit({CodeTextEdit.LANG_DISPLAY(self.lang)})>"
    ...
    def paintEvent(self, event):
        ...
        for i in range((self.height() // int(gap))):
            font = pt.font()
            pt.setFont(font)
            pt.setPen(QPen(self.highlighter.theme["lines"]))

            pt.drawText(13, int(i * gap), str(i + self.sliderVal))
