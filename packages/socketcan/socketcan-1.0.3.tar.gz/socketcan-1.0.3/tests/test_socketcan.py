...
def receive_from_can_isotp_socket(interface, rx_addr, tx_addr, bufsize, use_padding, listen_only, fc_stmin, q):
    """
...
class TestCanIsoTpSocket:
    ...
    def test_can_isotp_socket(self, can_interface):
        ...
        p = Thread(target=receive_from_can_isotp_socket,
                   args=(can_interface, tx_addr, rx_addr, bufsize, use_padding, False, None, q,))
        p.setDaemon(True)
        p.start()
        time.sleep(1)
        s.send(data)
    ...
    def test_can_isotp_socket_with_extended_ids_and_listenonly(self, can_interface):
        ...
        p2 = Thread(target=receive_from_can_isotp_socket,
                    args=(can_interface, tx_addr, rx_addr, bufsize, False, True, None, q2,))
        p2.setDaemon(True)
        p2.start()
    ...
    def test_fc_opt_stmin(self, can_interface):
        ...
        use_padding = True
        fc_stmin = 5
        s = CanIsoTpSocket(interface=can_interface, rx_addr=rx_addr, tx_addr=tx_addr, use_padding=use_padding, fc_stmin=fc_stmin)
        data = bytes(range(64))
        bufsize = len(data)
        q = Queue()
        # Note: the receive thread logically has rx_addr, tx_addr inverted!
        p = Thread(target=receive_from_can_isotp_socket,
                   args=(can_interface, tx_addr, rx_addr, bufsize, use_padding, False, fc_stmin, q,))
        p.setDaemon(True)
        p.start()
        time.sleep(1)
        s.send(data)
