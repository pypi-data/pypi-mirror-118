...
class ConnectSock:
    ...
    def dict_key_title(self, data):
        ...
        if isinstance(data, dict):
            return {k.title(): self.dict_key_title(v) for k, v in data.items()}
        elif isinstance(data, list):
            return [self.dict_key_title(v) for v in data]
        else:
            return data
    ...
    def prepare_body(self, payload=None, files=None):
        if files:
    ...
    def request(self, method="GET", url=None, headers={}, payload={}, files={}, return_dict=False):
        ...
        if self.sock:
            ...
            self.headers.update(self.dict_key_title(headers))

            self.prepare_body(payload, files)
            self.prepare_header()
            ...
            self.sock.close()
            # debug(contents)
            return self.parsing_response(contents, return_dict=return_dict)

    def parsing_response(self, response, return_dict=False):
        """
...
class ControlChain(ConnectSock):
    ...
    def __init__(self, unix_socket="cli.sock", url="/", cid=None, timeout=5, debug=False, auto_prepare=False, wait_state=False):
        """
...
class DockerSock(ConnectSock):
        ...
        if return_type == "merge" and len(return_values) > 0:
            ...
            for values in return_values:
                for r_key, r_val in values.items():
                    self.merge_value(r_key, self.get_simple_image_name(r_val))
    ...
    def merge_value(self, key, value, separator="|"):
