...
def darn_simulation(p, Q, Y, n, s, z_p='uniform', param='local'):
    ...
    if isinstance(z_p, str):
        if z_p == 'uniform':
            ...
            z_p = np.exp(-np.array(list(range(p)))) / np.sum(np.exp(-np.array(list(range(p)))))
        elif z_p == 'normal':
            z_p = normal.pdf(np.array(list(range(p))), 2, 1) / np.sum(normal.pdf(np.array(list(range(p))), 2, 1))
        else:
...
def cdarn(time_series, p, B, z_p='uniform', model_cross='neighbors', q='global', y='global', kernel=None,
          bandwidth=None):
    """
    Estimate the best parameters for the temporal network following the :math:`CDARN(p)` model.
