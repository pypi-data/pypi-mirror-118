...
def subgraph_centrality(G, t=1, tol=1e-7, maxit=50):
    ...
    centrality = dict(list(zip(node_list, subgraph_centrality)))
    return centrality
...
def node_subgraph_centrality(G, u, t=1, tol=1e-7, maxit=50):
    ...
    n = G.number_of_nodes()
    node_list = G.nodes
    enumerated_nodes = dict(list(zip(node_list, arange(n))))
...
def total_communicability(G, t=1):
    ...
    centrality = dict(list(zip(node_list, tot_communicability)))
    return centrality
...
def directed_subgraph_centrality(G, t=1, tol=1e-7, maxit=50):
    ...
    hub_centrality = dict(list(zip(node_list, hub_centrality)))
    authority_centrality = dict(list(zip(node_list, authority_centrality)))
...
def trip_centrality(edge_list, alpha, epsilon=None):
    ...
    if len(edge_list[0]) == 4:
        ...
        tc = dict(list(zip(primary_nodes, tc)))
    # -----------------------------------------------
    if len(edge_list[0]) == 5:  # multiple layers
        ...
        tc = dict(list(zip(primary_nodes, aggregated_centrality)))
    return tc
...
def betweenness_node_family(n_nodes, n_times, n_layers, node_index):
    ...
    """
    node_family = list(range(node_index, n_nodes * n_times * n_layers, n_nodes))
...
def betweenness_node_time_family(n_nodes, n_times, n_layers, node_index, time_index):
    ...
    node_time_family = list(range(node_index + n_nodes * time_index, n_nodes * n_times * n_layers, n_nodes * n_times))
...
def betweenness_centrality(edge_list, delta_t, alpha, epsilon):
    ...
    for e in edge_list:
        ...
        times.append(e[3])
        layers.append(e[4])
        if e[0] in list(node_layer_time_dict.keys()):
            if e[4] in list(node_layer_time_dict[e[0]].keys()):
    ...
    for e in edge_list:
        ...
        for layer in layers:
            if e[1] in list(node_layer_time_dict.keys()):
                if layer in list(node_layer_time_dict[e[1]].keys()):
            ...
            for i in v_t_family:
                ...
                for u in range(n_nodes):
                    if u != v:
                        ...
                        vu_distances = {}
                        for j in u_family:
                            if j in list(paths[0].keys()):
                                if paths[0][j] in list(vu_distances.keys()):
    ...
    betweenness = dict(list(zip(nodes, betweenness)))
    return betweenness
...
G.add_edge(1,3)
G.add_edge('u', 2)
x, y = node_directed_subgraph_centrality(G, 'u', t=4)
print((x, y))
