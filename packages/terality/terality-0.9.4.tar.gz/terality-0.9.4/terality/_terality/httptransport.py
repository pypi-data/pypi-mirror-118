    ...
    * retries at the HTTP level
    ...
    def request(
        ...
        if payload is not None:
            ...
            except TypeError as e:  # Object of type xxx is not JSON serializable
                # Raise a friendlier error message instead.
                # No way to do that apart from parsing the exception message.
                type_error = _parse_json_type_error(e)
                if type_error is not None:
                    raise TeralityClientError(
                        f"Serialization error: Terality does not support calling a function with a parameter of type '{type_error}' (or a collection or object containing such a type)"
                    ) from e
                raise TeralityClientError(
