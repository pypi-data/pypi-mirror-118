...
class Camera(PyHapCamera):
    ...
    async def get_snapshot(self, image_size):  # pylint: disable=unused-argument, no-self-use
        ...
        command = 'raspistill -w ' + width + '-h ' +height + '-o /home/openhubdaemon/snapshot.jpg)'
        logger.debug('Executing get snapshotd: "%s"', ' '.join(command))
        try:
