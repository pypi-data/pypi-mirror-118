...
class CommandV(Extensible):
    ...
    def restore_snapshot(self):
        ...
        db_host = postgres_info.get('NetworkSettings').get('IPAddress')
        self.instance_manager.config.get('env_vars').update({
            'db_host': db_host, 'db_port': 5432,
            'external_db_port': res['result'].get('port')})
        ...
        db_config.update({'db_port': res['result'].get('port'), 'db_host': '127.0.0.1'})
        user = self.instance_manager.get_odoo_users(database_name,
                                                    user_id=self.get_admin_id())
        res.get('result').update({'user_admin': (user and user[0] or {}).get("login")})
    ...
    def _production_commit_and_tag_stable(self):
        ...
        self.sender.send_message('Pushing image {}:{}'.format(image_repo, image_tag))
        ...
        variables = self.instance_manager.config_variables
        tag = "%(app)s-%(version)s-stable" % {
