#-*- coding:utf-8 -*-
# 2020-2021 FinanceData.KR http://financedata.kr fb.com/financedata
...
def list_date_ex(date=None, cache=True):
    ...
    * date: 조회일 (기본값: 당일)
    ...
    for page in range(1, 100):
        ...
        data_list = []
        soup = BeautifulSoup(xhtml_text)
        trs = soup.table.find_all('tr')
        for tr in trs[1:]:
            tds = tr.find_all('td')
            hhmm = tds[0].text.strip()
            corp_class = tds[1].span.span['title'].replace('시장', '') if tds[1].span.span else ''
            code = re.findall("openCorpInfoNew\('(.*?)', '", tds[1].span.a['href'])[0]
...
def attach_file_list(rcp_no): # rcp_no or URL
    ...
    * rcp_no: 접수번호를 지정합니다. rcp_no 대신 첨부문서의 URL(http로 시작)을 사용할 수 도 있습니다.
    ...
    if rcp_no.startswith('http'):
        ...
        dcm_no = None
        matches = re.findall(f"openPdfDownload\('{rcp_no}', '(.*)'\)", r.text)
        if matches:
            dcm_no = matches[0]

        if not dcm_no:
            print(f'{url} 다운로드 페이지를 포함하고 있지 않습니다')
...
def sub_docs(rcp_no, match=None):
    ...
    * match: 매칭할 문자열 (문자열을 지정하면 문서 제목과 가장 유사한 순서로 소트 합니다)
    ...
    doc_urls = []
    doc_titles = []
    multi_page_matches = re.findall(pattern, r.text)
    single_page_matches = re.findall('''viewDoc\('(.*?)', '(.*?)', '(.*?)', '(.*?)', '(.*?)', '(.*?)',''\);''', r.text)
    if  multi_page_matches: 
        for m in multi_page_matches:
            params = f'rcpNo={m[2]}&dcmNo={m[3]}&eleId={m[4]}&offset={m[5]}&length={m[6]}&dtd={m[7]}'
            url = f'http://dart.fss.or.kr/report/viewer.do?{params}'
            doc_urls.append(url)
            doc_titles.append(m[0])
    elif single_page_matches:
        m = single_page_matches[0]
        doc_titles.append(BeautifulSoup(r.text).find('title').text.strip())
        doc_urls.append(f'http://dart.fss.or.kr/report/viewer.do?rcpNo={m[0]}&dcmNo={m[1]}&eleId={m[2]}&offset={m[3]}&length={m[4]}&dtd={m[5]}')
    else:
        raise Exception(f'http://dart.fss.or.kr/dsaf001/main.do?rcpNo={rcp_no} 하위 페이지를 포함하고 있지 않습니다')
