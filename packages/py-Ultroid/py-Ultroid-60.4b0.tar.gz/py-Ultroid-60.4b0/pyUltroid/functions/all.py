...
async def get_ofox(codename):
    ...
    releases_url = ofox_baseurl + "releases?"
    device_url = ofox_baseurl + "devices/get?"
    releases = await async_searcher(releases_url)
    device = await async_searcher(device_url)
...
async def dler(ev, url, opts=None, download=False):
    try:
        ...
        ytdl_data = YoutubeDL(opts).extract_info(url=url, download=download)
