...
from deepcrawl.sitemaps.sitemap import DeepCrawlProjectSitemap
...
class DeepCrawlProject(ImmutableAttributesMixin):
    ...
    def load_project_sitemaps(
        ...
        """
        connection = connection or deepcrawl.DeepCrawlConnection.get_instance()
        self.sitemaps = connection.get_project_sitemaps(self.account_id, self.id, filters=filters, **kwargs)
    ...
    def get_project_sitemaps(
        ...
        """
        Get sitemaps for current instance

        * use_cache=True > get_project_sitemaps will return cached sitemaps or will do a call to DeepCrawl if sitemaps attribute is empty.
        * use_cache=False > get_project_sitemaps will call DeepCrawl api and will override sitemaps attribute.

        >>> self.get_project_sitemaps()
        [xSbcfwBTi, aWimWxmnc]
        ...
        """
        connection = connection or deepcrawl.DeepCrawlConnection.get_instance()
        if use_cache and self.sitemaps:
            return self.sitemaps
        return self.load_project_sitemaps(connection=connection, filters=filters, **kwargs)
