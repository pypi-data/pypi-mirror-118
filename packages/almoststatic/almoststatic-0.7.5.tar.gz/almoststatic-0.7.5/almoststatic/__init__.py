...
class Almoststatic():
    ...
    def get_media(self, media_obj):
        ...
        if self.media_prefix is None:
            media_prefix = os.path.join(
                self._relative_url(), self.content, self.media)
        else:
            media_prefix = self._relative_url() + self.media
        filename = os.path.join(media_prefix, media_obj)
        return filename
    ...
    def get_url(self, page):
        ...
        try:
            ...
            anchor = ''
        return f"{self._relative_url()}{page}{self.vars['url_suffix']}{anchor}"
    ...
    def render_content(self, content, widgets_envelope=None):
        ...
        for widget in content:
            ...
            if widget['type'] == 'include':
                ...
                text = self.render_template(widget['type']+'.html', widget=widget)
