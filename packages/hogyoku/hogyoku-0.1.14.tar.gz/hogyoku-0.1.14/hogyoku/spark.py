...
class SparkCache(object):
    ...
    def enable_global_dump(self): self.global_dump = True
    def disable_global_dump(self): self.global_dump = False

    def enable_dumpable(self): self.dumpable = True
    def disable_dumpable(self): self.dumpable = False
...
def spark_step(out, deps=[], dump=False, verbose=True, joinkeys=[], join_sample_rate=1.,
    ...
    class SparkStep(object):
        ...
        def __call__(self):
            ...
            if cache.global_dump or self.dump:
                if cache.dumpable and cache.dumphdfs(self.out) and self.verbose:
