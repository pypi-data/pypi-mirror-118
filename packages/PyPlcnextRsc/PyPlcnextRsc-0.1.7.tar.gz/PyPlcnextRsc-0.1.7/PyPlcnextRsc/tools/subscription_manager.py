...
class RecordSubsMaintainer(MaintainerBase):
    def __init__(self, sub: RecordingSubscription):
        super().__init__(sub)
...
class SubscriptionManager:
    ...
    def createRecordingSubscription(self, recordCount: int = 10, sampleRate: int = 100000) -> RecordSubsMaintainer:
        sub = self.subscriptionService.createRecordingSubscription(recordCount, sampleRate)
        maintainer = RecordSubsMaintainer(sub)
        self.all_subs.add(maintainer)
        return maintainer
