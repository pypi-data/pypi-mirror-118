...
const Test = class {
    ...
    goToScrive(id) {
        ...
        window.open(`${host}/public/?limeDocId=${id}&lang=${lang}&usePerson=${this.includePerson}&useCoworker=${this.includeCoworker}&clone=${this.cloneDocument}`);
    }
    files() {
        var _a;
    ...
    render() {
        ...
        return (core.h("section", null, core.h("limel-config", { config: { iconPath: '/static/limepkg_scrive/static/' } }), core.h("limel-button", { label: label, outlined: true, icon: "scrive", onClick: () => this.goToScrive(this.context.id) }), core.h("limel-checkbox", { label: cloneLabel, checked: this.cloneDocument, onChange: this.setCloneDocument }), core.h("p", null, cloneWarning)));
    }
