...
class Visualizer(Extractor):
    ...
    def set_jaal_formatted_ontology_node_df(self):
        ...
        self.node_df[self.node_df.duplicated(subset=['id'], keep=False)].to_excel(f'{self.out_dir}/duplicate_nodes.xlsx')
        self.node_df = self.node_df.drop_duplicates(subset=['id'], keep='first')
        self.node_df.to_excel(f'{self.out_dir}/nodes.xlsx')
    ...
    def set_jaal_formatted_ontology_edge_df(self):
        ...
        self.edge_df[self.edge_df.duplicated(subset=['from', 'to'], keep=False)].to_excel(f'{self.out_dir}/duplicate_edges.xlsx')
        self.edge_df = self.edge_df.drop_duplicates(subset=['from', 'to'], keep='first')
        self.edge_df.to_excel(f'{self.out_dir}/edges.xlsx')
